 ...
 * <PRE>
 * 1. ClassName :
 * 2. FileName  : CollectorMssql2000.java
 ...
 * </PRE>
 */
public class CollectorMssql2000 implements CollectorMssqlService{

	private static final Logger logger = Logger.getLogger(CollectorMssql2000.class);
	...
	public CollectorMssql2000() {

	}

	public CollectorMssql2000(Connection source, Connection target,	List<TargetDbmsDM> lsitdm) {
		this.con_org = source;
		this.con_tgt = target;
	...
	private int insertSYSINDEXKEYS() throws Exception {
	    ...
	    sb.append("\n          ON C.xtype = 'U'                                     ");                              
	    sb.append("\n         AND C.ID = B.ID                                       ");
	    sb.append("\n INNER JOIN ").append(src_nm).append("SYSUSERS D               ");
		sb.append("\n         ON D.uid = C.uid         ");
...
logger.debug(sb.toString());		
	...
	private int insertExtendedProperties() throws SQLException {
		StringBuffer sb = new StringBuffer(); 
		sb.append("\n SELECT ID, SMALLID, TYPE, NAME, CONVERT(nvarchar(3000),VALUE) AS VALUE  ");
		sb.append("\n   FROM ").append(src_nm).append("SYSPROPERTIES ");
logger.debug(sb.toString());
		getResultSet(sb.toString());
		
		sb = new StringBuffer();
		sb.append("\n INSERT INTO WAE_MS_EXTENDED_PROPERTIES (");
		sb.append("\n        CLASS,  MAJOR_ID,  MINOR_ID,  NAME,  VALUE, DB_NM, SCH_NM, DB_CONN_TRG_ID  ) ");
		sb.append("\n VALUES (?, ?, ?, ?, ?, ?, ?, ? ) ");
logger.debug(sb.toString());		
		int cnt = 0;
		if( rs != null ) {
			...
			while(rs.next()) {	
				pst_org.setBigDecimal(rsGetCnt++, rs.getBigDecimal("ID"));
				pst_org.setBigDecimal(rsGetCnt++, rs.getBigDecimal("SMALLID"));
	...
	private int insertSYSINDEXES() throws SQLException {
	    ...
	    sb.append("\n          ON B.xtype = 'U'                                     ");           
	    sb.append("\n         AND B.ID = A.ID                                       ");           
	    sb.append("\n INNER JOIN ").append(src_nm).append("SYSUSERS C               ");
		sb.append("\n         ON C.uid = B.uid         ");
...
logger.debug(sb.toString());
	...
	private int insertSYSCOLUMNS() throws SQLException {
		...
		sb.append("\n      , C.NAME AS   SCHEMA_NM                                       "); //sys.schemas 테이블 없음
...
logger.debug(sb.toString());
	...
	private int insertSYSOBJECTS() throws SQLException {
		...
		sb.append("\n      , A.REPLINFO        ");
		sb.append("\n      , A.PARENT_OBJ      ");
		sb.append("\n      , CONVERT(varchar(20), A.CRDATE, 120) AS  CRDATE         ");
		sb.append("\n      , A.FTCATID         ");
		sb.append("\n      , A.SCHEMA_VER      ");
		sb.append("\n      , A.STATS_SCHEMA_VER");
		sb.append("\n      , A.TYPE            ");
		...
		sb.append("\n INNER JOIN ").append(src_nm).append("SYSUSERS B               ");
		sb.append("\n         ON B.uid = a.uid         ");
		sb.append("\n  WHERE A.XTYPE = 'U'                   ");
...
logger.debug(sb.toString());
		...
		if( rs != null ) {
			...
			while(rs.next()) {
				...
				pst_org.setString(rsGetCnt++, rs.getString("CRDATE"));
	...
	private int insertwattbl() throws Exception {
		...
		sb.append("\n      , ''                    AS NUSE_SIZE                             ");
		sb.append("\n      , NULL                  AS ANA_DTM                               ");
		sb.append("\n      , TO_DATE(A.CRDATE, 'YYYY-MM-DD hh24:mi:ss') AS CRT_DTM                               ");
		sb.append("\n      , NULL                  AS CHG_DTM                               ");
		sb.append("\n      , 'A'                   AS TBL_CLLT_DCD                          ");
...
logger.debug(sb.toString());
	...
	private String updateDataCpct() throws SQLException {
		...
		tgtSQL = tgtSQL.append("\n CONVERT(NUMERIC(38,2),sum(o.reserved)*8192.0/1024.0/1024.0) ");
		tgtSQL = tgtSQL.append("\n from sysindexes o inner join sysobjects i on o.id = i.id    ");
		tgtSQL = tgtSQL.append("\n where o.indid in (0,1,225) and i.xtype = 'U'                ");
