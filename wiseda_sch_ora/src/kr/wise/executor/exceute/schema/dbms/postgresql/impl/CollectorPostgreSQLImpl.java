 ...
 * <PRE>
 * 1. ClassName :
 * 2. FileName  : CollectorPostgreSQLImpl.java
 * 3. Package  : kr.wise.executor.exceute.schema.dbms.postgresql.impl
 * 4. Comment  :
 * 5. 작성자   : insomnia
 * 6. 작성일   : 2014. 6. 17. 오후 6:07:16
 * </PRE>
 */
public class CollectorPostgreSQLImpl implements CollectorPostgresqlService{

	private static final Logger logger = Logger.getLogger(CollectorPostgreSQLImpl.class);
	...
	public CollectorPostgreSQLImpl() {

	}

	public CollectorPostgreSQLImpl(Connection source, Connection target, List<TargetDbmsDM> lsitdm) {
		this.con_org = source;
		this.con_tgt = target;
...
logger.debug(sb.toString());		
	...
	private int insertPG_DESCRIPTTION() throws SQLException {
		...
		sb.append("\n     , C.DESCRIPTION					");
		sb.append("\n   FROM PG_ATTRIBUTE A   				"); 
		sb.append("\n INNER JOIN PG_DESCRIPTION C    		");
		sb.append("\n on A.ATTRELID = C.OBJOID       		");
		sb.append("\n  and A.ATTNUM   = C.OBJSUBID     		");
		logger.debug(sb.toString());
		getResultSet(sb.toString());
	...
	private int deleteSchema() throws Exception {
		...
		strSQL.append("\n     WHERE DB_CONN_TRG_ID = '"+dbms_id+"' ");
		result = setExecuteUpdate_Org(strSQL.toString());
		logger.debug("delete WAE_POST_PG_STAT_USER_TABLES : " + result);

		strSQL.setLength(0);
		strSQL.append("\n  DELETE FROM WAE_POST_PG_DESCRIPTION ");
		strSQL.append("\n     WHERE DB_CONN_TRG_ID = '"+dbms_id+"' ");
		result = setExecuteUpdate_Org(strSQL.toString());
		logger.debug("delete WAE_POST_PG_DESCRIPTION : " + result);
	...
	private int insertwatcol() throws Exception {
		...
		sb.append("\n        LEFT JOIN                                                                     ");
		sb.append("\n        (                                                                             ");
		sb.append("\n         SELECT A.DB_NM, A.SCH_NM, A.TABLE_NAME, A.DB_CONN_TRG_ID                     ");
		sb.append("\n              , B.COLUMN_NAME                                                         ");
