...
public class VrfcRuleDAO {
	/**
	 * Logger for this class
	 */
	private static final Logger logger = Logger.getLogger(VrfcRuleDAO.class);
	

	//프로파일정보조회
	public Map<String, String> selectProfileInfo(String ruleRelId) throws SQLException, Exception {
	...
	public VrfcSqlGeneratorVO sqlGenerator(String ruleRelId ) throws SQLException, Exception {
		...
		//진단대상 DBMS_TYP_CD 조회
		VrfcRuleVO  vrfcVO = getSqlGenDbmsInfoByRuleRelId(ruleRelId, con); 
		...
		if(vrfcNm.equals("PT01")){
			//관계분석 상세
			WamPrfRelTblVO result = getPrfPT01Dtl(ruleRelId, con);
			//관계컬럼
			if(null != result){
				List<WamPrfRelColVO> list =getPrfPT01RelColLst(ruleRelId, con);
				result.setWamPrfRelColVO((ArrayList<WamPrfRelColVO>) list);
			}
			sqlGenMap.put("prfDtlVO", result);
			
			//진단대상 DBMS_TYP_CD 조회
			WamPrfMstrVO  prfmstrVO = getSqlGenDbmsInfoByPrfId(ruleRelId, con);
	...
	//프로파일 정보
	public VrfcRuleVO getSqlGenDbmsInfoByRuleRelId(String ruleRelId, Connection con ) throws SQLException{
		VrfcRuleVO  vrfcRuleVO = new VrfcRuleVO();
...
//		strSQL.append("\n          AND  S.EXP_DTM = TO_DATE('9999-12-31', 'YYYY-MM-DD')   ");
		...
		strSQL.append("\n           ON DB.EXP_DTM =TO_DATE('9999-12-31', 'YYYY-MM-DD')    ");
...
//        strSQL.append("\n      , A.PRF_KND_CD                                       ");
		...
		pstmt.setString(1, ruleRelId);
		pstmt.setString(2, ruleRelId);
		pstmt.setString(3, ruleRelId);  
		
		rs = pstmt.executeQuery();
		
		while(rs.next()) {
			
			vrfcRuleVO.setRuleRelId(rs.getString("RULE_REL_ID")); 
			vrfcRuleVO.setDbcTblNm(rs.getString("DBC_TBL_NM"));
			vrfcRuleVO.setDbcColNm(rs.getString("DBC_COL_NM"));			
			vrfcRuleVO.setDbConnTrgId(rs.getString("DB_CONN_TRG_ID"));
			vrfcRuleVO.setDbConnTrgPnm(rs.getString("DB_CONN_TRG_PNM"));
			vrfcRuleVO.setDbmsTypCd(rs.getString("DBMS_TYP_CD"));
			vrfcRuleVO.setDbSchId(rs.getString("DB_SCH_ID"));
			vrfcRuleVO.setDbSchPnm(rs.getString("DB_SCH_PNM"));
			
			vrfcRuleVO.setVrfcNm(rs.getString("VRFC_NM"));
			vrfcRuleVO.setVrfcRule(rs.getString("VRFC_RULE"));	
			vrfcRuleVO.setVrfcDescn(rs.getString("VRFC_DESCN"));
			vrfcRuleVO.setVrfcTyp(rs.getString("VRFC_TYP"));   
			vrfcRuleVO.setAdtCndSql(rs.getString("ADD_CND"));
			 
			vrfcRuleVO.setCdSql(rs.getString("CD_SQL"));
			vrfcRuleVO.setCdClsColNm(rs.getString("CD_CLS_COL_NM"));
			vrfcRuleVO.setCdClsNmColNm(rs.getString("CD_CLS_NM_COL_NM"));
			vrfcRuleVO.setCdIdColNm(rs.getString("CD_ID_COL_NM"));
			vrfcRuleVO.setCdNmColNm(rs.getString("CD_CLS_NM_COL_NM"));
			vrfcRuleVO.setCdTypCd(rs.getString("CD_TYP_CD"));
//			vrfcRuleVO.setPrfKndCd(rs.getString("PRF_KND_CD"));
		}
	...
	//컬럼분석
	public WamPrfColAnaVO getColAnaDtl(String ruleRelId, Connection con ) throws SQLException{
	...
	//유효값 분석
	public WamPrfEfvaAnaVO getPrfPC02Dtl(String ruleRelId, Connection con ) throws SQLException{
	...
	//사용자정의유효값
	public List<WamPrfEfvaUserDfVO> getPrfPC02UserDfLst(String ruleRelId, Connection con ) throws SQLException{
	...
	public MetaDmnItfVO getMetaDmnDtl (MetaDmnItfVO metaDmnVO, Connection metaCon ) throws SQLException{
		...
		strSQL.append("\n        AND CD.EXP_DTM =TO_DATE('9999-12-31', 'YYYY-MM-DD') ");
	...
	//날짜형식 분석
	public WamPrfDtfrmAnaVO getPrfPC03Dtl(String ruleRelId, Connection con ) throws SQLException{
	...
	//범위분석
	public WamPrfRngAnaVO getPrfPC04Dtl(String ruleRelId, Connection con ) throws SQLException{
	...
	//패턴분석 사용자정의패턴 
	public List<WamPrfPtrAnaVO> getPrfPC05UserDfLst(String ruleRelId, Connection con ) throws SQLException{
	...
	//관계분석
	public WamPrfRelTblVO getPrfPT01Dtl(String ruleRelId, Connection con ) throws SQLException{
		...
		strSQL.append("\n      , D.ADD_CND        AS PA_TBL_ADT_CND_SQL    ");		
		strSQL.append("\n   FROM WAM_PRF_MSTR M                            ");
	...
	//관계분석 컬럼
	public List<WamPrfRelColVO> getPrfPT01RelColLst(String ruleRelId, Connection con ) throws SQLException{
	...
	//중복분석 컬럼
	public List<WamPrfUnqColVO> getPrfPT02UnqColLst(String ruleRelId, Connection con ) throws SQLException{
	...
	//날짜형식분석 유형 조회
	public Map<String, String> selectPrfValType(String ruleRelId) throws SQLException, Exception {
		StringBuilder query = new StringBuilder();
		query.append("\n SELECT VALTYPE_CD            ");
	...
	public void saveProfileResult(String logId, String staDttm, String endDttm, ExecutorDM executorDM, ProfileResultDM resultDM, ProfileResultPatternDM resultPatternDM) throws SQLException, Exception {
		...
		try {
			...
			if(executorDM.getAnaDgr() != null){
				...
				pstmt = con.prepareStatement(" SELECT TO_CHAR(TO_DATE(ANA_STR_DTM,'YYYY-MM-DD HH24:MI:SS'),'YYYYMMDDHH24MISS') AS ANA_STR_DTM FROM WAM_PRF_RESULT WHERE PRF_ID = ? AND ANA_DGR = ? ");
				...
				pstmt = con.prepareStatement(" DELETE FROM WAM_PRF_ERR_DATA WHERE PRF_ID = ? AND ANA_STR_DTM = TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS')  ");
				...
				// 기존 오류패턴 및 오류패턴정보, 결과 삭제
				pstmt = con.prepareStatement(" DELETE FROM WAM_PRF_ERR_DATA_PKDATA WHERE PRF_ID = ? AND ANA_STR_DTM = TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS')  ");
				for(int i=0; i<prevStaDttmList.size(); i++) {
					pstmt.setString(1, executorDM.getShdJobId());
					pstmt.setString(2, prevStaDttmList.get(i));
					pstmt.executeUpdate();
			...
			//차수미존재시
				...
				query.append("\n SELECT TO_CHAR(TO_DATE(ANA_STR_DTM,'YYYY-MM-DD HH24:MI:SS'),'YYYYMMDDHH24MISS') AS ANA_STR_DTM ");
				query.append("\n   FROM WAM_PRF_RESULT ");
				query.append("\n  WHERE PRF_ID = ? ");
				query.append("\n    AND ANA_DGR IS NULL ");
			...
			query.append("\n )  VALUES ( ");
			query.append("\n          ?   "); //--PRF_ID ");
			query.append("\n        , TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS')  "); // --ANA_STR_DTM 
			query.append("\n        , TO_DATE(?, 'YYYY-MM-DD HH24:MI:SS')  "); // --ANA_END_DTM
			query.append("\n        , ?   "); //--ANA_DGR ");
			query.append("\n        , ?   "); //--ANA_CNT ");
			query.append("\n        , ?   "); //--ESN_ER_CNT ");
			query.append("\n        , ?   "); //--ANA_LOG_ID ");
...
//				logger.debug(query.toString());
	...
	public void saveProfilePkData(String staDttm, ExecutorDM executorDM, TargetDbmsDM targetDbmsDM) throws Exception{
		...
		try {
			...
			String ruleRelId = executorDM.getShdJobId();
			//테이블, PK 컬럼 조회
			TargetDbmsDAO targetDbmsDAO = new TargetDbmsDAO();
			Map<String, String> dbcInfo = targetDbmsDAO.selectPkDbcInfo(con, ruleRelId );
			 
			 //PK 컬럼 존재시
			 if(dbcInfo.size() > 0 ){
			 	//데이터패턴 컬럼 건수
				int pkColCnt = 0; 
			 	pkColCnt =  selectColCnt(con, ruleRelId, staDttm);

			 	if(pkColCnt > 0){
				 	//PK DATA 조회시 사용할 조건 컬럼
			 		Map<String, String>  colMap = selectCol(ruleRelId, staDttm, pkColCnt, con);
				    
				    //PK DATA 조회시 사용할 조건 컬럼값
				    List<Map<String, String>> colValList = selectColData(con, ruleRelId, staDttm, executorDM.getPkDataLdCnt(), pkColCnt);
...
//			logger.debug(profileResultPatternDM.getTotalCount()+":"+profileResultPatternDM.getPatternCount());
	...
	public ProfileResultPatternDM executeFrmPatternQueryOfTargetDbms(Connection con, String query, VrfcSqlGeneratorVO vo, int colLimit) throws SQLException {
		...
		try {
			...
			while(rs.next()){
				...
				String sPattern = UtilString.null2Blank(rs.getString("PATTERN"));
				String sRule    = vo.getVrfcRule();
				
				totCnt += rs.getLong("COUNT");
				
				if(!Pattern.matches(sRule, sPattern)) {
					
					n = 0;
					patCnt ++;
					errCnt += rs.getLong("COUNT"); 
...
//		case "HH:MM":
//		case "MM":
//			sdf = new SimpleDateFormat(dateFormat);	//yyyyMMddHHmmss
...
//		}
		...
		if(!dateFormat.isEmpty() && (dateFormat.equals("YYYY") || dateFormat.equals("YYYYMM") || dateFormat.equals("YYYYMMDD") || dateFormat.equals("YYYYMMDDHH24MISS") 
				|| dateFormat.equals("YYYY-MM-DD") || dateFormat.equals("YYYY-MM") || dateFormat.equals("YYYY-MM") || dateFormat.equals("HH:MM") || dateFormat.equals("MM"))) {
			sdf = new SimpleDateFormat(dateFormat);	//yyyyMMddHHmmss
			sdf.setLenient(false);
		} else {
			ptrFlag = false;
...
//		ProfileResultPatternDM profileResultPatternDM = null;
	...
	//					logger.debug("=== error date format ===");
	//					logger.debug("ERRNO["+patCnt+"]PATTERN["+tmpDate+"]COUNT["+tmpCount+"]");
						errPatCnt ++;
						errCnt += tmpCount;
					...
					String sPattern = UtilString.null2Blank(tmpDate);
					String sRule    = dateFormat;
...
//			logger.debug(profileResultPatternDM.getTotalCount()+":"+profileResultPatternDM.getPatternCount());
	...
	public Map<String, String> selectDateType(String ruleRelId) throws Exception {
		...
		logger.debug( String.format("%s \n param [%s]", strSQL.toString(), ruleRelId));
		...
		try {
			...
			if(rs.next()) {
				//map = new HashMap<String, String>();
				map.put("DATE_FRM_CD"		, rs.getString("DATE_FRM_CD"));
	...
	/**
		 ...
		 * @throws Exception
		 */
		public ProfileResultPatternDM executeValidPatternProfileOfTargetDbms(Connection con, String query, int limitCnt, Map<String, String> validPat) throws SQLException, Exception { 
			PreparedStatement pstmt = null;
			ResultSet rs = null;
			ProfileResultPatternDM profileResultPatternDM = null;
...
//						logger.debug("=== error pattern format ===");
//						logger.debug("ERRNO["+patCnt+"]PATTERN["+strReg+"]COUNT["+tmpCount+"]");
...
//					logger.debug("NO["+patCnt+"]PATTERN["+strReg+"]COUNT["+tmpCount+"]");
		...
		//유효패턴 분석 패턴 리스트 조회
		public Map<String, String> selectValidPattern(String ruleRelId) throws Exception {
			StringBuilder strSQL = new StringBuilder();
		...
		//조건 컬럼 건수
		public int  selectColCnt(Connection con, String ruleRelId, String exeStaDttm) throws SQLException{
			
			//패턴 컬럼 건수 조회
			StringBuffer strSQL = new StringBuffer();
			strSQL.append("\n SELECT COL_CNT ");
			strSQL.append("\n  FROM WAM_PRF_ERR_DATA ");
			strSQL.append("\n WHERE PRF_ID = '").append(ruleRelId).append("' ");
...
//				if(con   != null) try { con.close(); } catch(Exception igonred) {}
		...
		//조건 컬럼 조회
		public Map<String, String>  selectCol(String ruleRelId, String exeStaDttm, int colCnt, Connection con) throws SQLException{
...
//				if(con   != null) try { con.close(); } catch(Exception igonred) {}
		...
		//조건 컬럼값 조회
		public List<Map<String, String>>  selectColData(Connection con, String ruleRelId, String exeStaDttm, BigDecimal PkDataLdCnt, int colCnt) throws SQLException{
