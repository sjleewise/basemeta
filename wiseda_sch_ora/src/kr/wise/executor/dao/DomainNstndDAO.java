...
public class DomainNstndDAO {
	...
	public void saveResult(String logId, String exeStaDttm, ExecutorDM executorDM, String rqstNo) throws Exception {
		//결과파일을 로드하여 저장한다.
		logger.debug("*****Start 비표준 Domain Result["+rqstNo+"]*****");
		...
		try{
			...
			//String[] arrdmn = {"금액","날짜","내용","명칭","번호","수","율","코드"};
			
			String[] arrdmn = {"금액","날짜","내용","명칭","번호","수","연락처","율","코드","플래그"};
			
			
			strSQL.setLength(0);
			strSQL.append("\n DELETE FROM WAD_DMN_PDT_NSTND  ");
			strSQL.append("\n  WHERE ANL_VAR_ID = ?          ");
			
			String delsql = strSQL.toString();
			
			strSQL.setLength(0);
			strSQL.append("\n    INSERT INTO WAD_DMN_PDT_NSTND                    ");
			strSQL.append("\n    (                                                ");
...
//					logger.debug("*****COUNT:["+cnt+"]");
			...
			logger.debug("*****End 비표준 Domain Result Save ["+rqstNo+"]*****");
