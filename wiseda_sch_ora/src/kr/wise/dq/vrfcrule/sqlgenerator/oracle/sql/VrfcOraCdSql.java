...
public class VrfcOraCdSql {
	...
	private String cdTypCd = new String();
	...
	public VrfcOraCdSql(Map<String, Object> sqlGenMap){
		...
		this.cdTypCd =   VrfcruleVO.getCdTypCd();
		...
		if(cdTypCd.equals("LC")) { //목록성코드
			sTemp += "\n SELECT 코드 " ;  
			sTemp += "\n   FROM ( ";
			sTemp += "\n   "+ codeSql;
			sTemp += "\n   ) C ";
			
		}else if(cdTypCd.equals("CC")) { //공통코드
			sTemp += "\n SELECT 코드 " ;  
			sTemp += "\n   FROM ( ";
			sTemp += "\n   "+ codeSql;
			sTemp += "\n   ) C ";
