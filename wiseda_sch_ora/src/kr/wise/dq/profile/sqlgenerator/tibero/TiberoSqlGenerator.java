	...
	public SqlGeneratorVO getSql(Map<String, Object> sqlGenMap)  {
		...
		if(prfKndCd.equals("PC01")){
			...
			if("Y".equals(prfDtlVO.getStdvAnaYn())){
				stddevValSql = tiberoPC01Sql.getStddevSql();
			}
			//분산분석 VRN_ANA_YN
			if("Y".equals(prfDtlVO.getVrnAnaYn())){
				varianceValSql = tiberoPC01Sql.getVarianceSql();
			}
			//평균분석 AVG_ANA_YN
			if("Y".equals(prfDtlVO.getAvgAnaYn())){
				avgValSql = tiberoPC01Sql.getAvgSql();
			}
			//유일값수분석
			if(UtilString.null2Blank(prfDtlVO.getMinMaxValAnaYn()).equals("Y")){
				unqCntSql = tiberoPC01Sql.getUnqCntSql();
			}
			//최소빈도값분석
			if(UtilString.null2Blank(prfDtlVO.getMinMaxValAnaYn()).equals("Y")){
				minCntValSql = tiberoPC01Sql.getMinCntValSql();
			}
			//최대빈도값분석
			if(UtilString.null2Blank(prfDtlVO.getMinMaxValAnaYn()).equals("Y")){
				maxCntValSql = tiberoPC01Sql.getMaxCntValSql();
			}
			
			dateYnSql  = tiberoPC01Sql.getDateYnSql();
			telYnSql   = tiberoPC01Sql.getTelYnSql();
			spaceRtSql = tiberoPC01Sql.getSpaceRtSql();
			crlfYnSql  = tiberoPC01Sql.getCrlfYnSql();
			alphaYnSql  = tiberoPC01Sql.getAlphaYnSql();
			dataFmtSql  = tiberoPC01Sql.getDataFmtSql();
			numYnSql    = tiberoPC01Sql.getNumYnSql();
			hundRtSql   = tiberoPC01Sql.getHundRtSql();
			cntRtSql    = tiberoPC01Sql.getCntRtSql();
