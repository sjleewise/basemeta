...
public class Db2PC05Sql {
	...
	public Db2PC05Sql(Map<String, Object> sqlGenMap){
...
//		this.dbcTblNm = prfMstrVO.getDbcTblNm();
	...
	public String getErrorCountSql() 
	{
		...
		strQuery.append(" SELECT VALUE(COUNT(*),0) AS ERR_CNT ");
		//패턴분석 SQL
	...
	public String getErrorValidPatternCondSql()  {
		...
		strQuery.append("\n   FROM (SELECT A."+dbcColNm );
		strQuery.append("\n               ,REPLACE(TRAN_DATA1,TRAN_DATA2, LPAD('C',TRAN_DATA2_LENGTH, 'C')) AS PATTERN ");
		strQuery.append("\n           FROM (SELECT A."+dbcColNm );
		strQuery.append("\n                        ,A.TRAN_DATA1 ");
		strQuery.append("\n                        ,COALESCE(REPLACE(REPLACE(A.TRAN_DATA1,'"+G_TRANS_STRING+"', LPAD(' ', "+G_TRANS_STRING.length()+",' ')), ' ', ''),'C') AS TRAN_DATA2 ");
		strQuery.append("\n                        ,LENGTH(COALESCE(REPLACE(REPLACE(A.TRAN_DATA1,'"+G_TRANS_STRING+"', LPAD(' ', "+G_TRANS_STRING.length()+",' ')), ' ', ''),'C')) AS TRAN_DATA2_LENGTH ");
