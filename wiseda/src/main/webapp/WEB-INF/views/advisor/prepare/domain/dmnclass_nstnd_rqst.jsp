...
// 	alert('${search.rqstNo}');
    ...
    $("#btnDmnLearn").click(function(){
    	// 처리중이니 잠시 기다려 주십시요.
		/* 기존소스_bak 181019  */
    	/* showMsgBox("PRC", "처리중이니 잠시 기다려 주십시요."); */
    	
    	showMsgBox("PRC", "<s:message code='PRC.WAIT' />");
    }).hide();
    
    
  //도메인 판별 요청...
    $("#btnDmnPdtExec").click(function(){
    	//체크박스 있는지 확인한다....
		/* 기존소스_bak 181019  */
    	/* if(checkDelIBS (grid_tbl, "도메인판별을 요청할 데이터셋이 없습니다.<br>목록에서 테이블을 선택 후 실행해 주십시요.")) { */
    	
    	if(checkDelIBS (grid_tbl, "<s:message code='MSG.DMNCLASS.RQST' />")) {

			for(var i = 1; i <= grid_tbl.RowCount() ; i++) {

				var vChk = grid_tbl.GetCellValue(i,"ibsCheck");

				if(vChk != "1") continue;
				
				var prfId = grid_tbl.GetCellValue(i,"prfId");

				if(prfId == ""){

					showMsgBox("ERR","컬럼프로파일 실행후 비표준도메인판별 가능합니다.");
					return;
				}
	        }
			...
			//삭제 확인 메시지
			//alert("삭제하시겠어요?");
			showMsgBox("CNF", "선택한 테이블에 속한 모들 컬럼에 대해 도메인판별을 요청합니다.<br>도메인판별을 요청하시겠습니까?", 'DmnPdtExec');
    	}
...
// 	$("#frmSearch #schDbSchId").append('<option value="">선택</option>');
	...
	$("#frmSearch #schDbSchId").change(function() {
		...
		param.srcDbcSchNm = $("#schDbSchId option:selected").text();
...
function initGrid()
{
    ...
    with(grid_tbl){
        ...
        var headers = [
                    {Text:"<s:message code='BDQ.HEADER.DMNCLASS.NSTND.RQST'/>", Align:"Center"}
                ];
        //No|상태|선택|주제영역ID|주제영역명|진단대상ID|진단대상명|스키마ID|스키마명|테이블명|테이블한글명|데이터셋ID|데이터셋(논리명)|데이터셋(물리명)|메타연계분석|PRF_ID'/>
...
//         InitComboNoMatchText(1, "");
	...
	//진단대상 컬럼 grid
   	with(grid_col){
           ...
           var headers = [
                       {Text:"<s:message code='BDQ.HEADER.DMNPREDICT.LST3'/>", Align:"Center"}
                   ];
           //Position|상태|변수ID|프로파일ID|주제영역ID|주제영역명|진단대상ID|진단대상명|스키마ID|스키마명|테이블명|컬럼명|컬럼한글명|Pk여부|데이터타입|
           //변수타입|추천도메인|추천도메인확률|최종도메인|품질지표|Null여부|Default|메타연계분석|최소값|최대값
           ...
           var cols = [                        
                   ...
                   {Type:"Text",   Width:100,  SaveName:"varType",    	Align:"Left", Edit:0, Hidden:1},
                   {Type:"Text",   Width:100,  SaveName:"dmnPdt",    	Align:"Left", Edit:0, Hidden:0},
                   {Type:"Float",   Width:100,  SaveName:"dmnPrb",    	Align:"Center", Edit:0, Hidden:0, Format:"#0.0"},
                   {Type:"Combo",   Width:100,  SaveName:"dmngNm",    	Align:"Left", Edit:1},
                   {Type:"Text",   Width:100,  SaveName:"dqiNm",    	Align:"Left", Edit:0, Hidden:1},
	   ...
	   /* SetColProperty("dmngNm", {ComboText: '번호|금액|날짜|내용|명칭|수|율|코드', ComboCode: '번호|금액|날짜|내용|명칭|수|율|코드'}); */
	   SetColProperty("dmngNm", {ComboText: '금액|날짜|내용|명칭|번호|수|연락처|율|코드|플래그', ComboCode: '금액|날짜|내용|명칭|번호|수|연락처|율|코드|플래그'});
...
//         	grid_tbl.DoSearch("<c:url value="/dq/criinfo/anatrg/getPrfTblLst.do" />", param);
        	grid_tbl.DoSearch("<c:url value="/advisor/prepare/dataset/getDataSetNstndList.do" />", param);
        	break;
        	
       	/*진단대상 컬럼 조회*/
        case "SearchAnaTrgCol":
//         	grid_col.DoSearch("<c:url value="/dq/profile/getPrfColLst.do" />", param);
        	grid_col.DoSearch("<c:url value="/advisor/prepare/dataset/getAanVarNstndList.do" />", aparam);
        	break;
...
// 	    	if(SaveJson.data.length == 0) return;
			...
			var url = "<c:url value="/advisor/prepare/domain/requestNstndDmnClass.do"/>";
        ...
        case "SaveDmnResult":
        	...
        	if(!rows) {
//         		alert("저장할 대상이 없습니다...");
        		showMsgBox("ERR", "변경한 도메인이 없습니다.<br>목록에서 도메인을 변경 후 실행해 주십시요.");
        		return;
        	}
...
// 	    	if(SaveJson.data.length == 0) return;
			...
			var url = "<c:url value="/advisor/prepare/domain/regnstnddmnresult.do"/>";
			var param = ""; //$("#mstFrm").serialize();
			IBSpostJson2(url, SaveJson, param, ibscallback);
...
function grid_col_OnClick(row, col, value, cellx, celly) {
	...
	$('div#dmn_add_feature').load('<c:url value="/advisor/prepare/domain/getnstnddmnfeature_dtl.do"/>', param1, function(){});
...
function grid_col_OnSearchEnd(code, message, stCode, stMsg) {
	...
	if(code  < 0) {
		...
		for(var i = headrow; i < datarow+headrow; i++) {
			...
			case "코드" : 
			case "플래그" :
				grid_col.SetCellValue(i, "dqiNm", "코드 정확성(CodeAccuracy)");
				break;
			case "수" :
			case "금액" :
				grid_col.SetCellValue(i, "dqiNm", "이상값 탐지");
				break;
			case "번호" :
			case "율" :
				grid_col.SetCellValue(i, "dqiNm", "범위-패턴 정확성(patternaccuracy)");
				break;
			case "날짜" :
				grid_col.SetCellValue(i, "dqiNm", "일시-패턴 정확성(patternaccuracy)");
				break;
			case "연락처" :
				grid_col.SetCellValue(i, "dqiNm", "형식-패턴 정확성(patternaccuracy)");
				break;
			case "명칭" :
				grid_col.SetCellValue(i, "dqiNm", "텍스트중복");
				break;
			case "내용" :
				grid_col.SetCellValue(i, "dqiNm", "업무규칙 작성");
				break;
			//case "ID" :
			//	grid_col.SetCellValue(i, "dqiNm", "조건 완전성(Condition Completeness)");
			//	break;
	                ...
	                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='CLMN.PROF.MNG'/>">
	                   ...
	                   <tbody>                            
	                       <tr>                               
	                           ...
	                           <td>
	                           		<select id="schDbSchId" name="schDbSchId" class="wd98p">
	                           		</select>
...
<%-- 			    <button class="btn_save" id="btnPrfSave" 	name="btnPrfSave"><s:message code="STRG" /></button> <!-- 저장 -->  --%>
					<button class="btn_rqst_new2" id="btnDmnPdtExec" 	name="btnDmnPdtExec">도메인판별 실행</button> <!--조회-->
...
<%-- 			    <button class="btn_save" id="btnPrfSave" 	name="btnPrfSave"><s:message code="STRG" /></button> <!-- 저장 -->  --%>
				<button class="btn_rqst_new2" id="btnSaveDmn" 	name="btnSaveDmn">도메인 저장</button>
				<button class="btn_rqst_new2" id="btnDmnLearn" 	name="btnDmnLearn">도메인 판별 재학습</button>
<%-- 			    <button class="btn_delete" id="btnPrfDelete" 	name="btnPrfDelete"><s:message code="DEL" /></button> <!-- 삭제 -->  --%>
...
<!-- 컬럼프로파일 상세 조회 -->
<div id="dmnpdttab" style="width:30%; float: left;">
<div class="stit">도메인판별상세</div><!--검색조건-->
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li id="tab-pc01"><a href="#tabs-pc01">도메인결과</a></li><!--컬럼분석-->
	    <li id="tab-pc02"><a href="#tabs-pc02">컬럼분석</a></li><!--코드분석-->
	    <li id="tab-pc03"><a href="#tabs-pc03">데이터패턴</a></li><!--날짜형식분석-->
	    <li id="tab-pc04"><a href="#tabs-pc04">파생변수</a></li><!--날짜형식분석-->
	  </ul>
	  <div id="tabs-pc01">
	  	<%@include file="nstnddmnpdtresult_dtl.jsp" %>
	  </div>
	  <div id="tabs-pc02">
	  <div id="colanaresdtl"></div>
	  </div>
	  <div id="tabs-pc03">
	  	<%@include file="colpc02dataptr_dtl.jsp" %>
	  </div>
	  <div id="tabs-pc04">
	  <div id="dmn_add_feature"></div>
	  </div>
