...
function initGridBr()
{
    ...
    with(grid_br){
        ...
        //No.|상태|검토상태|검토내용|요청구분|등록유형|검증결과|업무영역ID|업무영역명|업무규칙ID|업무규칙명|업무규칙담당자ID|업무규칙담당자|진단대상ID|진단대상명|진단대상스키마ID|진단대상스키마명|테이블명|컬럼명|품질지표ID|품질지표명|중요정보항목ID|중요정보항목명|사용여부|설명|건수SQL|분석SQL검증대상DBID|검증대상DB명|검증테이블명|검증컬럼명|검증비교KEY컬럼|검증비교값컬럼|검증JOIN방식|건수SQL(검증)|분석SQL(검증)|표준편차|분산|평균|유일값수|최소빈도값|최대빈도값
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   	Width:50,  SaveName:"dbSchPnm",   	Align:"Left", Edit:0, Hidden:1, KeyField:1},
...
// 	        	detailPc05();
        ...
        case "Down2Pdf":
        	...
        	if(grid_pt01.SearchRows()>0){
        		grid_pt01.Down2Pdf({FileName:'profile_list'}); /*관계분석*/
        	}
        	if(grid_pc01.SearchRows()>0){
        		grid_pt02.Down2Pdf({FileName:'profile_list'}); 
        	}
        	if(grid_pc01.SearchRows()>0){
        		grid_pc01.Down2Pdf({FileName:'profile_list'}); /*컬럼분석*/
        	}
        	if(grid_pc02.SearchRows()>0){
        		grid_pc02.Down2Pdf({FileName:'profile_list'}); /*코드분석*/
        	}
        	if(grid_pc03.SearchRows()>0){
        		grid_pc03.Down2Pdf({FileName:'profile_list'}); 
        	}
        	if(grid_pc04.SearchRows()>0){
        		grid_pc04.Down2Pdf({FileName:'profile_list'}); 
        	}
        	if(grid_pc05.SearchRows()>0){
        		grid_pc05.Down2Pdf({FileName:'profile_list'}); /*<!--패턴분석-->*/
        	}
        ...
        <input type="hidden" name="anaStrDtm" id="anaStrDtm" />
            ...
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='PROF.INQ'/>"><!--프로파일조회-->
                   ...
                   <tbody>                            
                       <tr>                               
                           <th scope="row" class="th_require"><label for="dbSchId"><s:message code="DB.MS.SCHEMA.NM"/></label></th><!--진단대상명/스키마명-->
