...
function grid_sheet2_OnSearchEnd(code) {
	if(code < 0) {
		...
		//조회 성공....
		var strVal = 1;
		var endVal = grid_sheet2.SearchRows();
...
// 	progChart.RemoveAll();
	...
	if(grid_sheet2.SearchRows() == 0) return;
...
function chartDraw(strVal, endVal)	{
	...
	for(var i=strVal; i<= endVal; i++) {
		category.push(grid_sheet2.GetCellValue(i, "bizAreaLnm"));
	}
...
// 	var seriesSigma = progChart.CreateSeries();
	...
	for(var i=strVal, j=0; i<=endVal; i++, j++) {
		pointsQuality.push({x:j, y:grid_sheet2.GetCellValue(i, "erRate"), name:grid_sheet2.GetCellText(i, "fullPath")});
		pointsSigma.push({x:j, y:grid_sheet2.GetCellValue(i, "sigma"), name:grid_sheet2.GetCellText(i, "fullPath")});
	}
...
<!-- 그리드 입력 입력 -->
<div class="grid_01">
     <script type="text/javascript">createIBSheet("grid_sheet2", "100%", "250px");</script>            
</div>
