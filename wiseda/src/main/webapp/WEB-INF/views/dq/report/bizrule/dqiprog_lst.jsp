...
$(document).ready(function() {
	...
	createIBChart("dqiChart", "dqiChart", {
		width: "auto",
		height: "350px"
	});
});
...
function initGrid()
{
    with(grid_sheet){
        ...
        var cols = [                        
		            {Type:"Seq",    	Width:80,   SaveName:"ibsSeq",      Align:"Center", Edit:0, Hidden:0},
		            ...
		            {Type:"Text",   	Width:100,  SaveName:"dbcTblNm",   	Align:"Left", Edit:0, Hidden:1, ColMerge:1},
		            {Type:"Text",   	Width:100,  SaveName:"dbcColNm",   	Align:"Left", Edit:0, Hidden:1, ColMerge:1},
		            ...
		            {Type:"Text",   	Width:150,  SaveName:"anaDgrDisp",   	Align:"Left", Edit:0, Hidden:0, ColMerge:0},
		            {Type:"Text",   	Width:100,  SaveName:"brCnt",   	Align:"Right", Edit:0, Hidden:0, ColMerge:0},
		            ...
		            {Type:"Int",   	Width:100,   SaveName:"weight", 	Align:"Right", Edit:0, Hidden:0, ColMerge:0},
		            {Type:"Text",   	Width:100,   SaveName:"erRate", 	Align:"Right", Edit:0, Hidden:0, ColMerge:0},
		            {Type:"Text",   	Width:100,   SaveName:"goal", 	Align:"Right", Edit:0, Hidden:0, ColMerge:0},
		            {Type:"Text",   	Width:100,   SaveName:"archRate", 	Align:"Right", Edit:0, Hidden:0, ColMerge:0, Hidden:1},
		            {Type:"Text",   	Width:100,   SaveName:"dpmo", 	Align:"Right", Edit:0, ColMerge:0, Hidden:1},
		            {Type:"Text",   	Width:100,   SaveName:"sigma", 	Align:"Right", Edit:0, Hidden:0, ColMerge:0}
                ];
...
// 	    	getLineChart(param);
  		...
  		case "Down2Excel":  //엑셀내려받기
            grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'데이터품질지표별품질현황'});
//             dqiChart.Down2Image({FileName:"DqiProg", Type:IBExportType.JPEG, Width:800, Url:"./../../../js/IBChart/jsp/Down2Image.jsp"});
            break;
    }
}
...
function chartDraw(strVal, endVal)	{
	...
	var category = new Array(); // 시리즈 이름
	var titleText = new Array(); // 타이틀
	
	//차트 카테고리값 설정... (컬럼명 검색시는 차트표시 안하므로 return)
	if($("#sortTyp").val() == "dbConnTrgLnm") {
		titleText = grid_sheet.GetCellValue(strVal, "dbConnTrgLnm");
		for(var i=strVal, j=0; i<=endVal; i++, j++) {
			category.push (grid_sheet.GetCellValue(strVal+j, "dqiLnm"));
		}
	} else if($("#sortTyp").val() == "dbcTblNm") {
		titleText = grid_sheet.GetCellValue(strVal, "dbcTblNm");
		for(var i=strVal, j=0; i<=endVal; i++, j++) {
			category.push (grid_sheet.GetCellValue(strVal+j, "dqiLnm"));
		}
	} else if($("#sortTyp").val() == "dbcColNm") {
		titleText = grid_sheet.GetCellValue(strVal, "dbcColNm");
		for(var i=strVal, j=0; i<=endVal; i++, j++) {
			category.push (grid_sheet.GetCellValue(strVal+j, "dqiLnm"));
		}
	}
	...
	dqiChart.setOptions(toCamel({
		...
		XAxis : {
			...
			Title : { //X축 제목 설정
				Text : "<s:message code='DATA.QLTY.INDC.NM'/>" + "(" + titleText + ")"/*데이터품질지표명*/
			},
			Opposite : 0
		},
		YAxis : [{
	...
	//시리즈 생성
	var seriesQuality = toCamel({
		Name:"<s:message code='QLTY.SCR'/>",
		Type:"spline"
	});
	
	var seriesSigma = toCamel({
	...
	for(var i=strVal, j=0; i<=endVal; i++, j++) {
		pointsQuality.push(toCamel({X:j, Y:grid_sheet.GetCellValue(i, "erRate"), Name:grid_sheet.GetCellText(i, "fullPath")}));
		pointsSigma.push(toCamel({X:j, Y:grid_sheet.GetCellValue(i, "sigma"), Name:grid_sheet.GetCellText(i, "fullPath")}));
	}
	
	console.log(pointsQuality);
	console.log(pointsSigma);
	
	seriesQuality.data = pointsQuality;
	seriesSigma.data = pointsSigma;
	
	dqiChart.addSeries(seriesQuality);
	dqiChart.addSeries(seriesSigma);
...
<div class="main_chart_01" id="dqiChart">
        	
            <%-- <script type="text/javascript">createIBChart("dqiChart", "100%", "350px");</script> --%>
</div>
