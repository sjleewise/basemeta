...
// 			doAllApprove(grid_sheet, "2");
		...
		$("#btnReqApprove").click(function(){
			...
			if (cntappr > 0) message += "[DDL 테이블:"+cntappr+"건 중 승인 "+cntAppr+"건, 반려 "+cntReject+"건]<br>";
        ...
        $("#btnBlank").click( function(){
    		location.href = '<c:url value="/meta/ddltsf/ddltsftbl_rqst.do" />';
        } );
...
//         initGrid();
        ...
        $("#btnNew").click(function(){
			var url   = "<c:url value="/meta/ddltsf/popup/ddltsftbl_pop.do"/>";
        ...
        $("#btnDelete").click(function(){ 
    		...
    		if(checkDelIBS (grid_sheet, "삭제할 대상이 없습니다.")) {
    			//삭제 확인 메시지
    			//alert("삭제하시겠어요?");
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        //No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|DDL테이블ID|테이블(물리명)|테이블(논리명)|DDL|소스DB접속대상|소스DB스키마|타겟DB접속대상|타겟DB스키마
        //|테이블스페이스|테이블변경유형|물리모델테이블ID|스크립트정보|요청일시|요청자ID|요청자명|설명|요청번호|요청일련번호|TGT_DB_CONN_TRG_ID

        var headtext  = "<s:message code='META.HEADER.DDLTSFTBL.RQST1'/>";
        	headtext += "<s:message code='META.HEADER.DDLTSFTBL.RQST2'/>";
            headtext += "<s:message code='META.HEADER.DDLTSFTBL.RQST3'/>";
            headtext += "<s:message code='META.HEADER.DDLTSFTBL.RQST4'/>";
		...
		SetColProperty("odsYn", 	{ComboCode:"N|Y", 	ComboText:"아니요|예"});
...
// 			if (saveJson.Code == "IBS000") return;
			...
			var url = "<c:url value="/meta/ddl/regDdlTsfSaveRow.do" />";  
		...
		case "Search":
			...
			//grid_sheet.DoSearch("<c:url value="/meta/ddltsf/getddltsftblrqstlist.do" />", param);
			
			grid_sheet.DoSearch("<c:url value="/meta/ddl/getDdlTsfTblRqstList.do" />", param);
...
function loadDetail(param, flag) {
	...
	$('div#detailInfo').load('<c:url value="/meta/ddltsf/ddltsftbl_rqst_dtl.do"/>', param, function( res, status, xhr ) {	
		  if ( status == "error" ) {
...
//DB별 테이블스페이스 셋팅한다.
function getTableSpac(obj, dbConnTrgId) {
	
	var vUrl = "<c:url value='/commons/damgmt/db/getDbTblSpacList.do'/>";
	...
	$.ajax({
		  ...
		  success: function(res){
			 ...
			 $.each(data, function(i, map){
				 ...
				 obj.append("<option value="+ map.dbTblSpacPnm +">"+ map.dbTblSpacPnm +"</option>");					   						   
			 });
...
function postProcessIBS(res) {
	...
	switch(res.action) {
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/ddltsf/ddltsftbl_rqst.do" />';
			if(!isBlankStr(res.resultVO.rqstNo)) {
...
<div style="clear:both; height:5px;"><span></span></div>  
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1">테이블정의서</a></li>  <!-- 테이블정의서 -->
	    <li><a href="#tabs-2">컬럼목록</a></li>      <!-- 컬럼목록 -->
	  <!--   <li><a href="#tabs-3-1">인덱스목록</a></li>  인덱스목록 -->
	    <c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S'}">
	    <li id="tabs-rqstvrf"><a href="#tabs-4">검증결과</a></li>  <!-- 검증결과 -->	    
	    </c:if>
	    <li><a href="#tabs-5">테이블변경항목</a></li>  <!-- 테이블변경항목 -->
	    <li><a href="#tabs-6" id="colinfo">컬럼변경항목</a></li>	<!-- 컬럼변경항목 -->   
	  </ul>
	  ...
	  <!-- 컬럼 목록 탭 -->
	  <div id="tabs-2">
		<%@include file="../ddl/ddlcol_rqst.jsp" %>  
	  </div>
<%-- 	  <div id="tabs-3-1">
		<%@include file="../ddl/ddltblidxcol_dtl.jsp" %>  
	  </div> --%>
	  <c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S'}">
	  <div id="tabs-4">
