...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        SetMergeSheet(1);
        var headers = [
                    {Text:"No.|DDL컬럼ID|만료일시|시작일시|이관단계|타겟DBMS명|타겟DB스키마명|소스DBMS명|소스DB스키마명|이관DDL컬럼물리명|이관DDL컬럼논리명|DDL테이블ID|컬럼순서|데이터타입|데이터길이|데이터소수점길이|PK여부|PK순서|NOTNULL여부|DEFAULT값|요청번호|요청일련번호|요청상세일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자명|요청일시|요청사용자명|승인일시|승인사용자명", Align:"Center"}
                ];
        ...
        var cols = [                        
					...
					{Type:"Text",   Width:80,   SaveName:"colOrd",	 	Align:"Center", Edit:0, Hidden:0, ColMerge:0},
...
function grid_sub_ddltblcolchange_OnSearchEnd(code, message) {
	//alert(grid_sub_ddltblcolchange.GetDataBackColor()+":"+ grid_sub_ddltblcolchange.GetDataAlternateBackColor());
	if(code < 0) {
