...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        var headers = [
                    {Text:"No.|이관단계|타겟DBMS명|타겟DB스키마명|소스DBMS명|소스DB스키마명|이관DDL컬럼ID|이관DDL컬럼물리명|이관DDL컬럼논리명|이관DDL테이블ID|컬럼순서|데이터타입|데이터길이|데이터소수점길이|PK여부|PK순서|NOTNULL여부|DEFAULT값|요청번호|요청일련번호|요청상세일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자명|요청일시|요청사용자명|승인일시|승인사용자명", Align:"Center"}
                ];
        ...
        var cols = [                        
					...
					{Type:"Text",   Width:100,   SaveName:"srcDbSchPnm", 		Align:"Left", Edit:0, Hidden:1},
						...
						{Type:"Text",   Width:120,   SaveName:"ddlColLnm", 		Align:"Left", Edit:0, Hidden:0},
...
function loadDetailCol(param) {
	$('div#detailInfo2').load('<c:url value="/meta/ddltsf/ajaxgrid/ddltsftblcolinfo_dtl.do"/>', param, function(){
...
function grid_sub_ddltblcollist_OnSearchEnd(code, message) {
	//alert(grid_sub_ddltblcollist.GetDataBackColor()+":"+ grid_sub_ddltblcollist.GetDataAlternateBackColor());
	if(code < 0) {
