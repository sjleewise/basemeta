...
// 		loadDetail();
		...
		setautoComplete($("#frmSearch #appStwdLnm"), "APD");
...
// 	alert('window.load');
	...
	if(!isBlankStr("${appStwdId}")) {
		var param = "appStwdId="+"${appStwdId}";
		grid_sheet.DoSearch("<c:url value="/meta/app/getStndWordlist.do" />", param);
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        SetConfig(cfg);
        var headers = [
                    {Text:"<s:message code='META.HEADER.APPSTWD.LST'/>", Align:"Center"}
                    /* No.|상태|선택|비표준단어ID|비표준단어논리명|비표준단어물리명|영문의미|요청번호|요청일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID */
                ];
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   Width:40,  SaveName:"appStwdId",    Align:"Left", Edit:0, Hidden:1},
                    {Type:"Text",   Width:150,  SaveName:"appStwdLnm",    Align:"Left", Edit:0, Hidden:0}, 
                    {Type:"Text",   Width:150,  SaveName:"appStwdPnm",    Align:"Left", Edit:0, Hidden:0}, 
                    {Type:"Text",   Width:200,  SaveName:"engMean",    Align:"Left", Edit:0, Hidden:0}, 
//                     {Type:"Text",   Width:80,  SaveName:"cchNm",    Align:"Left", Edit:0}, 
//                     {Type:"Text",   Width:150,  SaveName:"orgDs",    Align:"Left", Edit:0, Hidden:0},
//                     {Type:"Combo",   Width:80,  SaveName:"wdDcd",    Align:"Center", Edit:0, Hidden:0}, 
...
function loadDetail(param) {
	$('div#detailInfo1').load('<c:url value="/meta/app/ajaxgrid/appstwdinfo_dtl.do"/>', param, function(){
		if(!isBlankStr(param)) {
			//그리드 초기화 한다.
			initDtlGrids();
			
// 			grid_sub_stwdchange.DoSearch('<c:url value="/meta/app/ajaxgrid/appstwdchange_dtl.do" />', param);
...
// 			grid_sub_stwdchange.RemoveAll();
        ...
        case "Down2Excel":  //엑셀내려받기
            ...
            grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'변수단어조회'});
...
// 	if ('ibsSeq' == colsavename || 'ibsStatus' == colsavename || 'ibsCheck' == colsavename) return;
	...
	var appStwdId = "&appStwdId="+grid_sheet.GetCellValue(row, "appStwdId");
        ...
        <form id="frmSearch" name="frmSearch" method="post">
            ...
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='APP.WORD.INQ' />"><!-- APP단어조회 -->
                   ...
                   <tbody>                            
                            <tr>
                                <th scope="row"><label for="appStwdLnm"><s:message code="APP.WORD.NM" /></label></th> <!-- APP단어명 -->
                                <td><input type="text" id="appStwdLnm" name="appStwdLnm" class="wd200" /></td>
...
<!-- 	  <div id="tabs-2"> -->
			<!-- 	상세정보 ajax 로드시 이용 -->
<%-- 			<%@include file="appstwdchange_dtl.jsp" %> --%>
			<!-- 	상세정보 ajax 로드시 이용 END -->
<!-- 	  </div> -->
<!-- 	  <div id="tabs-3"> -->
			<!-- 	상세정보 ajax 로드시 이용 -->
