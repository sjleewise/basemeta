...
// 		$("#tabs").tabs().show();
		...
		//업무구분상세 초기화...
		$("#mstFrm #bizDtlCd").val('APM');

		$("#mstFrm #bizDCd").val('APM');
...
// 		//imgConvert($('div.tab_navi a img'));
		...
		$("#btnRegRqst").click(function(){
			...
			var message = "";
			if (cntrqst > 0) message += "[응용프로그램용어 : "+cntrqst+"]"; //물리모델, 건
...
// 			doAllApprove(grid_sheet, "1");
		...
		$("#btnReqApprove").click(function(){
			...
			if (cntappr > 0) message += "[응용프로그램용어:"+cntappr+"<s:message code='CNT.APRV' />  "+cntAppr+"<s:message code='CNT.RTN' /> "+cntReject+"]<br>";
			// 물리모델, 건 중 승인, 건, 반려, 건
...
//         create_selectbox(sysareaJson, $("#sysAreaId"));
      ...
      //신규요청서
        $("#btnBlank").click( function(){
    		location.href = '<c:url value="/meta/app/appprgmitem_rqst.do" />';
        } );
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.APPPRGMITEM.RQST'/>";

        	//headtext  = No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|응용프로그램용어ID|응용프로그램용어논리명|응용프로그램용어물리명|
        	//			+= 응용프로그램용어구분|담당자ID|담당자명|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "AddWam": //기존 테이블 추가
			var url   = "<c:url value="/meta/app/popup/appprgmitemSearchPop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/app/delappprgmitemrqstlist.do"/>";
...
// 			if (saveJson.Code == "IBS000") return;
			...
			var url = "<c:url value="/meta/app/regappprgmitemrqstlist.do"/>";
...
//         	ibsSaveJson = grid_sheet.GetSaveJson(1);	//DoAllSave와 동일...
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/app/getappprgmitemrqstlist.do" />", param);
			//코드도메인 조회...
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/app/approveappprgmitem.do"/>";
			var param = $('form[name=mstFrm]').serialize();
        ...
        case "Down2Excel":  //엑셀내려받기
            grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'APP용어등록요청'});
            break;
            
        case "LoadExcel":  //엑셀업로드
			
			var url = "<c:url value="/meta/app/popup/appprgmitem_xls.do" />";
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/meta/app/ajaxgrid/appprgmitem_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/app/appprgmitem_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
//     tblClick(row);
	...
	var tmphtml = '응용프로그램용어 : ' + param.appPrgmLnm + ' [' + param.appPrgmLnm +']'; //테이블
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs" style="display: none;">
	  <ul>
	    <li><a href="#tabs-1">응용프로그램용어 구성</a></li><!-- 테이블 정의서 -->
	    ...
	    <li id="tabs-rqstchg"><a href="#tabs-3">변경항목</a></li> <!-- 테이블변경항목 -->
	    </c:if>
	  </ul>
	  <div id="tabs-1">
		<!-- 	상세정보 ajax 로드시 이용 -->
