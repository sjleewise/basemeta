...
$(document).ready(function() {
    ...
    $("#btnSave").click(function(){
		icons: { primary: "ui-icon-lightbulb" }
    		...
    		//저장할래요? 확인창...
    		var message = "<s:message code='MSG.STRD.ITEM.ATMD.DIV.EXCT' />"; //표준항목 자동분할을 실행하시겠습니까?
    		showMsgBox("CNF", message, 'AutoDivision');	
	}).show().find(".ui-button-text").text("<s:message code='ATMD.DIV' />"); //자동분할
    ...
    $("#btn_rqst").click(function(){
    	...
    	//저장할래요? 확인창...
		var message = "<s:message code='MSG.STRD.ITEM.LORQ.FLIN.DEMD.POSB.DATA.DEMD.STRD.ITEM.LORQ.FLIN' />";
		//표준항목 요청서를 작성 시 요청가능한 데이터만 요청됩니다.<br>표준항목 요청서를 작성하시겠습니까?
...
function initGrid()
{
    ...
    with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.APPSTNDITEMDIV.LST1'/>";
		/* No.|상태|선택|요청구분|등록유형 */
		headtext += "<s:message code='META.HEADER.APPSTNDITEMDIV.LST2'/>";
		/* |DV_RQST_NO|분할결과|표준항목논리명|표준항목분할논리명|표준항목분할물리명|논리명기준구분|물리명기준구분|도메인논리명|도메인물리명|도메인그룹|인포타입논리명|데이터타입|길이|소수점|암호화여부|설명 */
		headtext += "<s:message code='META.HEADER.APPSTNDITEMDIV.LST3'/>";
		/* |요청가능여부 */
	var headers = [
	...
	var cols = [						
				...
				{Type:"Text",   Width:100,  SaveName:"dicAsmLnm",  Align:"Left", Edit:0, KeyField:0},
				{Type:"Text",   Width:100,  SaveName:"dicAsmPnm",  Align:"Left", Edit:0, KeyField:0}, 
				{Type:"Text",   Width:100,  SaveName:"dicAsmDsLnm",  Align:"Left", Edit:0, KeyField:0}, 
				{Type:"Text",   Width:100,  SaveName:"dicAsmDsPnm",  Align:"Left", Edit:0, KeyField:0}, 
...
// 				{Type:"Combo",  Width:60,   SaveName:"infotpId",  Align:"Left", Edit:0, Hidden:0},
				...
				{Type:"Int",    Width:60,   SaveName:"dataLen",	  Align:"Rigth", Edit:0, Hidden:0},
				{Type:"Int",    Width:60,   SaveName:"dataScal",  Align:"Rigth", Edit:0, Hidden:0},
				{Type:"Combo",  Width:60,  SaveName:"dvEncYn",	  Align:"Center", Edit:1},		
	...
	SetColProperty("dvEncYn", 	{ComboCode:"N|Y", ComboText:"N|Y"});
...
//         	var schParam = $("#frmSearch").serialize();
			...
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getItemDvRqstList.do" />", tmpstr);
        	break;
...
// 			if(DupRow != ""){
// 				var message = "<s:message code="ERR.EMPTY"  arguments="동일한 표준항목논리명이 존재 합니다." />";
//     			showMsgBox("INF", message); 
// 				return;
// 			}
 	 	    var url = "<c:url value="/meta/stnd/regItemAutoDiv.do"/>";
        	var param = "";
 	 	...
 	 	case "Delete" : 
        	...
        	var url = '<c:url value="/meta/stnd/delItemAutoDiv.do"/>';
        	var param = "";
        ...
        case "RqstWrite":
        	...
        	var url = "<c:url value="/meta/stnd/regStndItemByDiv.do"/>";
        	var param = "";
            IBSpostJson2(url, SaveJson, param, ibscallback);
        	break;
