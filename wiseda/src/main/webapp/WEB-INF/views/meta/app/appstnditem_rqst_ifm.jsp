...
<html>
<head>
<title><s:message code="APP.ITEM.REG.DEMD" /></title> <!-- APP항목등록요청 -->
...
// 	$("#btnDmnSch").click(function(){ setDmn($("#dmnLnm").val());  });
	...
	$("#btnChangAdd").click(function(){
		doAction("AddWam");
// 		var popup = OpenWindow("<c:url value="/meta/app/sditmlst_pop.do"/>","sditmSch","800","600","yes");
// 		popup.focus();
	}).show();
...
//         	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
    ...
    $("#btnBlank").click( function(){
		parent.location.href = '<c:url value="/meta/app/stndtot_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.APPSTNDITEM.RQST.IFM1'/>";
		/* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과 */
		headtext += "<s:message code='META.HEADER.APPSTNDITEM.RQST.IFM2'/>";
		/* |APP항목논리명|APP항목물리명|논리명기준구분|물리명기준구분| */
		headtext += "<s:message code='META.HEADER.APPSTNDITEM.RQST.IFM3'/>";
		...
		var cols = [						
					...
					{Type:"Text",   Width:150,  SaveName:"appSditmLnm",  Align:"Left", Edit:0, KeyField:1},
					{Type:"Text",   Width:150,  SaveName:"appSditmPnm",  Align:"Left", Edit:0, KeyField:1}, 
...
// 			loadDetail();
		...
		case "AddWam": //기존 표준항목 추가
			var url   = "<c:url value="/meta/app/popup/stnditem_pop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/app/delSditmrqstlist.do"/>";
...
// 			if(ibsSaveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/app/regitemrqstlist.do"/>";
...
// 			var url = "<c:url value="/meta/app/regsditmrqstlist.do"/>";
...
// 			parent.$("#clickBtnNm").val("");
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/app/getitemrqstlist.do" />", param);
...
// 			var url = "<c:url value="/meta/app/approveStndItem.do"/>";
			var url = "<c:url value="/meta/app/regapproveStndItem.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 			grid_sheet.LoadExcel({Mode:'HeaderMatch'});
			var url = "<c:url value="/meta/app/popup/stnditem_xls.do" />";
...
// 	$('div#detailInfo').load('<c:url value="/meta/app/ajaxgrid/stnditem_rqst_dtl.do"/>', param, function( response, status, xhr ) {
// 		  if ( status == "error" ) {
...
// 			var url = '<c:url value="/meta/app/stnditem_rqst.do" />';
// 			var url = '<c:url value="/meta/app/stndtot_rqst.do" />';
// 			var param = $('form[name=mstFrm]').serialize();
...
function grid_sheet_OnClick(row, col, value, cellx, celly) {
	...
	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '<s:message code="STRD.ITEM" /> : ' + param.sditmLnm + ' [' + param.sditmPnm +']'; //표준항목
	$('#stnditem_sel_title').html(tmphtml);
...
// 	$("#divTitle").text("표준항목 : " + grid_sheet.GetCellValue(row,"sditmLnm"));
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title"><s:message code="APP.ITEM.REG" /></div> <!-- APP항목등록 -->
...
<div id="tabs">
	<ul>
		<li><a href="#tabs-1"><s:message code="STRD.ITEM.DTL.INFO" /></a></li> <!-- 표준항목 상세정보 -->
...
<!-- 	<div id="detailInfo"></div> -->
		<%@include file="appstnditem_rqst_dtl.jsp" %>
	</div>
	<c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S'}">
	<div id="tabs-2">
...
<form name="mstFrm" id="mstFrm" style="display:none;">
	...
	<input type="text" name="bizDtlCd" id="bizDtlCd" value="API" />
