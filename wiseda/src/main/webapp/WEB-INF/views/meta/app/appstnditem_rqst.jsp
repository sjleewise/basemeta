...
//         	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
    ...
    $("#btnBlank").click( function(){
		parent.location.href = '<c:url value="/meta/app/appstnditem_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.APPSTNDITEM.RQST'/>";
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|전문용어논리명|전문용어물리명|
		//논리명기준구분|물리명기준구분|데이터타입|길이|소수점|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
...
// 			loadDetail();
		...
		case "AddWam": //기존 전문용어 추가
			var url   = "<c:url value="/meta/app/popup/appstnditem_pop.do"/>";
// 			var param = "?popRqst=Y";
...
// 			var url = "<c:url value="/meta/app/approveStndItem.do"/>";
		...
		case "Down2Excel":  //엑셀내려받기
			...
			grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'전문용어등록요청'});
			
			break;
		case "LoadExcel":  //엑셀업로
		
// 			grid_sheet.LoadExcel({Mode:'HeaderMatch'});
			var url = "<c:url value="/meta/app/popup/appstnditem_xls.do" />";
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/app/appstnditem_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
function grid_sheet_OnClick(row, col, value, cellx, celly) {
	...
	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '전문용어 : ' + param.sditmLnm + ' [' + param.sditmPnm +']';
	$('#stnditem_sel_title').html(tmphtml);
...
// 	$("#divTitle").text("전문용어 : " + grid_sheet.GetCellValue(row,"sditmLnm"));
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title">전문용어등록</div>
...
<div id="tabs">
	<ul>
		<li><a href="#tabs-1">전문용어 상세정보</a></li>
