...
//         	if(Sort == 'govStnd'){
//         		grid_sheet.DoSearch("<c:url value="/dq/gap/getMtNonStndGovColGapList.do" />", param);
//         	} else {
//         		grid_sheet.DoSearch("<c:url value="/dq/gap/getMtNonStndPdmColGapList.do" />", param);
//         	}

console.log(param);

			grid_sheet.DoSearch("<c:url value="/meta/gap/getMtNonStndPdmColGapList.do" />", param);
...
//         	console.log("dbmsId : " + dbmsId + " stndSort : " +stndSort + " schId : " + schId + " asrt : " + asrt);
        	
        	//hidden input을 가지고있는 poiForm
        	var myForm = document.poiForm;
        	var url ='<c:url value="/meta/result/gapPoiDown.do" />';
...
<%-- 							<th scope="row"><label for="dbConnTrgId"><s:message code="DB.MS.SCHEMA.NM" /></label></th> <!-- DBMS/스키마명 --> --%>
                       ...
                       <tr>
<%--                            	<th scope="row"><label for="SortGap"><s:message code="STND.DIAG.SORT" /></label></th> --%>
<!-- 							<td > -->
<%-- 								<select id="stndType" name="stndType"> --%>
<!-- <!-- 						             <option value="0">표준항목/DBC</option> --> 
<!-- 									 <option value="ORG">기관표준</option> -->
<!-- 									 <option value="GOV">범정부표준</option> -->
<!-- 						             <option value="FID">개별 DB</option> -->
<%-- 					            </select> --%>
<!-- 					        </td>  -->
	                        <th scope="row" class="th_require"><label for="stndAsrt">진단대상표준사전</label></th> <!-- 표준분류  -->
						    <td colspan="3">
							    <select  id="stndAsrt" name="stndAsrt">
							   		<option value=""><s:message code="CHC"/></option>
									<c:forEach var="code" items="${codeMap.stndAsrt}" varStatus="status">
										<option value="${code.codeCd}">${code.codeLnm}</option>
									</c:forEach>
									<option value="govStnd">개별DB</option>
							    </select>
						    </td>
                       </tr>
                   </tbody>
