...
//var connTrgSchJson = ${codeMap.connTrgSchIdByOwner} ;  
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
		...
		case "Search":
			...
			if(SortGap == 1){
				grid_sheet.DoSearch("<c:url value="/dq/gap/getMdlDbTblGapList.do" />", param); 
			} else {
				grid_sheet.DoSearch("<c:url value="/dq/gap/getGovDbTblGapList.do" />", param); 
			}
...
//         	console.log(schId);
        	...
        	var url ='<c:url value="/dq/result/gapPoiDown2.do" />';
...
// 	alert(param1);
	...
	if(SortGap == 1){
		col_sheet.DoSearch("<c:url value="/dq/gap/getMdlDbColGapList.do" />", param1); 
	} else {
		col_sheet.DoSearch("<c:url value="/dq/gap/getGovDbColGapList.do" />", param1); 
	} 	
}
...
function col_sheet_OnSearchEnd(code, message) {
	...
	if(code == 0) {
		...
		for(var i = col_sheet.HeaderRows(); i <= col_sheet.RowCount() + col_sheet.HeaderRows() - 1; i++) {
			...
			//DBMS유형이 PosgteSQL인 경우 소문자기준이기 때문에 upper처리하여 비교
			var pdmdatatype = col_sheet.GetCellValue(i, "pdmDataType").toUpperCase();
			var dbcdatatype = col_sheet.GetCellValue(i, "dbcDataType").toUpperCase();
			
			if(pdmdatatype != dbcdatatype) { 			
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title">구조품질 GAP분석</div>
	</div>
</div>
	     ...
	     <div class="tb_basic2">
	         <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="구조품질 GAP분석">
	            ...
	            <tbody>                            
					...
					<tr>
					    <th scope="row"><label for="SortGap">모델</label></th>
						<td>
							<select id="SortGap" name="SortGap">
								<option value="1">물리DB모델-DBC</option>
								<option value="2">개별DB모델-DBC</option>
					        </select>
					    </td> 			     						
	...
	<div id="tabs" style="display: none;">
	  ...
	  <div id="tabs-1">
		  ...
		  <div id="grid_02" class="grid_02">
			    <script type="text/javascript">createIBSheet("col_sheet", "100%", "100%");</script>            
		  </div>
		  <!-- 그리드 입력 입력 -->
	  </div>	  
	</div>
