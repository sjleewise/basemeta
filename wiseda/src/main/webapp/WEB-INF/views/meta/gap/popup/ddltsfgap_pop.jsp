...
//     	var cfg = {SearchMode:2,Page:100};
        SetConfig(cfg);
        SetMergeSheet(5);
        var header1 = "<s:message code='META.HEADER.DDLTSFGAP.POP1'/>";
        /* No.|상태|테이블명|테이블한글명|컬럼명|컬럼한글명|DDL|DDL|DDL|DDL|DDL|DDL|DDL|DDL|DDL|DDL|DDL(운영)|DDL(운영)|DDL(운영)|DDL(운영)|DDL(운영)|DDL(운영)|DDL(운영)|DDL(운영)|DDL(운영) */
        var header2 = "<s:message code='META.HEADER.DDLTSFGAP.POP2'/>";
        /* No.|상태|테이블명|테이블한글명|컬럼명|컬럼한글명|DDL컬럼ID|DDL테이블ID|순서|데이터타입|길이|소수점|PK여부|PK순서|NOTNULL|DEFAULT|운영컬럼명|순서|데이터타입|길이|소수점|PK여부|PK순서|NOTNULL|DEFAULT */
        var headers = [
                       {Text:header1},
        ...
        var cols = [                        
						...
						{Type:"Text",   Width:100,   SaveName:"ddlColPnm", 		Align:"Left", Edit:0, Hidden:0},
						...
						{Type:"Text",   Width:30,   SaveName:"ddlTsfColPnm",	 	Align:"Center", Edit:0, Hidden:1},
...
function doAction(sAction)
{
    ...
    {		    
        case "Search":
        	var param = "ddlTblId=${search.ddlTblId}&dbConnTrgId=${search.dbConnTrgId}&dbSchId=${search.dbSchId}";
        	    param += "&ddlTsfTblId=${search.ddlTsfTblId}&dbTsfConnTrgId=${search.dbTsfConnTrgId}&dbTsfSchId=${search.dbTsfSchId}";
            //if($("#dbSchId option:selected").val()==''){
            //	showMsgBox("INF", "<s:message code="REQ.NO.TGTSCH" />");	
            //	return false;
            //}
        	gapcollist_grid.DoSearch('<c:url value="/meta/gap/getDdlTsfColGap.do" />', param);
        	//alert(param);
        	break;
        case "Down2Excel":  //엑셀내려받기
        	...
        	if(gapcollist_grid.SearchRows()>0){
        		gapcollist_grid.Down2Excel({FileName:'ddltsfgapcol_list',SheetName:'<s:message code="DDL.DEV.VS.DDL.OPR.CLMN.LIST"/>', HiddenColumn:1, Merge:1}); //DDL(개발) VS DDL(운영)컬럼목록
        	}
