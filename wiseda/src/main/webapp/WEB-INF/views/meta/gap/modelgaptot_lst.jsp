<!DOCTYPE html PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN" "http://www.w3.org/TR/html4/loose.dtd">
<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="c"   uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<%@ taglib prefix="fn"  uri="http://java.sun.com/jsp/jstl/functions"%>
<%@ taglib uri="http://www.springframework.org/tags" prefix="s" %>
<%@ taglib uri="http://www.springframework.org/tags/form" prefix="form" %>
<%@ taglib uri="http://tiles.apache.org/tags-tiles" prefix="tiles"%>
<%@page import="kr.wise.commons.WiseMetaConfig"%>

<!-- <html> -->
<head>
<title>모델GAP분석 조회</title>

<script type="text/javascript">

var interval = "";
EnterkeyProcess("Search");
var connTrgSchJson = ${codeMap.connTrgSch} ;
var initGridYn2 = false;

$(document).ready(function() {
	
		//alert(sysareaJson[0].codeCd + ":" + sysareaJson[0].codeLnm);
		
		//마우스 오버 이미지 초기화
		//imgConvert($('div.tab_navi a img'));
		
        $("#grid_02").hide();                     
		//달력팝업 추가...
	 	$( "#searchBgnDe" ).datepicker();
		$( "#searchEndDe" ).datepicker();
		
        //그리드 초기화 
//         initGrid();
        // 조회 Event Bind
        $("#btnSearch").click(function(){ 
        	doAction("Search");  
        	
        });
        
        $("#btnDelete").hide();
        
        $("#btnTreeNew").change(function(){}).hide();
                      
        // 엑셀내리기 Event Bind
        $("#btnExcelDown").click( function(){ doAction("Down2Excel"); } );

        
    	//주제영역 검색 팝업 호출
    	$("#subjSearchPop").click(function(){
    		var param = ""; //$("form#frmInput").serialize();
    		openLayerPop ("<c:url value='/meta/subjarea/popup/subjSearchPop.do' />", 800, 600, param);
//     		OpenWindow("<c:url value='/meta/subjarea/popup/subjSearchPop.do' />", "dkafda",  800, 600, "yes");
//     		openSearchPop("<c:url value='/meta/model/pop/subjSearchPop.do' />", param);
        });
    	
    	$("#subjSearchDel").click(function(){
    		$("#frmSearch #subjId").val("");
    		$("#frmSearch #fullPath").val("");
        }).show();
        
    	//======================================================
	   	 // 셀렉트 박스 초기화
	   	//======================================================
	   	double_select(connTrgSchJson, $("#frmSearch #dbConnTrgId"));
	   	$('select', $("#frmSearch #dbConnTrgId").parent()).change(function(){
	   		double_select(connTrgSchJson, $(this));
	   	});
    	
	   	//$( "#tabs" ).tabs();
	   	
        
      //파라미터 : (자동완성 대상 오브젝트, 검색할 단어종류, 최대표시 갯수(default-10개))
    	setautoComplete($("#frmSearch #subjLnm"), "SUBJ");
	   	$("#gapDcd").change(function(){
	   		var gapDcd = $("#gapDcd").val();
	   		if(gapDcd == "MP"){
// 	   			$("#grid_01").show();
// 	   			$("#grid_02").hide();
// 	   			grid_sheet.FitColWidth();
	   			grid_sheet.RemoveAll();
	   			grid_sheet.SetColHidden("dbcTblNm",1);
 	   			grid_sheet.SetColHidden("dbcColCnt",1);
 	   		    grid_sheet.SetColHidden("subjLnm",       0);      
 	   		    grid_sheet.SetColHidden("pdmTblPnm",     0);      
 	   		    grid_sheet.SetColHidden("pdmTblLnm",     1);      
 	   		    grid_sheet.SetColHidden("pdmColCnt",     0);
 	   	        grid_sheet.SetColHidden("ddlTblPnm",     1);
 	   		    grid_sheet.SetColHidden("ddlTblLnm",     1);
	   		    grid_sheet.SetColHidden("dbConnTrgLnm", 1);
	   	        grid_sheet.SetColHidden("dbSchLnm", 1);
	            grid_sheet.SetColHidden("ddlColCnt", 1);
   		        grid_sheet.SetColHidden("martTblPnm", 0);
 	   	        grid_sheet.SetColHidden("martTblLnm", 0);
 	            grid_sheet.SetColHidden("martColCnt", 0);
 	            grid_sheet.SetColHidden("dbTsfConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbTsfSchLnm", 1);
	            grid_sheet.SetColHidden("ddlTsfTblPnm", 1);
	            grid_sheet.SetColHidden("ddlTsfTblLnm", 1);
	            grid_sheet.SetColHidden("ddlTsfColCnt", 1);
	            grid_sheet.SetColHidden("dbRealSchId", 1);
	            grid_sheet.SetColHidden("dbcRealTblNm", 1);
	            grid_sheet.SetColHidden("dbcRealColCnt", 1);
	            grid_sheet.SetColHidden("dbcRealDbConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbcRealDbSchLnm", 1);
	            grid_sheet.SetColHidden("dbcDbConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbcDbSchLnm", 1);
	            grid_sheet.SetColHidden("dbcTblLnm", 1);
 	            grid_sheet.FitColWidth();
	   		} else if(gapDcd == "PD"){
	   			grid_sheet.RemoveAll();
 	   			grid_sheet.SetColHidden("dbcTblNm",1);
 	   			grid_sheet.SetColHidden("dbcColCnt",1);
 	   		    grid_sheet.SetColHidden("subjLnm",       0);      
 	   		    grid_sheet.SetColHidden("pdmTblPnm",     0);      
 	   		    grid_sheet.SetColHidden("pdmTblLnm",     0);      
 	   		    grid_sheet.SetColHidden("pdmColCnt",     0);     
 	   	        grid_sheet.SetColHidden("ddlTblPnm",     0);
 	   	        grid_sheet.SetColHidden("ddlTblLnm",     1);
	   		    //grid_sheet.SetColHidden("ddlTblErrDescn",     1);      
 	   		   // grid_sheet.SetColHidden("ddlColErrDescn", 1);
 	   		    grid_sheet.SetColHidden("dbConnTrgLnm", 0);
   	            grid_sheet.SetColHidden("dbSchLnm", 0);
                grid_sheet.SetColHidden("ddlColCnt", 0);
 	   		    grid_sheet.SetColHidden("martTblPnm", 1);
 	   	        grid_sheet.SetColHidden("martTblLnm", 1);
 	            grid_sheet.SetColHidden("martColCnt", 1);
 	            grid_sheet.SetColHidden("dbTsfConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbTsfSchLnm", 1);
	            grid_sheet.SetColHidden("ddlTsfTblPnm", 1);
	            grid_sheet.SetColHidden("ddlTsfTblLnm", 1);
	            grid_sheet.SetColHidden("ddlTsfColCnt", 1);
	            grid_sheet.SetColHidden("dbRealSchId", 1);
	            grid_sheet.SetColHidden("dbcRealTblNm", 1);
	            grid_sheet.SetColHidden("dbcRealColCnt", 1);
	            grid_sheet.SetColHidden("dbcRealDbConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbcRealDbSchLnm", 1);
	            grid_sheet.SetColHidden("dbcDbConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbcDbSchLnm", 1);
	            grid_sheet.SetColHidden("dbcTblLnm", 1);      
	   		   grid_sheet.FitColWidth();
	   		}else if(gapDcd == "DC" || gapDcd == "TC" || gapDcd == "RC"){
	   			
	   			grid_sheet.RemoveAll();
 	   			grid_sheet.SetColHidden("dbcTblNm",0);
 	   			grid_sheet.SetColHidden("dbcColCnt",0);
 	   		    grid_sheet.SetColHidden("subjLnm",       1);      
 	   		    grid_sheet.SetColHidden("pdmTblPnm",     1);      
 	   		    grid_sheet.SetColHidden("pdmTblLnm",     1);      
 	   		    grid_sheet.SetColHidden("pdmColCnt",     1); 
 	   	        grid_sheet.SetColHidden("ddlTblPnm",     0);
 	   		    grid_sheet.SetColHidden("ddlTblLnm",     0);
	   		    grid_sheet.SetColHidden("dbConnTrgLnm", 0);
	   	        grid_sheet.SetColHidden("dbSchLnm", 0);
	            grid_sheet.SetColHidden("ddlColCnt", 0);
   		        grid_sheet.SetColHidden("martTblPnm", 1);
 	   	        grid_sheet.SetColHidden("martTblLnm", 1);
 	            grid_sheet.SetColHidden("martColCnt", 1);
 	            grid_sheet.SetColHidden("dbTsfConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbTsfSchLnm", 1);
	            grid_sheet.SetColHidden("ddlTsfTblPnm", 1);
	            grid_sheet.SetColHidden("ddlTsfTblLnm", 1);
	            grid_sheet.SetColHidden("ddlTsfColCnt", 1);
	            grid_sheet.SetColHidden("dbRealSchId", 1);
	            grid_sheet.SetColHidden("dbcRealTblNm", 1);
	            grid_sheet.SetColHidden("dbcRealColCnt", 1);
	            grid_sheet.SetColHidden("dbcRealDbConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbcRealDbSchLnm", 1);
	            grid_sheet.SetColHidden("dbcDbConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbcDbSchLnm", 1);
	            grid_sheet.SetColHidden("dbcTblLnm", 1);
	   		    grid_sheet.FitColWidth();
	   		}else if(gapDcd == "DT" || gapDcd == "DR" || gapDcd == "TR"){
	   			grid_sheet.RemoveAll();
 	   			grid_sheet.SetColHidden("dbcTblNm",1);
 	   			grid_sheet.SetColHidden("dbcColCnt",1);
 	   		    grid_sheet.SetColHidden("subjLnm",       1);      
 	   		    grid_sheet.SetColHidden("pdmTblPnm",     1);      
 	   		    grid_sheet.SetColHidden("pdmTblLnm",     1);      
 	   		    grid_sheet.SetColHidden("pdmColCnt",     1); 
 	   	        grid_sheet.SetColHidden("ddlTblPnm",     0);
 	   		    grid_sheet.SetColHidden("ddlTblLnm",     0);
	   		    grid_sheet.SetColHidden("dbConnTrgLnm", 0);
	   	        grid_sheet.SetColHidden("dbSchLnm", 0);
	            grid_sheet.SetColHidden("ddlColCnt", 0);
   		        grid_sheet.SetColHidden("martTblPnm", 1);
 	   	        grid_sheet.SetColHidden("martTblLnm", 1);
 	            grid_sheet.SetColHidden("martColCnt", 1);
 	            grid_sheet.SetColHidden("dbTsfConnTrgLnm", 0);
 	            grid_sheet.SetColHidden("dbTsfSchLnm", 0);
 	            grid_sheet.SetColHidden("ddlTsfTblPnm", 0);
 	            grid_sheet.SetColHidden("ddlTsfTblLnm", 1);
 	            grid_sheet.SetColHidden("ddlTsfColCnt", 0);
	            grid_sheet.SetColHidden("dbRealSchId", 1);
	            grid_sheet.SetColHidden("dbcRealTblNm", 1);
	            grid_sheet.SetColHidden("dbcRealColCnt", 1);
	            grid_sheet.SetColHidden("dbcRealDbConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbcRealDbSchLnm", 1);
	            grid_sheet.SetColHidden("dbcDbConnTrgLnm", 1);
	            grid_sheet.SetColHidden("dbcDbSchLnm", 1);
	            grid_sheet.SetColHidden("dbcTblLnm", 1);
 	            grid_sheet.FitColWidth();
	   		} else if(gapDcd == "CC"){
	   			grid_sheet.RemoveAll();
 	   			grid_sheet.SetColHidden("dbcTblNm",0);
 	   			grid_sheet.SetColHidden("dbcColCnt",0);
 	   		    grid_sheet.SetColHidden("subjLnm",       1);      
 	   		    grid_sheet.SetColHidden("pdmTblPnm",     1);      
 	   		    grid_sheet.SetColHidden("pdmTblLnm",     1);      
 	   		    grid_sheet.SetColHidden("pdmColCnt",     1); 
 	   	        grid_sheet.SetColHidden("ddlTblPnm",     1);
 	   		    grid_sheet.SetColHidden("ddlTblLnm",     1);
	   		    grid_sheet.SetColHidden("dbConnTrgLnm", 1);
	   	        grid_sheet.SetColHidden("dbSchLnm", 1);
	            grid_sheet.SetColHidden("ddlColCnt", 1);
   		        grid_sheet.SetColHidden("martTblPnm", 1);
 	   	        grid_sheet.SetColHidden("martTblLnm", 1);
 	            grid_sheet.SetColHidden("martColCnt", 1);
 	            grid_sheet.SetColHidden("dbTsfConnTrgLnm", 1);
 	            grid_sheet.SetColHidden("dbTsfSchLnm", 1);
 	            grid_sheet.SetColHidden("ddlTsfTblPnm", 1);
 	            grid_sheet.SetColHidden("ddlTsfTblLnm", 1);
 	            grid_sheet.SetColHidden("ddlTsfColCnt", 1);
	            grid_sheet.SetColHidden("dbcRealTblNm", 0);
	            grid_sheet.SetColHidden("dbcRealColCnt", 0);
	            grid_sheet.SetColHidden("dbcRealDbConnTrgLnm", 0);
	            grid_sheet.SetColHidden("dbcRealDbSchLnm", 0);
	            grid_sheet.SetColHidden("dbcDbConnTrgLnm", 0);
	            grid_sheet.SetColHidden("dbcDbSchLnm", 0);
	            grid_sheet.SetColHidden("dbcTblLnm", 0);
	            
 	            grid_sheet.FitColWidth();
	   		}              
	       
	   		
	   	});
      
});

$(window).on('load',function() {
	//그리드 초기화 
	initGrid();
	
	//$( "#layer_div" ).show();
});

//주제영역 팝업 리턴값 처리
function returnSubjPop (ret) {
// 	alert(ret);
	
	var retjson = jQuery.parseJSON(ret);
	
	$("#frmSearch #subjId").val(retjson.subjId);
	$("#frmSearch #fullPath").val(retjson.fullPath);
	
}


$(window).resize(
    
    function(){
                
    	// grid_sheet.SetExtendLastCol(1);    
    }
);


function initGrid()
{
    
    with(grid_sheet){
    	
    	var cfg = {SearchMode:2,Page:100};
        SetConfig(cfg);
        
        SetMergeSheet(5);
        var headtext1 = "No.|상태|GAP내용|주제영역명|모델마트|모델마트|모델마트|물리모델(PDM)|물리모델(PDM)|물리모델(PDM)|물리모델(PDM)|물리모델(PDM)|물리모델(PDM)"		//13
                        +"|DDL(Source)|DDL(Source)|DDL(Source)|DDL(Source)|DDL(Source)|DDL(Source)|DDL(Source)|DDL(Source)|DDL(Source)"		//9
                        +"|DBC(Source)|DBC(Source)|DBC(Source)|DBC(Source)|DBC(Source)|DBC(Source)|DBC(Source)"								//7
                        +"|DDL(Target)|DDL(Target)|DDL(Target)|DDL(Target)|DDL(Target)|DDL(Target)|DDL(Target)|DDL(Target)|DDL(Target)"		//9
                        +"|DBC(Target)|DBC(Target)|DBC(Target)|DBC(Target)|DBC(Target)|DBC(Target)";
        var headtext2 = "No.|상태|GAP내용|주제영역명|테이블명|테이블한글명|컬럼수|테이블ID|테이블명|테이블한글명|컬럼수|물리테이블오류내용|물리컬럼오류내용"
                        +"|DB명|Owner명|DDL테이블ID|테이블명|테이블한글명|컬럼수|PDM기준 GAP컬럼수|DDL테이블오류내용|DDL컬럼오류내용"
                        +"|스키마ID|DB명|스키마명|테이블명|테이블한글명|컬럼수|PDM기준 GAP컬럼수"
                        +"|DDL기준 GAP컬럼수|이관DBID|이관스키마ID|DB명|Owner명|DDL이관테이블ID|테이블명|테이블한글명|컬럼수"
                        +"|운영스키마ID|DB명|스키마명|테이블명|테이블한글명|컬럼수";
                        ;        
        var headers = [
                    {Text: headtext1},
                    	{Text:headtext2, Align:"Center"}
                    
                ];
        
        var headerInfo = {Sort:1, ColMove:1, ColResize:1, HeaderCheck:1};
        
        InitHeaders(headers, headerInfo); 

        var cols = [                        
                    {Type:"Seq",    Width:40,   SaveName:"ibsSeq",      Align:"Center", Edit:0},
                    {Type:"Combo",   Width:40,   SaveName:"gapStatus",   Align:"Center", Edit:0},
                    {Type:"Text",   Width:80,   SaveName:"gapConts",   Align:"Left", Edit:0},
                    {Type:"Text",   Width:50,   SaveName:"subjLnm",   Align:"Left", Edit:0},
                    {Type:"Text",   Width:100,   SaveName:"martTblPnm",   Align:"Left", Edit:0, Hidden:0},
                    {Type:"Text",   Width:100,   SaveName:"martTblLnm",   Align:"Left", Edit:0, Hidden:0},
                    {Type:"Int",   Width:50,   SaveName:"martColCnt",   Align:"Center", Edit:0, Hidden:0},
                    {Type:"Text",   Width:10,   SaveName:"pdmTblId",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"pdmTblPnm",   Align:"Left", Edit:0},
                    {Type:"Text",   Width:100,   SaveName:"pdmTblLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Int",   Width:50,   SaveName:"pdmColCnt",   Align:"Center", Edit:0},
                    {Type:"Text",   Width:100,   SaveName:"pdmTblErrDescn",   Align:"Left", Edit:0, Hidden :1},
                    {Type:"Text",   Width:100,   SaveName:"pdmColErrDescn",   Align:"Left", Edit:0, Hidden :1},
                    {Type:"Text",   Width:80,   SaveName:"dbConnTrgLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:80,   SaveName:"dbSchLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:10,   SaveName:"ddlTblId",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"ddlTblPnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"ddlTblLnm",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Int",   Width:50,   SaveName:"ddlColCnt",   Align:"Center", Edit:0,Hidden:1},
                    {Type:"Int",   Width:10,   SaveName:"pdmGapColCntDdl",   Align:"Right", Edit:0, Hidden:1},
                    {Type:"Text",   Width:200,   SaveName:"ddlTblErrDescn",   Align:"Left", Edit:0, Hidden :1},
                    {Type:"Text",   Width:200,   SaveName:"ddlColErrDescn",   Align:"Left", Edit:0, Hidden :1},
                    {Type:"Text",   Width:10,   SaveName:"dbSchId",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"dbcDbConnTrgLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"dbcDbSchLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:150,   SaveName:"dbcTblNm",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Text",   Width:150,   SaveName:"dbcTblLnm",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Int",   Width:100,   SaveName:"dbcColCnt",   Align:"Center", Edit:0, Hidden:1},
                    {Type:"Int",   Width:10,   SaveName:"pdmGapColCntDbc",   Align:"Right", Edit:0, Hidden:1},
                    {Type:"Int",   Width:10,   SaveName:"ddlGapColCntDbc",   Align:"Right", Edit:0, Hidden:1},
                    {Type:"Text",   Width:80,   SaveName:"dbTsfConnTrgId",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:80,   SaveName:"dbTsfSchId",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:80,   SaveName:"dbTsfConnTrgLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:80,   SaveName:"dbTsfSchLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:10,   SaveName:"ddlTsfTblId",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"ddlTsfTblPnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"ddlTsfTblLnm",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Int",   Width:50,   SaveName:"ddlTsfColCnt",   Align:"Center", Edit:0,Hidden:1},
                    {Type:"Text",   Width:10,   SaveName:"dbRealSchId",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"dbcRealDbConnTrgLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:100,   SaveName:"dbcRealDbSchLnm",   Align:"Left", Edit:0,Hidden:1},
                    {Type:"Text",   Width:150,   SaveName:"dbcRealTblNm",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Text",   Width:150,   SaveName:"dbcRealTblLnm",   Align:"Left", Edit:0, Hidden:1},
                    {Type:"Int",   Width:100,   SaveName:"dbcRealColCnt",   Align:"Center", Edit:0, Hidden:1},
                ]; 

        InitColumns(cols);

        SetColProperty("gapStatus", 	{ComboCode:"NML|GAP", 	ComboText:"정상|GAP"});
        
	     //콤보 목록 설정...
        InitComboNoMatchText(1, "");
        
        SetSheetHeight(700);
      
        FitColWidth();
        
        //SetExtendLastCol(1);    
    }
    
    //==시트설정 후 아래에 와야함=== 
    init_sheet(grid_sheet);    
    //===========================
   
}

function doAction(sAction)
{
        
    switch(sAction)
    {
        
                    
        case "Search":
        	var param = $('#frmSearch').serialize();
        	var gapDcd = $("#gapDcd").val();
        	if(gapDcd == "MP"){
      		     grid_sheet.DoSearch("<c:url value="/meta/gap/getMartPdmGapList.do" />", param);
      	    } else if(gapDcd == "PD"){
        		 grid_sheet.DoSearch("<c:url value="/meta/gap/getPdmDdlGapList.do" />", param);
        	} else if(gapDcd == "DT" || gapDcd == "DR" || gapDcd == "TR"){
         		 grid_sheet.DoSearch("<c:url value="/meta/gap/getDdlTsfGapList.do" />", param);
	    	} else if(gapDcd == "DC" || gapDcd == "TC" || gapDcd == "RC"){
        		 grid_sheet.DoSearch("<c:url value="/meta/gap/getDdlDbcGapList.do" />", param);
        	} else if(gapDcd == "CC"){
     		     grid_sheet.DoSearch("<c:url value="/meta/gap/getDbcGapList.do" />", param);
     	    }
        	break;
       
        case "Down2Excel":  //엑셀내려받기
            grid_sheet.Down2ExcelBuffer(true);  
        
        	if(grid_sheet.SearchRows()>0){
        		grid_sheet.Down2Excel({FileName:'pdmddlgap_list',SheetName:'물리모델VSDDLGAP분석', HiddenColumn:1, Merge:1}); 
        	}
//         	if(col_sheet.SearchRows()>0){
//         		col_sheet.Down2Excel({FileName:'gap_list',SheetName:'물리모델컬럼', HiddenColumn:1, Merge:1}); 
//         	}
//         	if(grid_sub_ddltblcollist.SearchRows()>0){
//         		grid_sub_ddltblcollist.Down2Excel({FileName:'gap_list',SheetName:'DDL컬럼', HiddenColumn:1, Merge:1}); 
//         	}
//         	if(grid_sub_dbctblcollist.SearchRows()>0){
//         		grid_sub_dbctblcollist.Down2Excel({FileName:'gap_list',SheetName:'DBC컬럼', HiddenColumn:1, Merge:1}); 
//         	}
        	grid_sheet.Down2ExcelBuffer(false);   
        	
            break;
    }       
}
 



//IBS 리스트 저장, 단건 저장, 삭제 상태에 따라 후처리 하는 함수...
function postProcessIBS(res) {
	
	//alert(res.action);
	
	switch(res.action) {
		//삭제 후처리...
		case "<%=WiseMetaConfig.IBSAction.DEL%>" :
				doAction("Search");
		
			break;
		//단건 등록 후처리...
		case "<%=WiseMetaConfig.IBSAction.REG%>" :
			

			
			break;
		//여러건 등록 후처리...
		case "<%=WiseMetaConfig.IBSAction.REG_LIST%>" : 
			  doAction("Search");
			
			
			break;
		
		default : 
			// 아무 작업도 하지 않는다...
			break;
			
	}
	
}


/*
    row : 행의 index
    col : 컬럼의 index
    value : 해당 셀의 value
    x : x좌표
    y : y좌표
*/
function grid_sheet_OnDblClick(row, col, value, cellx, celly) {
	if(row < 1) return;

	var param = grid_sheet.RowSaveStr(row);
	var url = "";
	var gapDcd = $("#gapDcd").val();
	if(gapDcd == "MP"){
		url = "<c:url value="/meta/gap/martpdmgap_pop.do"/>";
	} else if(gapDcd == "PD"){
		url = "<c:url value="/meta/gap/pdmddlgap_pop.do"/>";
	} else if(gapDcd == "DT" || gapDcd == "TR" || gapDcd == "DR"){
		url = "<c:url value="/meta/gap/ddltsfgap_pop.do"/>";
	} else if(gapDcd == "DC"){
		url = "<c:url value="/meta/gap/ddldbcgap_pop.do"/>";
	} else if(gapDcd == "TC"){
		
		
	} else if(gapDcd == "RC"){
		url = "<c:url value="/meta/gap/ddltsfdbcgap_pop.do"/>";
	} else if(gapDcd == "CC"){
		url = "<c:url value="/meta/gap/dbcgap_pop.do"/>";
	}
	openLayerPop(url, "1100",  "650", param);
}

// function ddlDbc_sheet_OnDblClick(row, col, value, cellx, celly) {
    
// 	if(row < 1) return;
	
// 	var param = ddlDbc_sheet.RowSaveStr(row);
// 	var url = "<c:url value="/meta/gap/ddldbcgap_pop.do"/>"
	
// 	openLayerPop(url, "1200",  "600", param);
		
//}

function grid_sheet_OnClick(row, col, value, cellx, celly) {
    
	if(row < 1) return;
	
	//선택한 셀의 savename이 아래와 같으면 리턴...
// 	var colsavename = grid_sheet.ColSaveName(col);
// 	if ('ibsSeq' == colsavename || 'ibsStatus' == colsavename || 'ibsCheck' == colsavename) return;
	
	//선택한 셀이 Edit 가능한 경우는 리턴...
// 	if(grid_sheet.GetColEditable(col)) return;
// 	//alert("상세정보 조회 가능"); return;

// 	//tblClick(row);
	
// 	//선택한 상세정보를 가져온다...
 	var param =  grid_sheet.GetRowJson(row);
 	var dbc = "&dbSchId="+grid_sheet.GetCellValue(row, "dbSchId")+"&dbcTblNm="+grid_sheet.GetCellValue(row, "dbcTblNm");
 	var ddl = "&ddlTblId="+grid_sheet.GetCellValue(row, "ddlTblId");
 	var pdm = "&pdmTblId="+grid_sheet.GetCellValue(row, "pdmTblId");

// 	//선택한 그리드의 row 내용을 보여준다.....
//  	var tmphtml = '테이블명 : ' + param.pdmTblPnm;
//  	$('#sel_title').html(tmphtml);
	
// 	//메뉴ID를 토대로 조회한다....
 	//col_sheet.DoSearch("<c:url value="/meta/model/getgappdmcollist.do" />", pdm);
 	//grid_sub_ddltblcollist.DoSearch('<c:url value="/meta/ddl/ajaxgrid/ddltblcollistgap_dtl.do" />', ddl);
 	//grid_sub_dbctblcollist.DoSearch('<c:url value="/meta/dbc/ajaxgrid/dbctblcollist_dtl.do" />', dbc);
    
}


function grid_sheet_OnSearchEnd(code, message) {
	//alert(grid_sheet.GetDataBackColor()+":"+ grid_sheet.GetDataAlternateBackColor());
	if(code < 0) {
		showMsgBox("ERR", "<s:message code="ERR.SEARCH" />");
		return;
	}
}


// function col_sheet_OnSearchEnd(code, message) {
// 	//alert(grid_sheet.GetDataBackColor()+":"+ grid_sheet.GetDataAlternateBackColor());
// 	if(code < 0) {
// 		showMsgBox("ERR", "<s:message code="ERR.SEARCH" />");
// 		return;
// 	}
// }

// function grid_sub_ddltblcollist_OnSearchEnd(code, message) {
// 	//alert(grid_sheet.GetDataBackColor()+":"+ grid_sheet.GetDataAlternateBackColor());
// 	if(code < 0) {
// 		showMsgBox("ERR", "<s:message code="ERR.SEARCH" />");
// 		return;
// 	}
// }

// function grid_sub_dbctblcollist_OnSearchEnd(code, message) {
// 	//alert(grid_sheet.GetDataBackColor()+":"+ grid_sheet.GetDataAlternateBackColor());
// 	if(code < 0) {
// 		showMsgBox("ERR", "<s:message code="ERR.SEARCH" />");
// 		return;
// 	}
// }




// function grid_sub_dbctblcollist_OnClick(row, col, value, cellx, celly) {
// 	if(row < 1) return;
	
// 	var bpdmRow = $("#frmSearch #bpdmRow").val();
// 	var bddlRow = $("#frmSearch #bddlRow").val();
	
// 	if(bpdmRow > 0){
// 		grid_sub_ddltblcollist.SetRowFontColor(bpdmRow, "#000000");
// 		col_sheet.SetRowFontColor(bddlRow, "#000000");
// 	}
	
// // 	ddl컬럼
// 	var ddlRow = grid_sub_ddltblcollist.FindText("ddlColPnm", grid_sub_dbctblcollist.GetCellValue(row, "dbcColNm"));
// // 	물리모델
// 	var pdmRow = col_sheet.FindText("pdmColPnm", grid_sub_dbctblcollist.GetCellValue(row, "dbcColNm"));
	
// 	if(grid_sub_dbctblcollist.GetCellValue(row, "colErrConts") != ""){
// 		grid_sub_ddltblcollist.SetRowFontColor(ddlRow, "#FF0000");
// 		col_sheet.SetRowFontColor(pdmRow, "#FF0000");
// 	}
	
// 	$("#frmSearch #bpdmRow").val(pdmRow);
// 	$("#frmSearch #bddlRow").val(ddlRow);
// }

</script>
</head>

<!-- <body> -->

<div id="layer_div" >
<!-- 메뉴 메인 제목 -->
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title">주제영역 조회</div>
	</div>
</div>
<!-- 메뉴 메인 제목 -->
<div style="clear:both; height:5px;"><span></span></div>

<!-- 검색조건 입력폼 -->
<div id="search_div">
        <div class="stit">검색조건</div>
        <div style="clear:both; height:5px;"><span></span></div>
        
        <form id="frmSearch" name="frmSearch" method="post">
            <input type="hidden" id="bpdmRow" name="bpdmRow" readonly="readonly"/>
            <input type="hidden" id="bddlRow" name="bddlRow" readonly="readonly"/>
            <fieldset>
            <legend>머리말</legend>
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="주제영역조회">
                   <caption>테이블 이름</caption>
                   <colgroup>
                   <col style="width:10%;" />
                   <col style="width:25%;" />
                   <col style="width:10%;" />
                   <col style="width:25%;" />
                   <col style="width:10%;" />
                   <col style="width:25%;" />
                   </colgroup>
                   
                   <tbody>                            
                       <tr> 
                           <th scope="row"><label for="subjId">GAP비교대상</label></th>
                             <td>
                                <select id = "gapDcd" name ="gapDcd">
                                    <option value ="MP">모델마트 VS 물리모델(메타)</option>  
                                    <option value ="PD">물리모델(메타) VS 개발DDL</option>
                                    <option value ="DT">개발DDL VS 테스트DDL</option>
                                    <option value ="TR">테스트DDL VS 운영DDL</option>
                                    <!-- <option value ="DR">개발DDL VS 운영DDL</option> --> 
<%--                                     <option value ="DC"><s:message code="DEV.DDL.VS.DEV.DBC"/></option> <!-- 개발DDL VS 개발DBC --> --%>
<%--                                     <option value ="TC"><s:message code="TEST.DDL.VS.TEST.DBC"/></option> <!-- 테스트DDL VS 테스트DDL --> --%>
<%--                                     <option value ="RC"><s:message code="OPR.DDL.VS.OPR.DBC"/></option> <!-- 운영DDL VS 운영DBC --> --%>
                                    <!-- <option value ="CC">개발DBC VS 운영DBC</option> -->
                                </select> 
                            </td> 
                           <th scope="row"><label for="subjId">주제영역명</label></th>
                            <td>
                            <input type="hidden" id="subjId" name="subjId" readonly="readonly"/>
							<input type="text" class="wd60p" id="fullPath" name="fullPath" readonly="readonly"/>
						 	<button class="btnSearchPop" id="subjSearchPop">검색</button>
					        <button class="btnDelPop" id="subjSearchDel" >삭제</button>
                            </td>
                           
                            <th scope="row" class=""><label for="dbSchId">DB/스키마정보</label></th>
	                      <td>
				            <select id="dbConnTrgId" class="" name="dbConnTrgId">
				             <option value="">----선택----</option>
				            </select>
				            <select id="dbSchId" class="" name="dbSchId">
				             <option value="">----선택----</option>
				             </select>
				           </td>
                       </tr>
                       <tr> 
<!--                            <th scope="row"><label for="searchTyp">조회조건</label></th> -->
<!--                             <td> -->
<%--                                 <select id="searchTyp" class="" name="searchTyp"> --%>
<!--                                         <option value="CU">신규/변경</option> -->
<!--                                         <option value="CUD">삭제포함</option> -->
<%--                                  </select> --%>
<!--                             </td>                                -->


                           <th scope="row"><label for="tblNm">테이블명</label></th>
                           <td><input type="text" name="tblNm" id="tblNm" class="wd60p" /></td>
<!--                       <th scope="row"><label for="colNm">컬럼명</label></th> -->
<!--                       <td><input type="text" name="colNm" id="colNm" class="wd60p" /></td> -->

                            <th scope="row"><label for="searchBgnDe">승인일자</label></th>
                            <td><input id="searchBgnDe" name="searchBgnDe" type="text" class="wd80"/>
                              - <input id="searchEndDe" name="searchEndDe" type="text" class="wd80"/></td>
                           <th scope="row"><label for="gapStatus">상태</label></th>
                            <td>
                                <select id="gapStatus" class="" name="gapStatus">
                                        <option value="">선택</option>
                                        <option value="NML">정상</option>
                                        <option value="GAP">GAP</option>
                                 </select>
                            </td>
                       </tr>
                   </tbody>
                 </table>   
            </div>
            </fieldset>
            
        <div class="tb_comment"><s:message  code='ETC.COMM2' /></div>
        </form>
		<div style="clear:both; height:10px;"><span></span></div>
        
         <!-- 조회버튼영역  -->
<%-- 		<%@ include file="" %> --%>
		<tiles:insertTemplate template="/WEB-INF/decorators/buttonTree.jsp" />
<%-- 		<tiles:insertTemplate template="/WEB-INF/decorators/buttonMain.jsp" /> --%>
</div>
<div style="clear:both; height:5px;"><span></span></div>
        
	<!-- 그리드 입력 입력 -->
	<div id="grid_01" class="grid_01">
	     <script type="text/javascript">createIBSheet("grid_sheet", "100%", "200px");</script>            
<%-- 	     <script type="text/javascript">createIBSheet2($("#grid_01"), "grid_sheet", "100%", "100%");</script>             --%>
	</div>

<div style="clear:both; height:5px;"><span></span></div>

	<!-- 그리드 하단 영역 : 레코드 선택시 내용 표시 및 수정 가능하도록 -->
	<div class="selected_title_area" style="display: none;">
		    <div class="selected_title">주제영역명 : <span></span></div>
	</div>

<div style="clear:both; height:5px;"><span></span></div>

	<!-- 그리드 하단 영역 : 레코드 선택시 내용 표시 및 수정 가능하도록 -->
<!-- 	<div class="selected_title_area"> -->
<%-- 		    <div class="selected_title" id="sel_title"> <span></span></div> --%>
<!-- 	</div> -->
	<!-- 그리드 하단 영역 : 레코드 선택시 내용 표시 및 수정 가능하도록 End-->
	<div style="clear:both; height:5px;"><span></span></div>

	<!-- 선택 레코드의 내용을 탭처리... -->
<!--  	<div id="tabs">  -->
<!--  	  <ul>  -->
<!--  	    <li><a href="#tabs-1">상세정보</a></li>  -->
    
<!--  	  </ul>  -->
<!--  	  <div id="tabs-1"> -->
<!-- <!--  				상세정보 ajax 로드시 이용  --> 
<%--  			<%@include file="modelgap_dtl.jsp" %>  --%>
		
 				
<!--  	  </div> -->
  
<!--  	 </div>  -->
</div>
<!-- 그리드 하단 영역 : 레코드 선택시 내용 표시 및 수정 가능하도록 -->

<%-- <%= application.getRealPath("/") %> --%>
<!-- </body> -->
</html>