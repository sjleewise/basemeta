...
<html>
<head>
<title><s:message code="DMN.UTLZ.ANLY" /></title> <!-- 도메인 활용도 분석 -->
...
function initGrid()
{
    ...
    with(grid_sheet){
		...
		var headers = [
						{Text:"<s:message code='META.HEADER.DMNPUSAOCD.LST1'/>"}
						/* No.|도메인|도메인ID|도메인영문명|설명|사용건수|사용건수|사용건수|사용건수|사용건수 */
					,{Text:"<s:message code='META.HEADER.DMNPUSAOCD.LST2'/>", Align:"Center"} 
					/* No.|도메인|도메인ID|도메인영문명|설명|표준단어|인포타입|용어|테이블|합계 */
						]
					;
			...
			var cols = [						
						...
						{Type:"Text",   Width:150,  SaveName:"dmnId",			Align:"Left", Edit:0 ,Hidden:1},
						{Type:"Text",   Width:150,  SaveName:"dmnPnm",			Align:"Left", Edit:0},
						{Type:"Text",   Width:400,  SaveName:"domnDesc",		Align:"Left", Edit:0},
						{Type:"Int",   Width:100,  SaveName:"stwdNumCnt",		Align:"Center", Edit:0},
						{Type:"Int",   Width:100,  SaveName:"infotypeNumCnt",	Align:"Center", Edit:0},
						{Type:"Int",   Width:100,  SaveName:"itemNumCnt",		Align:"Center", Edit:0},
...
function doAction(sAction)
{
	...
	case "Search":
    	...
    	grid_sheet.DoSearch("<c:url value="/meta/aocd/getDmnPusAocd.do" />", param);
		break;
	case "Down2Excel": //엑셀내려받기
		grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'도메인활용도분석'});
		break;
	}
...
function grid_sheet_OnDblClick(row, col, value, cellx, celly){
	...
	var objId =grid_sheet.GetCellValue(row,"dmnId");
    window.open().location.href ='<c:url value="/meta/stnd/dmn_lst.do" />'+'?objId='+objId;
}
...
function grid_sheet_OnClick(row, col, value, cellx, celly) {
	...
	var tmphtml = '<s:message code="DMN.NM" /> : ' + param.dmnLnm + ' , <s:message code="DMN.ENSN.NM" /> : '+param.dmnPnm; //도메인명, 도메인영문명
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title"><s:message code="DMN.UTLZ.ANLY" /></div> <!--도메인 활용도 분석-->
	</div>
</div>
<!-- 메뉴 메인 제목 -->
        ...
        <form id="frmSearch" name="frmSearch" method="post">
            ...
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="">
                   <caption><s:message code="DMN.UTLZ.ANLY.INQ.FORM" /></caption> <!-- 도메인 활용도 분석 검색폼 -->
                   <colgroup>
                   <col style="width:10%;" />
                   <col style="width:30%;" />
                    <col style="width:10%;" />
                   ...
                   <tbody>                      
                            ...
                            <tr>
                                ...
                                <td><input id="dmnLnm" name="dmnLnm" type="text" class="wd80" style="width:98%"/></td>
                                <th scope="row"><label for="domnDesc"><s:message code="CONTENT.TXT" /></label></th> <!-- 설명 -->
                                <td><input id="domnDesc" name="domnDesc" type="text" class="wd80" style="width:98%" /></td>
                            </tr>
                            <tr>
                                ...
                                <td>
	                                <select id="orderby" class="wd100" name="orderby" style="width:25%">
	                               		<option value="" selected><s:message code="DMN.NM" /></option> <!-- 도메인명 -->
...
<div id="tabs">
	  ...
	  <div id="tabs-1">
			<!-- 	상세정보 ajax 로드시 이용 -->
			<%@include file="../stnd/dmnwhereused_dtl.jsp" %>
