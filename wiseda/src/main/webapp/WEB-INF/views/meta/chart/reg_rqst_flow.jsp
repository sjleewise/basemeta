...
function drawERD(){
	...
	var tableGroup = svg.selectAll("svg.tableGroup")
	 ...
	 //사각형 안에 글씨 쓸때 ie는 text로 크롬은 html로 -> ie에서는 1줄로 표시되고 크롬에서는 줄바뀜이 되서 2줄로 표시됨
	 //ie에서 html function이 안먹음 추후 변경이 필요할것 같음 
		 ...
		 tableGroup.selectAll("g.tableColumn")
			...
			.html(function (d,i) {
				if(d.Name.indexOf("<br/>")>1){
					var val = (66 + i * 50)*per;//2줄 일때 줄 바꿈 기준선
				    return "<tspan x="+(175*per)+" y="+(val-6)+">"+d.Name.replace("<br/>","<\/tspan><tspan x="+(175*per)+" y="+(val+10)+">")+"<\/tspan>";
				}else{
					return d.Name;
				}
			})
	 ...
	 var circle = tableGroup.append("circle")
		...
		.attr("y", function(d){
			if(d.Attribute.length<2){
				return 55*per;//개발영역관리,테스트영역관리,운영영역관리
			}else{
				return (d.Attribute.length*25+30)*per;//표준데이터관리,모델관리
			}
		})
		.attr("text-anchor", "middle")
		.text(function (d,i) {
