...
// 	grid_sub_ddlidxcolchange.FitColWidth();
});


function initsubgrid_ddlidxcolchange() {

    with(grid_sub_ddlidxcolchange){
...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        SetMergeSheet(0);
        var headers = [
                    {Text:"<s:message code='META.HEADER.DDLIDXCOLCHANGE.DTL'/>", Align:"Center"}
                    /* No.|인덱스컬럼ID|만료일시|시작일시|인덱스컬럼물리명|인덱스컬럼논리명|DDL인덱스ID|DDL컬럼ID|인덱스컬럼순서|정렬유형|요청번호|요청일련번호|요청상세일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자명|요청일시|요청사용자명|승인일시|승인사용자명 */
                ];
...
//         FitColWidth();
   	...
   	//==시트설정 후 아래에 와야함=== 
    init_sheet(grid_sub_ddlidxcolchange);    
    //===========================
  	
    //저장 처리 과정을 디버깅 메시지를 팝업으로 표시 (-1)
//     grid_sub_ddlidxcolchange.ShowDebugMsg(-1);	
...
y : y좌표
*/
function grid_sub_ddlidxcolchange_OnDblClick(row, col, value, cellx, celly) {
...
function grid_sub_ddlidxcolchange_OnClick(row, col, value, cellx, celly) {
...
function grid_sub_ddlidxcolchange_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<!-- 그리드 입력 입력 -->
	<div class="grid_01">
	     <script type="text/javascript">createIBSheet("grid_sub_ddlidxcolchange", "100%", "150px")</script>
	</div>
	<!-- 그리드 입력 입력 End -->
