...
<html>
<head>
<title><s:message code="DDL.IDEX.REG" /></title> <!-- DDL인덱스 등록 -->
...
// 			doAction("Approve");
        ...
        $("#btnBlank").click( function(){
    		location.href = '<c:url value="/meta/ddl/ddlidx_rqst.do" />';
        } );
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.DDLIDX.RQST1'/>";
        /* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과 */
        	headtext += "<s:message code='META.HEADER.DDLIDX.RQST2'/>";
        	/* |주제영역|DDL테이블ID|테이블(물리명)|테이블(논리명) */
			        	
        	headtext += "<s:message code='META.HEADER.DDLIDX.RQST3'/>";
        	/* |DDL인덱스ID|인덱스(물리명)|인덱스(논리명)|DDL|UNIQUE여부|인덱스유형 */
        	
        	headtext += "<s:message code='META.HEADER.DDLIDX.RQST4'/>";
        	/* |DB접속대상명|DB스키마명|인덱스스페이스 */
        	headtext += "<s:message code='META.HEADER.DDLIDX.RQST5'/>";
        	/* |스크립트정보|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호 */
        	
        	//요청번호 요청일련번호 DDL테이블ID DDL테이블물리명 DDL테이블논리명 이전DDL테이블물리명 요청구분코드 검토상태코드 검토내용 검증코드 검증비고 
        	//DB접속대상물리명 DB사용자ID DB사용자물리명 테이블스페이스ID 테이블스페이스물리명 테이블변경유형코드 물리모델테이블ID 스크립트정보 
        	//객체설명 객체버전 등록유형코드 최초요청일시 최초요청사용자ID 요청일시 요청사용자ID 승인일시 승인사용자ID 
        ...
        var cols = [                        
                    ...
                    //{Type:"Text",   Width:60,  SaveName:"bfDdlTblPnm",   Align:"Left",   Edit:0},
                    
                    {Type:"Text",   Width:40,   SaveName:"ddlIdxId", 	 Align:"Left",   Edit:0, Hidden:1},
                    {Type:"Text",   Width:120,  SaveName:"ddlIdxPnm", 	 Align:"Left",   Edit:0, KeyField:1},
                    {Type:"Text",   Width:120,  SaveName:"ddlIdxLnm", 	 Align:"Left",   Edit:0, Hidden:1},
                    {Type:"CheckBox", Width:70, SaveName:"ddlCheck", 	 Align:"Center",   Edit:1},
                    {Type:"Combo",  Width:70,   SaveName:"ukIdxYn", 	 Align:"Center",   Edit:0, Hidden:1},
                    {Type:"Combo",  Width:100,  SaveName:"idxTypCd", 	 Align:"Center",   Edit:0},
                    ...
                    {Type:"Text",   Width:120,  SaveName:"idxSpacPnm"	,    Align:"Left",   Edit:0, KeyField:0},
                    
                    //{Type:"Combo",  Width:80,  SaveName:"tblChgTypCd",  Align:"Left",   Edit:0},
                    //{Type:"Text",   Width:40,  SaveName:"pdmTblId"	,   Align:"Left",   Edit:0},
...
function doAction(sAction)
{
  ...
  switch(sAction)
  {
      ...
      case "AddWam": //기존 테이블 추가
			var url   = "<c:url value="/meta/ddl/popup/ddlidx_pop.do"/>";
			var param = "popRqst=Y";
...
//			if(saveJson.data.length == 0) return;
      ...
      case "Down2Excel":  //엑셀내려받기
          grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'DDL인덱스생성요청'});
          break;
          
      case "LoadExcel":  //엑셀업로드
			var url = "<c:url value="/meta/ddl/popup/ddlidxcol_xls.do" />";
...
//			if (saveJson.Code == "IBS000") return;
	...
	//       	var DelJson = col_sheet.GetSaveJson(0, "ibsCheck");
	      	...
	      	var url = "<c:url value="/meta/ddl/delddlidxcolrqstlist.do"/>";
...
function loadDetail(param) {
	...
	$('div#detailInfo').load('<c:url value="/meta/ddl/ajaxgrid/ddlidx_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//				doActionCol("Search");
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/ddl/ddlidx_rqst.do" />';
			if(!isBlankStr(res.resultVO.rqstNo)) {
...
function showDDL() {
	...
	if(saveJson.data.length == 0) {
		...
		//DDL체크박스에 체크하세요.
		showMsgBox("ERR", "<s:message code='DDL.CHK.CHK' />"); 
		return;
	}
...
//   tblClick(row);
	...
	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '인덱스 : ' + param.ddlIdxPnm + ' [' + param.ddlIdxLnm +']';
	$('#tbl_sle_title').html(tmphtml);
...
//		$('#btnfrmReset').click();
		...
		if(grid_sheet.RowCount() > 0) {
			...
			grid_sheet.CheckAll("ddlCheck",1);
		}
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title"><s:message code="DDL.IDEX.REG" /></div> <!-- DDL인덱스 등록 -->
	</div>
</div>
<!-- 메뉴 메인 제목 -->
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div class="selected_title_area">
		    <div class="selected_title" id="tbl_sle_title"><s:message code="MSG.IDEX.CHC" /></div> <!-- 인덱스를 선택하세요. -->
	</div>
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1"><s:message code="IDEX.DFNT.P" /></a></li> <!-- 인덱스 정의서 -->
	    <li><a href="#tabs-2"><s:message code="IDEX.CLMN.LST" /></a></li> <!-- 인덱스컬럼 목록 -->
	    <li id="tabs-rqstvrf"><a href="#tabs-3"><s:message code="VRFC.RSLT" /></a></li> <!-- 검증결과 -->
	    <li id="tabs-rqstchg"><a href="#tabs-4"><s:message code="IDEX.CHG.ITEM" /></a></li> <!-- 인덱스변경항목 -->
	    <li id="tabs-rqstchg"><a href="#tabs-5" id="colinfo"><s:message code="IDEX.CLMN.CHG.ITEM" /></a></li> <!-- 인덱스컬럼변경항목 -->
	  ...
	  <div id="tabs-1">
		...
		<!-- 컬럼 목록 탭 -->
	  <div id="tabs-2">
		<%@include file="ddlidxcol_rqst.jsp" %>
	  </div>
	  <div id="tabs-3">
