...
function initRelGrid()
{
    ...
    with(rel_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.DDLREL.RQST1'/>";
        /* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|관계ID|관계물리명|관계논리명 */
	    	headtext += "<s:message code='META.HEADER.DDLREL.RQST2'/>";
	    	/* |관계유형코드|카디널리티유형코드|Parent Optionality유형코드 */
	    	headtext += "<s:message code='META.HEADER.DDLREL.RQST3'/>";
	    	/* |부모엔터티주제영역ID|부모엔터티주제영역명|부모엔터티ID|부모엔터티명|부모엔터티속성ID|부모엔터티속성명 */
	    	headtext += "<s:message code='META.HEADER.DDLREL.RQST4'/>";
	    	/* |자식엔터티주제영역ID|자식엔터티주제영역명|자식엔터티ID|자식엔터티명|자식엔터티속성ID|자식엔터티속성명 */
		    headtext += "<s:message code='META.HEADER.DDLREL.RQST5'/>";
        ...
        var cols = [                        
					...
					{Type:"Text",    Width:100, SaveName:"ddlRelId"    ,Align:"Left",   Edit:0, Hidden:1},
                    {Type:"Text",    Width:100, SaveName:"ddlRelPnm"   ,Align:"Left",   Edit:0, KeyField:1},
                    {Type:"Text",    Width:100, SaveName:"ddlRelLnm"   ,Align:"Left",   Edit:0},
...
//         	$.postJSON(url, DelJson, ibscallbackRel);
            IBSpostJson(url, param, ibscallbackRel);
        	break;
        case "Save" :
        	//TODO 공통으로 처리...
...
//         	ibsSaveJson = rel_sheet.GetSaveJson(1);	//DoAllSave와 동일...
            ...
            var url = "<c:url value="/meta/ddl/regddlrelrqstlist.do"/>";
...
// 			var param1 =  grid_sheet.GetRowJson(row);
        	...
        	//요청서 마스터 번호로 조회한다...
//         	var param = $('#relfrmSearch').serialize();
        	var param = $('#mstFrm').serialize();
        	    param += "&rqstSno=" + rqstsno;
        	
        	rel_sheet.DoSearch("<c:url value="/meta/ddl/getddlrelrqstlist.do" />", param);
//         	rel_sheet.DoSearchScript("testJsonlist");
        ...
        case "SearchRow": //단일 조회...
        	...
        	rel_sheet.DoRowSearch(crow, "<c:url value="/meta/ddl/getddlrelrqstinfo.do" />",  param ,0);
        break;
        ...
        case "LoadExcel":  //엑셀업로드
			var url = "<c:url value="/meta/ddl/popup/ddlrel_xls.do" />";
			
			var xlspopup = OpenWindow(url ,"ddlrelxls","800","600","yes");
...
function postProcessRelIBS(res) {
	...
	switch(res.ETC.action) {
		...
		case "<%=WiseMetaConfig.IBSAction.REG_LIST%>" : 
	    	...
	    	if(!isBlankStr(res.ETC.rqstNo)) {
	    		//alert(res.ETC.rqstNo);
	    		$("form#relfrmSearch input[name=rqstNo]").val(res.ETC.rqstNo);
// 	    		$("form#relfrmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
				doActionRel("Search");    		
	    	}
...
// 	$('div#detailInforel').load('<c:url value="/meta/ddl/ajaxgrid/ddlrel_rqst_dtl.do"/>', param, function( response, status, xhr ) {
// 		  if ( status == "error" ) {
...
function rel_sheet_OnSaveEnd(code, message) {
	//alert(code);
	if (code == 0) {
...
<div id="grid_02" class="grid_01">
	...
	<%-- 	     <script type="text/javascript">createIBSheet2($("#grid_01"), "rel_sheet", "100%", "100%");</script>             --%>
</div>
<!-- 그리드 입력 입력 -->
<div style="clear: both; height: 5px;">
	<span></span>
...
<!-- 	<div class="selected_title" id="rel_sel_title">컬럼을 선택하세요.</div> -->
<!-- </div> -->
