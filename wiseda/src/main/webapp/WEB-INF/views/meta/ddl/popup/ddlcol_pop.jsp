...
<html>
<head>
<title><s:message code="DDL.TBL.SRH" /></title> <!-- DDL 테이블 검색 -->
...
// 	alert("${search.dbConnTrgId}");
// 	alert("${search.dbSchId}");
	$("#dbConnTrgId").val("${search.dbConnTrgId}").change(); 
	$("#dbSchId").val("${search.dbSchId}").change();
	
// 	doAction("Search");
	if($("#ddlTblPnm").val() != '') {
		$("#ddlTblPnm").attr("readOnly", true);
		$("#dbConnTrgId").attr('disabled', 'true');
		$("#dbSchId").attr('disabled', 'true');
	}
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext = "<s:message code='META.HEADER.DDLCOL.POP'/>"
        	/* No.|상태|선택|주제영역ID|주제영역|테이블ID|DDL테이블(물리명)|DDL테이블(논리명)|컬럼ID|DDL컬럼(물리명)|DDL컬럼(논리명)|요청자명|객체설명| */
// 	        +"객체버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|";
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   Width:80, SaveName:"ddlColId",   	 Align:"Left",   Edit:0},
...
// 	     SetColProperty("regTypCd", 	{ComboCode:"C|U|D", ComboText:"<s:message code='NEW.CHG.DEL' />"});/* 신규|변경|삭제 */
        ...
        SetColHidden("ddlColId",1);
        SetColHidden("ddlTblLnm",1);
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "Search":
        	...
        	param += "&dbConnTrgId=" + $("#dbConnTrgId").val(); 
        	param += "&dbSchId=" + $("#dbSchId").val(); 
        	grid_sheet.DoSearch("<c:url value="/meta/ddl/ajaxgrid/ddltblcollist_dtl.do" />", param);
...
//         	alert(retval);
 	 	...
 	 	//DDL 파티션 키 팝업 호출시 ==> 컬럼명을 ','로 조인처리한다.
        	if ("DDLPART" == "${search.vrfRmk}" || "DDLSUBPART" == "${search.vrfRmk}"){
        		
        		if(checkDelIBS (grid_sheet, "파티션 키를 선택하세요.")) {
        			var retval = getibscheckjoin(grid_sheet, "ddlColPnm", ",");
        			//iframe 형태의 팝업일 경우
        			if ("${search.popType}" == "I") {
        				parent.returnPartKeyPop(retval, "${search.vrfRmk}");
        			} else {
        				opener.returnPartKeyPop(retval, "${search.vrfRmk}");
        			}
...
function grid_sheet_OnDblClick(row, col, value, cellx, celly) {
	...
	//iframe 형태의 팝업일 경우
	if ("${search.popType}" == "I") {
		parent.returnDdlColPop(JSON.stringify(retjson));
	} else {
		opener.returnDdlColPop(JSON.stringify(retjson));
	}
...
<div class="pop_tit"> <!-- 팝업가로사이즈 여기서 조절하면 됩니다 기본은 100% -->
    ...
    <div class="pop_tit_txt"><s:message code="DDL.CLMN.INQ" /></div> <!-- DDL컬럼 검색 -->
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='SUBJ.TRRT.INQ' />"> <!-- 주제영역조회 -->
                   ...
                   <tbody>    
                   	   <tr>
							...
							<td colspan="3"><select class = "wd100" id="dbConnTrgId" class="" name="dbConnTrgId">
...
<!--                             </td> -->
                           ...
                           <th scope="row"><label for="ddlColLnm"><s:message code="CLMN.NM" /></label></th> <!-- 컬럼명 -->
                            <td>
                                <span class="input_file">
                                <input type="text" name="ddlColLnm" id="ddlColLnm" value="${search.ddlColLnm}"/>
                                </span>
                            </td>
