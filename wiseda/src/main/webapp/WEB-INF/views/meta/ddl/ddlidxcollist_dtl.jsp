...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        var headers = [
                    {Text:"<s:message code='META.HEADER.DDLIDXCOLLIST.DTL'/>", Align:"Center"}
                    /* No.|등록유형코드|버전|인덱스컬럼ID|인덱스컬럼물리명|인덱스컬럼논리명|DDL인덱스ID|DDL컬럼ID|인덱스컬럼순서|정렬유형|요청번호|요청일련번호|요청상세일련번호|설명|최초요청일시|최초요청사용자명|요청일시|요청사용자명|승인일시|승인사용자명 */
                ];
...
function grid_sub_ddlidxcollist_OnDblClick(row, col, value, cellx, celly) {
	...
	if(row < 1) return;
	//선택한 상세정보를 가져온다...
	var ddlIdxColId = "&ddlIdxColId="+grid_sub_ddlidxcollist.GetCellValue(row, "ddlIdxColId");
	ddlIdxColId += "&objVers="+grid_sub_ddlidxcollist.GetCellValue(row, "objVers");
	ddlIdxColId += "&rqstNo="+grid_sub_ddlidxcollist.GetCellValue(row, "rqstNo");

	//DDL컬럼ID를 토대로 조회한다....
	loadDetailIdxCol(ddlIdxColId);
	

// 	grid_sub_ddlidxcolchange.DoSearch('<c:url value="/meta/ddl/getDdlIdxOneColChange.do" />', ddlIdxColId);
