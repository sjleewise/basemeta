...
function initGrid()
{
    ...
    with(grid_sheet){
		...
		var headers = [
						{Text:"<s:message code='META.HEADER.PARTITIONTBL.LST'/>", Align:"Center"}
						/* No.|상태|등록유형|DBMS ID|DBMS명|스키마ID|DB스키마명|테이블구분|오브젝트ID|테이블명|테이믈물리명|파티션여부|테이블종류 */
					];
			...
			var cols = [
						...
						{Type:"CheckBox",   Width:90,   SaveName:"partTblYn", 		Align:"Center", Edit:1, Hidden:0},
						{Type:"Combo",   Width:100,   SaveName:"tblTypCd", 		Align:"Center", Edit:0, Hidden:0},
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "Search":
			...
			grid_sheet.DoSearch("<c:url value="/meta/ddl/getPartitionTblList.do" />", param);
        ...
        case "Save" :
        	...
        	var url = '<c:url value="/meta/ddl/savePartitionTblPrc.do"/>';
...
//$("#hdnRow").val(row);
	...
	var param =  grid_sheet.GetRowJson(row);
		...
		ddlTblId += "&rqstNo="+grid_sheet.GetCellValue(row, "rqstNo");
