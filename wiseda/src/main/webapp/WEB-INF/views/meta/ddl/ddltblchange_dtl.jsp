...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        SetMergeSheet(1);
        var headers = [
                    {Text:"<s:message code='META.HEADER.DDLTBLCHANGE.DTL'/>", Align:"Center"}
                    /* No.|등록유형코드|DDL테이블ID|만료일시|시작일시|DDL테이블물리명|DDL테이블논리명|DB스키마ID|테이블스페이스ID|물리모델테이블ID|스크립트정보|요청번호|요청일련번호|설명|버전|최초요청일시|최초요청사용자명|요청일시|요청사용자명|승인일시|승인사용자명 */
                ];
...
function grid_sub_ddltblchange_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
