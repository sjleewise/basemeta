...
function initGrid()
{
    ...
    with(grid_sheet){
		...
		var headers = [
						{Text:"<s:message code='META.HEADER.DDLHIST.LST'/>", Align:"Center"}
						/* No.|DBMS명|DB스키마명|DDL테이블ID|DDL테이블물리명|DDL테이블논리명|테이블스페이스ID|물리모델테이블명|스크립트정보|요청번호|요청일련번호|등록유형코드|버전|설명|최초요청일시|최초요청자명|요청일시|요청자명|승인일시|승인자명 */
					];
			...
			var cols = [
						...
						{Type:"Text",   Width:110,   SaveName:"dbConnTrgLnm",	 	Align:"Left", Edit:0, Hidden:0},
						{Type:"Text",   Width:110,   SaveName:"dbSchLnm",	 	Align:"Left", Edit:0, Hidden:0},
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "Search":
			...
			loadDetailCol();
// 			loadDetailScript();
			grid_sheet.DoSearch("<c:url value="/meta/ddl/getDdlHistlist.do" />", param);
...
// 	if ('ibsSeq' == colsavename || 'ibsStatus' == colsavename || 'ibsCheck' == colsavename) return;
	...
	var ddlTblId = "&ddlTblId="+grid_sheet.GetCellValue(row, "ddlTblId")+"&objVers="+grid_sheet.GetCellValue(row, "objVers");

	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '<s:message code="DDL.TBL.NM" /> : ' + param.ddlTblPnm; //DDL 테이블명
...
//		grid_sub_dmnvaluechange.DoSearch('<c:url value="/meta/stnd/ajaxgrid/dmnvaluechange_dtl.do" />', param);
//		grid_sub_dmnwhereused.DoSearch('<c:url value="/meta/stnd/ajaxgrid/dmnwhereused_dtl.do" />', param);
}

//상세정보호출
function loadDetail(param) {
	$('div#detailInfo1').load('<c:url value="/meta/ddl/ajaxgrid/ddltblinfohist_dtl.do"/>', param, function(){
		if(param == null || param == "" || param =="undefined"){
			
		} else {
			//grid_sub_ddltblchange.DoSearch('<c:url value="/meta/ddl/ajaxgrid/ddltblchange_dtl.do" />', param);
			grid_sub_ddltblcollist.DoSearch('<c:url value="/meta/ddl/ajaxgrid/ddltblcollisthist_dtl.do" />', param);
			grid_sub_ddltblcollist.SetColHidden("objVers",0);
			//grid_sub_ddltblrellist.DoSearch('<c:url value="/meta/ddl/ajaxgrid/ddltblrellisthist_dtl.do" />', param);
			//grid_sub_ddltblrelchange.DoSearch('<c:url value="/meta/ddl/ajaxgrid/ddltblrelchange_dtl.do" />', param);
			//grid_sub_ddltblidxcol.DoSearch('<c:url value="/meta/ddl/ajaxgrid/ddltblidxcolhist_dtl.do" />', param);
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='STRD.TERMS.INQ' />"> <!-- 표준용어조회 -->
                   ...
                   <tbody>                            
						...
						<tr>
							...
							<th scope="row"><label for="ddlColLnm"><s:message code="CLMN.NM" /></label></th> <!-- 컬럼명 -->
