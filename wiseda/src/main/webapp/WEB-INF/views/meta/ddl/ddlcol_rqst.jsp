...
function initColGrid()
{
    ...
    with(col_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.DDLCOL.RQST1'/>";
        /* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과 */
        	headtext += "<s:message code='META.HEADER.DDLCOL.RQST2'/>";
        	/* |테이블(물리명)|컬럼ID|컬럼(물리명)|컬럼(논리명)|이전컬럼(물리명) */
        	headtext += "<s:message code='META.HEADER.DDLCOL.RQST3'/>";
        	/* |테이블ID|컬럼순서|데이터타입|길이|소수점길이|PK 여부|PK 순서|NOT NULL여부|DEFAULT 값 */
        	headtext += "<s:message code='META.HEADER.DDLCOL.RQST4'/>";
        	/* |컬럼수정여부|데이터타입수정여부|NOT NULL수정여부|DEFAULT값 수정여부 */
		    headtext += "<s:message code='META.HEADER.DDLCOL.RQST5'/>";
		    /* |암호화여부|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호|요청상세일련번호 */
        ...
        var cols = [                        
                    ...
                    {Type:"Text",    Width:150, SaveName:"ddlColPnm"   ,Align:"Left",   Edit:0, KeyField:1},
                    {Type:"Text",    Width:150, SaveName:"ddlColLnm"   ,Align:"Left",   Edit:0, KeyField:1},
                    {Type:"Text",    Width:40,  SaveName:"bfDdlColPnm" ,Align:"Left",   Edit:0, Hidden:1},
                    ...
                    {Type:"Text",    Width:60,  SaveName:"dataLen"     ,Align:"Center",   Edit:0},
                    {Type:"Text",    Width:60,  SaveName:"dataScal"    ,Align:"Center",   Edit:0},
                    {Type:"Combo",   Width:60,  SaveName:"pkYn"        ,Align:"Center",   Edit:0},
                    {Type:"Int",     Width:60,  SaveName:"pkOrd"       ,Align:"Center",   Edit:0},
                    {Type:"Combo",   Width:60,  SaveName:"fkYn"        ,Align:"Center",   Edit:0},
                    {Type:"Combo",   Width:60,  SaveName:"akYn"        ,Align:"Center",   Edit:0},
                    {Type:"Combo",   Width:100,  SaveName:"nonulYn"     ,Align:"Center",   Edit:0},
                    {Type:"Text",    Width:100,  SaveName:"defltVal"    ,Align:"Left",   Edit:0},

                    {Type:"Combo",    Width:80,  SaveName:"colUpdYn"    ,Align:"Center",   Edit:0, Hidden:1},
                    {Type:"Combo",    Width:80,  SaveName:"dataTypeUpdYn" ,Align:"Center",   Edit:0, Hidden:1},
                    {Type:"Combo",    Width:80,  SaveName:"nonulUpdYn"    ,Align:"Center",   Edit:0, Hidden:1},
                    {Type:"Combo",    Width:80,  SaveName:"defltValUpdYn" ,Align:"Center",   Edit:0, Hidden:1},
                    {Type:"Combo",    Width:80,  SaveName:"encYn"    ,Align:"Center",   Edit:0, Hidden:0},        
                    {Type:"Text",   Width:120,  SaveName:"objDescn",	Align:"Left", 	 Edit:0, Hidden:1},
					{Type:"Text",   Width:150,  SaveName:"rqstDtm",  	Align:"Center", Edit:0, Format:"yyyy-MM-dd HH:mm:ss"},
		...
		SetColProperty("colUpdYn", 	{ComboCode:"N|Y", ComboText:"<s:message code='COMBO.NO.YES'/>"}); /* 아니요|예 */
		SetColProperty("dataTypeUpdYn", 	{ComboCode:"N|Y", ComboText:"<s:message code='COMBO.NO.YES'/>"}); /* 아니요|예 */
		SetColProperty("nonulUpdYn", 	{ComboCode:"N|Y", ComboText:"<s:message code='COMBO.NO.YES'/>"}); /* 아니요|예 */
		SetColProperty("defltValUpdYn", 	{ComboCode:"N|Y", ComboText:"<s:message code='COMBO.NO.YES'/>"}); /* 아니요|예 */
...
//         		return false ;
//         	}
// 			var param1 =  grid_sheet.GetRowJson(row);
...
//         	var param = $('#colfrmSearch').serialize();
        	...
        	col_sheet.DoSearch("<c:url value="/meta/ddl/getddlcolrqstlist.do" />", param);
//         	col_sheet.DoSearchScript("testJsonlist");
        ...
        case "SearchRow": //단일 조회...
        	...
        	col_sheet.DoRowSearch(crow, "<c:url value="/meta/model/getpdmcolrqstinfo.do" />",  param ,0);
        break;
        ...
        case "LoadExcel":  //엑셀업로드
			var url = "<c:url value="/meta/model/popup/pdmcol_xls.do" />";
...
// 	$('div#detailInfocol').load('<c:url value="/meta/model/ajaxgrid/ddlcol_rqst_dtl.do"/>', param, function( response, status, xhr ) {
// 		  if ( status == "error" ) {
...
function col_sheet_OnDblClick(row, col, value, cellx, celly) {
	...
	//param1 += "&rqstSno=" + param.rqstSno + "&rqstDtlSno=" + param.rqstDtlSno  + "&searchObj=" + param.ddlTblPnm + "&subInfo=COL";
	//param1 += "&bizDcd=DTT&bizDtlCd=TSFCOL";
	var param1  = "bizDcd=DDL&bizDtlCd=DDLCOL";
	    param1 += "&rqstNo="+param.rqstNo;
	    param1 += "&rqstSno="+param.rqstSno;
	    param1 += "&rqstDtlSno="+param.rqstDtlSno;
	    param1 += "&searchObj="+param.ddlTblPnm;
	$("#tabs #colinfo").click();
...
// 	var tmphtml = '컬럼명 : ' + param.pdmColPnm + ' [' + param.pdmColLnm +']';
// 	$('#col_sel_title').html(tmphtml);
	
// 	loadDetailCol(param);
	
	var param1  = "bizDcd=DTT&bizDtlCd=TSFCOL";
	    param1 += "&rqstNo="+param.rqstNo;
	    param1 += "&rqstSno="+param.rqstSno;
	    param1 += "&rqstDtlSno="+param.rqstDtlSno;
	    param1 += "&searchObj="+param.ddlTblPnm;
...
<div class="divLstBtn" style="padding-right: 10px">
	<div class="bt03">
		<button class="btn_rqst_new" id="btnColRqstNew" name="btnColRqstNew"><s:message code="ADDT" />
</button> <!-- 추가 -->
		<ul class="add_button_menu">
			...
			<li class="btn_excel_load" id="btnColExcelLoad"><a><span class="ui-icon ui-icon-document"></span><s:message code="EXCL.UP" /></a></li> <!-- 엑셀 올리기 -->
		</ul>
...
<!-- 그리드 입력 입력 -->
<div id="grid_col" class="grid_01">
	<script type="text/javascript">createIBSheet("col_sheet", "100%", "500px");</script>
</div>
...
<!-- 	<div class="selected_title" id="col_sel_title">컬럼을 선택하세요.</div> -->
<!-- </div> -->
