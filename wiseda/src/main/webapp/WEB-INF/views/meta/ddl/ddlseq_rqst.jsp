...
// 			doAction("Approve");
        ...
        $("#btnBlank").click( function(){
    		location.href = '<c:url value="/meta/ddl/ddlseq_rqst.do" />';
        } );
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.DDLSEQ.RQST'/>";
        //No.|상태|선택|검토상태|검토내용|요청사유|요청구분|등록유형|검증결과|SR번호|서브프로젝트번호 |DDL시퀀스ID|시퀀스(물리명)|시퀀스(논리명)|DB접속대상(물리명)|DB사용자(물리명)|명명규칙|사용용도|테이블명|컬럼명|L1코드|L3코드|권한목록|INCREMENT BY|START WITH|MIN VALUE|MAX VALUE|CYCLE여부|ORDER여부|CACHE SIZE|패키지명|클래스명|초기화구분코드|초기화실패업무영향도|스크립트정보|적용요청구분|적용요청일자|DDL|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
        ...
        var cols = [                        
					...
					{Type:"Text",   Width:100,   SaveName:"ddlSeqPnm",	 Align:"Center",   Edit:0, KeyField:1},
					{Type:"Text",   Width:0,   SaveName:"ddlSeqLnm",	 Align:"Center",   Edit:0, Hidden:1},		
					{Type:"Text",   Width:100,   SaveName:"dbConnTrgPnm",	 Align:"Center",   Edit:0, KeyField:1},
					{Type:"Text",   Width:100,   SaveName:"dbSchPnm",	 Align:"Center",   Edit:0, KeyField:1},
					...
					{Type:"Text",   Width:100,   SaveName:"incby",	 Align:"Center",   Edit:0, KeyField:1},
					{Type:"Text",   Width:100,   SaveName:"strtwt",	 Align:"Center",   Edit:0, KeyField:1},
					{Type:"Text",   Width:100,   SaveName:"minval",	 Align:"Center",   Edit:0, KeyField:1},
					{Type:"Text",   Width:100,   SaveName:"maxval",	 Align:"Center",   Edit:0, KeyField:1},
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "AddWam": //기존 시퀀스 추가
			var url   = "<c:url value="/meta/ddl/popup/ddlseq_pop.do"/>";
			var param = "popRqst=Y";
...
//         	ibsSaveJson = grid_sheet.GetSaveJson(1);	//DoAllSave와 동일...
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/ddl/getddlseqrqstlist.do" />", param);
			//컬럼 리스트 조회...
// 			doActionCol("Search");
		...
		/* 우리은행용 	if(approveChkAplRegDt(saveJson, grid_sheet,$("form[name=mstFrm]").find("input[name=aqdAlwYn]").val())){
...
// 			if(saveJson.data.length == 0) return;
        ...
        case "Down2Excel":  //엑셀내려받기
            grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'DDL시퀀스생성요청'});
            break;
...
function loadDetail(param) {
	...
	$('div#detailInfo').load('<c:url value="/meta/ddl/ajaxgrid/ddlseq_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/ddl/ddlseq_rqst.do" />';
			if(!isBlankStr(res.resultVO.rqstNo)) {
...
<!--        		<b>* 시퀀스 명명규칙</b>
		...
		<div class="selected_title_area">
			<div class="selected_title" id="seq_sle_title">시퀀스를 선택하세요.</div>
		</div>
