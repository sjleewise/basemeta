...
function initColGrid()
{
    ...
    with(col_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.DDLIDXCOL.RQST1'/>";
        /* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과 */
        	headtext += "<s:message code='META.HEADER.DDLIDXCOL.RQST2'/>";
        	/* |DDL컬럼ID|인덱스ID|인덱스(물리명) */
        	headtext += "<s:message code='META.HEADER.DDLIDXCOL.RQST3'/>";
        	/* |인덱스컬럼ID|인덱스컬럼(물리명)|인덱스컬럼(논리명)|인덱스컬럼순서|정렬순서 */


        	//headtext += "|데이터타입|길이|소수점길이|PK 여부|PK 순서|NOT NULL여부|DEFAULT 값";
		    headtext += "<s:message code='META.HEADER.DDLIDXCOL.RQST4'/>";
...
//                     {Type:"Text",    Width:80,  SaveName:"defltVal"    ,Align:"Left",   Edit:0},
					...
					{Type:"Int",    Width:50,   SaveName:"rqstDtlDtlSno",  Align:"Center", Edit:0}
                ];
...
function dataMoveIBS(ibsobj, action) {
	...
	for(var i=1; i<=lrow; i++) {
		ibsobj.SetCellValue(i, "ddlIdxColOrd", i);
...
//         	ibsSaveJson = col_sheet.GetSaveJson(1);	//DoAllSave와 동일...
            ...
            var url = "<c:url value="/meta/ddl/regpdmcolrqstlist.do"/>";
...
// onClick시 아래 컬럼 입력폼 나타나지 않도록 주석처리. (인덱스 정의서에서 인덱스컬럼 수정하도록 하세요.) 
// 	//선택한 상세정보를 가져온다...
// 	var param = col_sheet.GetRowJson(row);
	
// 	//선택한 그리드의 row 내용을 보여준다.....
// 	var tmphtml = '<s:message code="CLMN.NM" /> : ' + param.ddlIdxColPnm + ' [' + param.ddlIdxColLnm +']'; //컬럼명
// 	$('#col_sel_title').html(tmphtml);
	
// 	loadDetailCol(param);
	
// 	var param1  = "bizDtlCd=DDLIDXCOL";
...
// 		param2 += "&rqstSno=" + param.rqstSno 
// 		param2 += "&rqstDtlSno=" + param.rqstDtlSno
// 		param2 += "&rqstDtlDtlSno=" + param.rqstDtlDtlSno;
// 		grid_changecol.RemoveAll();
...
<!-- 			    <button class="btn_reg_rqst" id="btnRegRqst" name="btnRegRqst">등록</button>  -->
            ...
            <div id="divColMove" class="bt03" style="padding-left: 10px;">
				<button class="btn_move_top"   id="btnColMoveTop"><s:message code="TOP.UPPER.MOVE" /></button> <!-- 맨 위로 이동 -->
				<button class="btn_move_up"   id="btnColMoveUp"><s:message code="UPPER.MOVE" /></button> <!-- 위로 이동 -->
				<button class="btn_move_down" id="btnColMoveDown"><s:message code="UNDER.MOVE" /></button> <!-- 아래로 이동 -->
				<button class="btn_move_bottom" id="btnColMoveBottom"><s:message code="TOP.UNDER.MOVE" /></button> <!-- 맨 아래로 이동 -->
			</div>
			<div class="bt03" style="padding-left: 10px;">
				<button class="btnSearchPop"   id="ddlColAutoPop"><s:message code="EASY.ADDT" /></button> <!-- 간편추가 -->
			</div>
			<div class="bt02">
<!-- 	          <button class="btn_excel_load" id="btnColExcelLoad" name="btnColExcelLoad">엑셀 업로드</button>                        -->
         ...
         <!-- 버튼영역  -->
<div style="clear:both; height:5px;"><span></span></div>
    <!-- <div class="tb_comment"> * <s:message code="MSG.EASY.ADDT.BTN.IDEX.CLMN.ADDT" /></div> <!-- 간편추가 버튼을 클릭하면 팝업창을 통해서 인덱스컬럼을 간편하게 추가할 수 있습니다. --> 
	<!-- 그리드 입력 입력 -->
...
<%-- 		    <div class="selected_title" id="col_sel_title"><s:message code="MSG.IDEX.CLMN.CHC" /></div> <!-- 인덱스컬럼을 선택하세요. --> --%>
<!-- 	</div> -->
