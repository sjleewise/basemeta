...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext = "<s:message code='META.HEADER.DDLTBLMST.POP'/>"
        	/* No.|상태|선택|주제영역id|주제영역|스키마ID|DDL테이블ID|DDL테이블물리명|DDL테이블논리명|설명| */
// 	        +"객체버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|";
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   Width:80, SaveName:"ddlTblId",   	 Align:"Left",   Edit:0,Hidden:1},
                    {Type:"Text",   Width:200, SaveName:"ddlTblPnm",     Align:"Left",   Edit:0}, 
                    {Type:"Text",   Width:200,  SaveName:"ddlTblLnm", 	 Align:"Left",   Edit:0},
...
function grid_sheet_OnDblClick(row, col, value, cellx, celly) {
	...
	if ("${search.popType}" == "I") {
		if("${search.mstDcd}" =="Y"){
		   parent.returnMstTblPop(JSON.stringify(retjson));
		}else{
		   parent.returnHistTblPop(JSON.stringify(retjson));
		}
	} else {
		if("${search.mstDcd}" =="Y"){
			   opener.returnMstTblPop(JSON.stringify(retjson));
			}else{
			   opener.returnHistTblPop(JSON.stringify(retjson));
			}
	}
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='SUBJ.TRRT.INQ' />"> <!-- 주제영역조회 -->
                   ...
                   <tbody>
                       ...
                       <tr>                               
                           ...
                           <th scope="row"><label for="ddlTblLnm"><s:message code="TBL.NM" /></div> <!-- 테이블명 -->
                            <td>
                                <span class="input_file">
                                <input type="text" class="wd200" name="ddlTblLnm" id="ddlTblLnm" value=""/>
                                </span>
                            </td>
                       </tr>
                       <tr>
