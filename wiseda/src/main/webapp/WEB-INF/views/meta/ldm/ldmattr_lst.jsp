...
function initColGrid()
{
    ...
    with(col_sheet){
        ...
        var headtext = "<s:message code='META.HEADER.PDMCOL.LST'/>";
            //No.|컬럼ID|물리모델테이블ID|물리모델테이블|물리모델테이블물리명|표준용어ID|표준용어명|표준용어물리명|도메인ID|도메인명|도메인물리명|인포타입ID|인포타입명|컬럼명|컬럼한글명|컬럼순서"
            //+ "|데이터타입|PK여부|PK순서|NOTNULL여부|DEFAULT값|암호화여부|등록유형" //길이|소수점|
            //+ "|최초요청자ID|최초요청자|최초요청일시|요청자ID|요청자|요청일시|승인자ID|승인자|승인일시|버전|설명

//         headtext = "No.|어트리뷰트ID|엔티티ID|엔티티명|표준용어ID|표준용어명|표준용어물리명|도메인ID|도메인명|도메인물리명|인포타입ID|인포타입명|어트리뷰트명|순서";    
//         headtext += "|데이터타입|PK여부|PK순서|NOTNULL여부|DEFAULT값|등록유형";
//         headtext += "|최초요청자ID|최초요청자|최초요청일시|요청자ID|요청자|요청일시|승인자ID|승인자|승인일시|버전|설명";
        ...
        var cols = [                        
                    ...
                    {Type:"Text",    Width:100,  SaveName:"ldmAttrId"     , Align:"Left",   Edit:0, Hidden:1},
                    {Type:"Text",    Width:100,  SaveName:"ldmEntyId"     , Align:"Left",   Edit:0, Hidden:1},
                    {Type:"Text",    Width:200,  SaveName:"ldmEntyLnm"    , Align:"Left",   Edit:0, Hidden:1},                    
                    ...
                    {Type:"Text",    Width:200,  SaveName:"ldmAttrLnm"    , Align:"Left",   Edit:0},
                    {Type:"Text",    Width:80,   SaveName:"attrOrd"       , Align:"Center",   Edit:0},

                    {Type:"Text",    Width:130,  SaveName:"dataType"      , Align:"Center",   Edit:0},
//                     {Type:"Text",    Width:50,  SaveName:"dataLen"     ,Align:"Center",   Edit:0},
//                     {Type:"Text",    Width:50,  SaveName:"dataScal"     ,Align:"Center",   Edit:0},
                    {Type:"Combo",   Width:100,  SaveName:"pkYn"          , Align:"Center",   Edit:0},
                    {Type:"Text",    Width:100,  SaveName:"pkOrd"         , Align:"Center",   Edit:0},
                    {Type:"Combo",   Width:100,  SaveName:"nonulYn"       , Align:"Center",   Edit:0},
                    {Type:"Text",    Width:130,  SaveName:"defltVal"      , Align:"Center",   Edit:0},                    
...
function doActionCol(sAction)
{
    switch(sAction)
    {
        case "Search":	//요청서 재조회...
        	...
        	col_sheet.DoSearch("<c:url value="/meta/ldm/getLdmAttrList.do" />", param);
...
// 	$("#frmInput2").reset();
    ...
    $("form#colfrmSearch input[name=ldmAttrId]").val(col_sheet.GetCellValue(row, "ldmAttrId"));
...
function col_sheet_OnSearchEnd(code, message, stCode, stMsg) {
	...
	if(code < 0) {
		...
		return;
		 ...
		 $("form#colfrmSearch input[name=ldmAttrId]").val(col_sheet.GetCellValue(1, "ldmAttrId"));
...
<!--         <div class="stit">검색조건</div>  -->
        ...
        <input type="hidden" name="ldmAttrId" />
