...
// 		//imgConvert($('div.tab_navi a img'));
		...
		$("#btnRegRqst").click(function(){
			...
			var message = "";
			if (cntrqst > 0) message += "[<s:message code='LGC.MDEL'/> : "+cntrqst+"]"; //논리모델, 건
...
// 				var tmphtml = '테이블 : ' + param.ldmEntyLnm + ' [' + param.ldmEntyLnm +']';
...
//	 			$("#frmInput #rvwConts").focus();
			...
			if (cntappr > 0) message += "[<s:message code='LGC.MDEL'/>:"+cntappr+"<s:message code='CNT.APRV' />  "+cntAppr+"<s:message code='CNT.RTN' /> "+cntReject+"]<br>";
			// 물리모델, 건 중 승인, 건, 반려, 건
...
//         	openSearchPop("<c:url value='/meta/ldm/pop/pdmtblSearchPop.do' />"); 
...
//         create_selectbox(sysareaJson, $("#sysAreaId"));
      ...
      //신규요청서
        $("#btnBlank").click( function(){
    		location.href = '<c:url value="/meta/ldm/ldmenty_rqst.do" />';
        } );
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.PDMTBL.RQST.1'/>";
        	...
        	headtext += "<s:message code='META.HEADER.PDMTBL.RQST.5'/>";

//         headtext = "No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|주제영역|엔티티ID|엔티티명|주제영역ID|표준적용여부|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호";
        ...
        var cols = [                        
                    ...
                    {Type:"Text",     Width:250, SaveName:"fullPath",	 Align:"Left", Edit:0, KeyField:1}, 
                    {Type:"Text",     Width:80,  SaveName:"ldmEntyId",   Align:"Left",   Edit:0},
                    {Type:"Text",     Width:200, SaveName:"ldmEntyLnm",  Align:"Left",   Edit:0, KeyField:1},                     
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "AddWam": //기존 테이블 추가
			var url   = "<c:url value="/meta/ldm/popup/ldmEntySearch_pop.do"/>";
			var param = "popRqst=Y";
			
// 			var popup = OpenWindow(url+param,"pdmtblSearch","800","600","yes");
			openLayerPop(url, 800, 750, param);
			break;
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/ldm/delpdmtblrqstlist.do"/>";
...
// 			if (saveJson.Code == "IBS000") return;
			...
			var url = "<c:url value="/meta/ldm/regLdmEntyRqstList.do"/>";
...
//         	ibsSaveJson = grid_sheet.GetSaveJson(1);	//DoAllSave와 동일...
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/ldm/getLdmEntyRqstList.do" />", param);
			//컬럼 
			doActionCol("Search");
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/ldm/approveLdmEnty.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
        ...
        case "LoadExcel":  //엑셀업로드
			...
			var url = "<c:url value="/meta/ldm/popup/ldmentyattr_xls.do" />"; 
        ...
        case "SaveColRow":        //컬럼 저장...
			...
			var url = "<c:url value="/meta/ldm/regLdmAttrRqstList.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/ldm/delpdmcolrqstlist.do"/>";
			var param = $("#mstFrm").serialize();
			IBSpostJson2(url, DelJson, param, ibscallback);
        	break;
...
//			alert(insrow);
	...
	//변경일 경우 기존 순서와 틀릴경우 처리한다.	
		...
		if(insrow > 0) {
			preord = col_sheet.GetCellValue(insrow, "attrOrd");
		}
...
//			alert(insrow);
	...
	if(trow > 0) {
		...
		col_sheet.SetCellValue(insrow, "ldmEntyLnm", tbljson.ldmEntyLnm); 				
	}
	...
	for (var i=fcol; i<=lcol; i++) {
		...
		if (col_sheet.GetCellValue(i, "rqstSno") == tblsno) {
			if(col_sheet.GetCellValue(i, "rqstDcd") == 'CU') {
				col_sheet.SetCellValue(i, "attrOrd", cntcolord);
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/meta/ldm/ajaxgrid/ldmenty_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/ldm/ldmenty_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
//     tblClick(row);
	...
	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '<s:message code="ENTY.NM" /> : ' + param.ldmEntyLnm ; //테이블
	$('#tbl_sle_title').html(tmphtml);
	...
	col_sheet.DoSearch("<c:url value="/meta/ldm/getLdmAttrRqstList.do" />", param1);
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs" style="display: none;">
	  <ul>
	    <li><a href="#tabs-1"><s:message code="ENTY.DFNT.P" /></a></li><!-- 엔티티 정의서 -->
	    <li><a href="#tabs-2"><s:message code="ATTR.LST" /></a></li> <!-- 어트리뷰트 목록 -->
	    ...
	    <li id="tabs-rqstchg"><a href="#tabs-5"><s:message code="ENTY.CHG.ITEM" /></a></li> <!-- 엔티티변경항목 -->
	    <li id="tabs-rqstchg"><a href="#tabs-6" id="colinfo"><s:message code="ATTR.CHG.ITEM" /></a></li> <!-- 어트리뷰트변경항목 -->	    
	    </c:if>
	  </ul>
	  <div id="tabs-1">
		<!-- 	상세정보 ajax 로드시 이용 -->
