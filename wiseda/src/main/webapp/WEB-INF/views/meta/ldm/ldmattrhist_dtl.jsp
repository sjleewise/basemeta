...
//  		headtext  = "No.|등록유형|버전|컬럼ID|만료일시|시작일시|엔티티ID|엔티티명|표준용어ID|표준용어(논리명)|표준용어(물리명)|도메인ID|도메인(논리명)|도메인(물리명)|인포타입ID|인포타입명|어트리뷰트명|순서"
//  	    headtext += "|데이터타입|길이|소수점|PK여부|PK순서|NOTNULL여부|DEFAULT값";
//  		headtext += "|최초요청자|최초요청일시|요청자|요청일시|승인자|승인일시|설명";    
        ...
        var cols = [                        
                    ...
                    {Type:"Text",    Width:150,  SaveName:"ldmEntyLnm"    , Align:"Left",   Edit:0},
                    ...
                    {Type:"Text",    Width:150,  SaveName:"ldmAttrLnm"    , Align:"Left",   Edit:0, ColMerge:0},
                    {Type:"Text",    Width:50,   SaveName:"attrOrd"       , Align:"Center",   Edit:0, ColMerge:0},
...
function doActionColH(sAction)
{
    ...
    switch(sAction)
    {
        case "Search":	        	
        	//선택한 상세정보를 가져온다...
        	var param =  "ldmEntyId=" + grid_sheet.GetCellValue(grid_sheet.GetSelectRow(), "ldmEntyId");
        	
        	colh_sheet.DoSearch("<c:url value="/meta/ldm/getHistAttrList.do" />", param);

        	break;
        case "SearchOne":
        	var param =  "ldmAttrId=" + col_sheet.GetCellValue(col_sheet.GetSelectRow(), "ldmAttrId");
        	colh_sheet.DoSearch("<c:url value="/meta/ldm/gethisttblcollist.do" />", param);
        	break;
    }
}
