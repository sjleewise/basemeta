...
// 	//imgConvert($('div.tab_navi a img'));
	...
	$("#btnRegRqst").click(function(){
		...
		//alert("등록");
			...
			var message = "";
			if (cntrqst > 0) message += "[유사항목 등록 :"+cntrqst+"건]";
	...
	$("#btnReqApprove").click(function(){
		...
		if (tmprow > 0 ) {
			...
			var tmphtml = '표준단어 : ' + param.symnItemLnm + ' [' + param.symnItemPnm +']';
			$('#symnItem_sel_title').html(tmphtml);
		...
		if (cntappr > 0) message += "[유사항목 :"+cntappr+"건 중 승인 "+cntAppr+"건, 반려 "+cntReject+"건]<br>";
	...
	$("#btnChangAdd").click(function(){
		...
		doAction("AddWam");
// 		var popup = OpenWindow("<c:url value="/meta/symn/popup/stndword_pop.do"/>","stndwordSearch","800","600","yes");
// 		popup.focus();
...
// 	$("#btnReset").click( function(){ doAction("Reset"); } );
    ...
    $("#btnBlank").click( function(){
		parent.location.href = '<c:url value="/meta/symn/symnItem_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headers = [
						{Text:"<s:message code='META.HEADER.SYMNITEM.RQST'/>", Align:"Center"}
					];
			//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|유사항목명|유사항목영문명|참조항목논리명|참조항목물리명|참조항목ID|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
			...
			var cols = [						
						...
						{Type:"Text",   Width:100,  SaveName:"symnItemLnm",   	Align:"Left", Edit:0, KeyField:1},
						{Type:"Text",   Width:100,  SaveName:"symnItemPnm",   	Align:"Left", Edit:0, KeyField:1},
						...
						{Type:"Text",   Width:100,   SaveName:"sditmId", 	Align:"Left", Edit:0, Hidden:0},
...
// 			var popup = OpenWindow("<c:url value="/meta/symn/popup/stndword_pop.do"/>","stndwordSearch","800","600","yes");
			...
			var url   = "<c:url value="/meta/symn/popup/symnItem_pop.do"/>";
// 			var param = "?popRqst=Y";
...
// 	    	var tmpkey = getibscheckjoin(grid_sheet, "rqstSno");
	    	...
	    	var url = "<c:url value="/meta/symn/delSymnItemRqstList.do"/>";
...
// 			ibsSaveJson = $("#frmInput").serializeObject();
			...
			var url = "<c:url value="/meta/symn/regSymnItemRqstList.do"/>";
		...
		case "Search":
			console.log("search : "+selectedRow);
			//var param = "rqstNo="+$("#rqstNo", parent.document).val();
// 			alert($("#frmInput #bizDtlCd").val());
			var param = $("#mstFrm").serialize();
			console.log("param : " + param);
			grid_sheet.DoSearch("<c:url value="/meta/symn/getSymnItemRqstList.do" />", param);
...
// 			doAllApprove(grid_sheet, "1");
// 			var url = "<c:url value="/meta/symn/approveStndWord.do"/>";
			var url = "<c:url value="/meta/symn/approveSymnItem.do"/>";
			var param = $('form[name=mstFrm]').serialize();
...
// 			grid_sheet.LoadExcel();
			var url = "<c:url value="/meta/symn/popup/symnItem_xls.do" />";
...
function loadDetail(param, row) {
	console.log(param);
	console.log(row);
	// 	$('div#detailInfo').load('<c:url value="/meta/symn/ajaxgrid/stndword_rqst_dtl.do"/>', param, function( response, status, xhr ) {
// 		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/symn/symnItem_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
function grid_sheet_OnLoadExcel() {
	...
	var url = "<c:url value="/meta/symn/regstwdrqstlist.do"/>";
...
// 	};
	...
	//선택한 상세정보를 가져온다...
	var param =  grid_sheet.GetRowJson(row);
	console.log("Gridparam1 : " + param);
	selectedRow = grid_sheet.GetSelectRow();

	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '유사용어 : ' + param.symnItemLnm + ' [' + param.symnItemPnm +']';
	$('#symnItem_sel_title').html(tmphtml);
...
function saveEndReLoad(){
	var pk1 = parent.document.getElementById("symnItemLnm");
	//	var pk2 = document.getElementById("param.symnItemLnm");
	var sRow = grid_sheet.FindText("symnItemLnm",pk1.value);
	var param =  grid_sheet.GetRowJson(sRow);
	if(sRow>0){
		selectedRow = sRow;

		console.log("Gridparam2 : " + param);

		var param1 = $("#mstFrm").serialize();
		param1 += "&rqstSno=" + param.rqstSno;
		
		loadDetail(param, sRow);
...
function grid_sheet_OnSearchEnd(code, message, stCode, stMsg) {
	console.log("se1 : "+ code);
	console.log("se2 : "+ message);
	console.log("se3 : "+ stCode);
	console.log("se4 : "+ stMsg);
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title">유사용어등록</div>
...
<div class="selected_title_area">
	<div class="selected_title" id="symnItem_sel_title"></div>
</div>
...
<div id="tabs">
	<ul>
		<li><a href="#tabs-1">유사용어 상세정보</a></li>
...
<!-- 		<div id="detailInfo"></div> -->
		<%@include file="symnItem_rqst_dtl.jsp" %>
	</div>
	<c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S'}">
	<div id="tabs-2">
