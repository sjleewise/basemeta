...
// 		loadDetail();
		...
		setautoComplete($("#frmSearch #symnItemLnm"), "SYMN");
...
// 	alert('window.load');
	...
	if(!isBlankStr("${symnItemId}")) {
		var param = "symnItemId="+"${symnItemId}";
		grid_sheet.DoSearch("<c:url value="/meta/symn/getSymnItemList.do" />", param);
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        SetConfig(cfg);
        var headers = [
                    {Text:"<s:message code='META.HEADER.SYMNITEM.LIST'/>", Align:"Center"}
                ];
                //No.|상태|선택|표준단어ID|표준단어논리명|표준단어물리명|영문의미|한자명|출처구분|표준단어구분|도메인여부|요청번호|요청일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID
        		//No.|상태|선택|유사항목ID|유사항목논리명|유사항목물리명|참조항목ID|참조항목논리명|참조항목물리명|설명|요청번호|요청일련번호|버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   Width:100,  SaveName:"symnItemId",   	Align:"Left", Edit:0, Hidden:0},
                    {Type:"Text",   Width:100,  SaveName:"symnItemLnm",   	Align:"Left", Edit:0, Hidden:0},
					{Type:"Text",   Width:100,  SaveName:"symnItemPnm",   	Align:"Left", Edit:0, Hidden:0},
...
function loadDetail(param) {
	$('div#detailInfo1').load('<c:url value="/meta/symn/ajaxgrid/symnItemInfo_dtl.do"/>', param, function(){
		if(!isBlankStr(param)) {
			//그리드 초기화 한다.
			initDtlGrids();
			
			grid_sub_stwdchange.DoSearch('<c:url value="/meta/symn/ajaxgrid/symnItemChange_dtl.do" />', param);
			//grid_sub_stwdwhereused.DoSearch('<c:url value="/meta/symn/ajaxgrid/stwdwhereused_dtl.do" />', param);
		}
...
// 	if ('ibsSeq' == colsavename || 'ibsStatus' == colsavename || 'ibsCheck' == colsavename) return;
	...
	var symnItemId = "&symnItemId="+grid_sheet.GetCellValue(row, "symnItemId");

	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = "<s:message code='SYMN.LNM'/> : " + param.symnItemLnm;  /*표준단어논리명*/
	$('#program_sel_title').html(tmphtml);
	
	//메뉴ID를 토대로 조회한다....
	loadDetail(symnItemId);
        ...
        <form id="frmSearch" name="frmSearch" method="post">
            ...
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='STRD.WORD.INQ' />"> <!-- 표준단어조회 -->
                   ...
                   <tbody>                            
                            <tr>
                                <th scope="row"><label for="symnItemLnm"><s:message code="SYMN.NAME" /></label></th> <!-- 유사항목명 -->
                                <td><input type="text" id="symnItemLnm" name="symnItemLnm" class="wd200" /></td>
                                <th scope="row"><label for="engMean"><s:message code="SYMN.REF.NAME" /></label></th> <!-- 참조항목명 -->
...
<!-- 	    <li><a href="#tabs-2">컬럼 목록</a></li> -->
	  ...
	  <div id="tabs-2">
			<!-- 	상세정보 ajax 로드시 이용 -->
			<%@include file="symnItemChange_dtl.jsp" %>
