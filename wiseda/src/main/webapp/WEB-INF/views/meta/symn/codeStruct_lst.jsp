...
function initGrid()
{
    ...
    with(grid_sheet){
		...
		var headers = [
					...
					//NO.|SRC_ITEM_ID|도메인명|도메인영문명|코드개수(소스)|동일비율(소스)|동일코드수|동일비율(타겟)|TGT_ITEM_ID|도메인명|도메인영문명|코드개수(타겟)|SRC_SAME_ORDER|TGT_SAME_ORDER
...
function doAction(sAction)
{
	...
	case "Search":
    	...
    	grid_sheet.DoSearch("<c:url value="/meta/symn/getCodeStructuer.do" />", param);
		break;
	case "Down2Excel": //엑셀내려받기
		grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'코드유사구성분석'});
		break;
	}
...
function grid_sheet_OnClick(row, col, value, cellx, celly) {
		...
		var tmphtml = '<s:message code="DMN.NM" /> : ' + param.srcItemKnm + ' , <s:message code="TARG.DMN.NM"/> : '+param.tgtItemKnm;
		//도메인명, 타겟도메인명
		...
		//tabs에 데이터 상세 
		grid_sub.DoSearch('<c:url value="/meta/symn/ajaxgrid/getCodeStructuer_dtl.do"/>',param1);
...
<div id="tabs">
	<ul>
		<li><a href="#tabs-1"><s:message code="CD.SIMI.CMPS.DTL.INFO"/></a></li> <!-- 코드 유사구성 상세정보 -->
	</ul>
	<div id="tabs-1">
		<%@include file="codeinit_dtl.jsp" %>
