...
<html>
<head>
<title><s:message code="APP.WORD.REG.DEMD" /></title> <!-- APP단어등록요청 -->
...
// 	//imgConvert($('div.tab_navi a img'));
	...
	$("#btnReqApprove").click(function(){
		...
		if (tmprow > 0 ) {
			...
			var tmphtml = '<s:message code="STRD.WORD" /> : ' + param.appStwdLnm + ' [' + param.appStwdPnm +']'; //표준단어
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headers = [
						{Text:"<s:message code='META.HEADER.APPSTNDWORD.RQST.IFM'/>", Align:"Center"}
...
// 			var popup = OpenWindow("<c:url value="/meta/app/popup/stndword_pop.do"/>","stndwordSearch","800","600","yes");
			...
			var url   = "<c:url value="/meta/app/popup/stndword_pop.do"/>";
// 			var param = "?popRqst=Y";
...
// 			grid_sheet.LoadExcel();
			var url = "<c:url value="/meta/app/popup/stndword_xls.do" />";
...
// 			var url = '<c:url value="/meta/app/stndword_rqst.do" />';
// 			var param = $('form[name=mstFrm]').serialize();
...
// 	};
	...
	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '<s:message code="APP.WORD" /> : ' + param.appStwdLnm + ' [' + param.appStwdPnm +']'; //APP단어
	$('#symnItem_sel_title').html(tmphtml);
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title"><s:message code="APP.WORD.REG" /></div> <!-- APP단어등록 -->
...
<div id="tabs">
	<ul>
		<li><a href="#tabs-1"><s:message code="APP.WORD.DTL.INFO" /></a></li> <!-- APP단어 상세정보 -->
...
<form name="mstFrm" id="mstFrm" style="display:none;">
	...
	<input type="text" name="bizDtlCd" id="bizDtlCd" value="SYM" />
