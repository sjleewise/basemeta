...
<html>
<head>
<title>대외기관오류코드매핑 등록요청</title>
<script type="text/javascript">

$(document).ready(function() { 

	//업무구분상세 초기화...
	$("#mstFrm #bizDtlCd").val('OEM');
...
//         	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
    ...
    $("#btnBlank").click( function(){
    	location.href = '<c:url value="/meta/stnd/messagemap_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.MESSAGE.MAP.RQST'/>";
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|오류코드|전산메세지오류원인내용|고객메세지오류원인내용|기관사용불가여부|대표오류코드여부|도메인명|도메인물리명|유효값(코드값)|유효값(코드값)명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
		var headers = [
...
function doAction(sAction)
{
	...
	switch(sAction)
	{
		...
		case "AddWam": //기존  추가
			var url   = "<c:url value="/meta/stnd/popup/messagemap_pop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/delmsgmaprqstlist.do"/>";
...
// 			if(ibsSaveJson.data.length == 0) return;
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getmsgmaprqstlist.do" />", param);
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/approveMsgMap.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 			grid_sheet.LoadExcel({Mode:'HeaderMatch'});
			var url = "<c:url value="/meta/stnd/popup/messagemap_xls.do" />";
...
// 	alert( JSON.stringify(param));
	$('div#detailInfo').load('<c:url value="/meta/stnd/ajaxgrid/messagemap_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/stnd/messagemap_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
