...
//     	grid_sub_messagechange.Down2Excel({HiddenColumn:1});
    });
...
// 	grid_sub_messagechange.FitColWidth();
});


function initsubgrid_messagechange() {

    with(grid_sub_messagechange){
...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        SetMergeSheet(1);
        var headers = [
                    {Text:"<s:message code='META.HEADER.MESSAGE.STD.CHG.DTL'/>", Align:"Center"}
                ];
        //No.|메시지ID|만료일자|시작일자|AS-IS오류코드|오류전문언어구분코드|오류유형코드|메시지구분|표준오류코드여부|오류코드|전산메세지오류원인내용|고객메세지오류원인내용|오류코드적용일자|조치코드|담당파트|담당자명|요청번호|요청일련번호|설명|버전|등록유형|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID|비고
        ...
        var cols = [                        
					...
					{Type:"Combo",   Width:100,  SaveName:"errTlgLanDscd",  Align:"Left", Edit:0, KeyField:0},
					{Type:"Combo",   Width:100,  SaveName:"errTpCd",  Align:"Left", Edit:0, KeyField:0}, 
					{Type:"Combo",   Width:100,  SaveName:"msgDit",  Align:"Left", Edit:0, KeyField:0}, 
					{Type:"Combo",   Width:100,  SaveName:"stdErrCdYn",  Align:"Left", Edit:0, KeyField:0}, 
					...
					{Type:"Text",   Width:100,  SaveName:"tskDmn",	Align:"Left", 	 Edit:0, KeyField:0},
...
//         FitColWidth();
   	...
   	//==시트설정 후 아래에 와야함=== 
    init_sheet(grid_sub_messagechange);    
    //===========================
  	
    //저장 처리 과정을 디버깅 메시지를 팝업으로 표시 (-1)
//     grid_sub_messagechange.ShowDebugMsg(-1);	
...
y : y좌표
*/
function grid_sub_messagechange_OnDblClick(row, col, value, cellx, celly) {

	if(row < 1) return;
	
}

function grid_sub_messagechange_OnClick(row, col, value, cellx, celly) {
...
function grid_sub_messagechange_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<!-- 그리드 입력 입력 -->
	<div class="grid_01">
	     <script type="text/javascript">createIBSheet("grid_sub_messagechange", "100%", "150px");</script>
	</div>
	<!-- 그리드 입력 입력 End -->
