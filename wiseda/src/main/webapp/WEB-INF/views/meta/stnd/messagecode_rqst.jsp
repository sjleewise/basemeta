...
<html>
<head>
<title>조치코드 등록요청</title>
<script type="text/javascript">

$(document).ready(function() { 

	//업무구분상세 초기화...
	$("#mstFrm #bizDtlCd").val('MAC');
...
//         	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
    ...
    $("#btnBlank").click( function(){
    	location.href = '<c:url value="/meta/stnd/messagecode_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.MESSAGE.CODE.RQST'/>";
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|조치코드|언어구분코드|조치코드사용여부|조치코드삭제여부|조치코드적용일자|조치내용|요청일시|요청자ID|요청자명|요청번호|요청일련번호
		var headers = [
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/delmsgcoderqstlist.do"/>";
...
// 			if(ibsSaveJson.data.length == 0) return;
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getmsgcoderqstlist.do" />", param);
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/approveMsgCode.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 			grid_sheet.LoadExcel({Mode:'HeaderMatch'});
			var url = "<c:url value="/meta/stnd/popup/messagecode_xls.do" />";
...
// 	alert( JSON.stringify(param));
	$('div#detailInfo').load('<c:url value="/meta/stnd/ajaxgrid/messagecode_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/stnd/messagecode_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
