...
<html>
<head>
<title><s:message code="MSG.REG.DEMD" /></title> <!-- 메시지 등록요청 -->
<script type="text/javascript">

$(document).ready(function() {

	//업무구분상세 초기화...
	$("#mstFrm #bizDtlCd").val('MSG');
...
//         	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
    ...
    $("#btnBlank").click( function(){
    	location.href = '<c:url value="/meta/stnd/message_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.MESSAGE.RQST.1'/>";
		headtext += "|<s:message code='META.HEADER.MESSAGE.RQST.2'/>";
		headtext += "|<s:message code='META.HEADER.MESSAGE.RQST.3'/>";

		//headtext  = "No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과";
		//headtext += "|유형구분코드|업무구분코드|시스템구분코드|메시지코드|메시지내용|사용여부|설명";
		...
		var cols = [						
					...
					{Type:"Text",   Width:100,  SaveName:"sysDivCd",  Align:"Left", Edit:0, KeyField:0, Hidden:1},
					{Type:"Text",   Width:200,  SaveName:"msgCd",  Align:"Left", Edit:0, KeyField:1}, 
					{Type:"Text",   Width:200,   SaveName:"msgConts", 	  Align:"Left", Edit:0, KeyField:1},
					{Type:"Combo",  Width:130,  SaveName:"useYn",	  Align:"Center", Edit:0},								
...
function doAction(sAction)
{
	...
	switch(sAction)
	{
		...
		case "AddWam": //기존 표준용어 추가
			var url   = "<c:url value="/meta/stnd/popup/message_pop.do"/>";
			var param = "?popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/delmsgrqstlist.do"/>";
...
// 			IBSpostJson(url, param, ibscallback);
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getmsgrqstlist.do" />", param);
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/approveMsg.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
//	alert( JSON.stringify(param));
	$('div#detailInfo').load('<c:url value="/meta/stnd/ajaxgrid/message_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/stnd/message_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
<div id="tabs">
	<ul>
		<li><a href="#tabs-1"><s:message code="MSG.DTL" /></a></li> <!-- 메시지 상세 -->
