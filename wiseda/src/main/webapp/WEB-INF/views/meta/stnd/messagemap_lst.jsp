...
// 	alert('window.load');
	...
	if(!isBlankStr("${msgMapId}")) {
		var param = "msgMapId="+"${msgMapId}";
		grid_sheet.DoSearch("<c:url value="/meta/stnd/getMessageMapList.do" />", param);
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        SetConfig(cfg);
        var headers = [
                    {Text:"<s:message code='META.HEADER.MESSAGE.MAP.LST'/>", Align:"Center"}
                ];
        //No.|상태|선택|대외기관매핑ID|오류코드|전산메세지오류원인내용|고객메세지오류원인내용|기관사용불가여부|대표오류코드여부|도메인명|도메인물리명|유효값(코드값)|유효값(코드값)명|요청번호|요청일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID
...
function loadDetail(param) {
	$('div#detailInfo1').load('<c:url value="/meta/stnd/ajaxgrid/messagemap_dtl.do"/>', param, function(){
		if(!isBlankStr(param)) {
			//그리드 초기화 한다.
			//initDtlGrids();
			
			grid_sub_messagechange.DoSearch('<c:url value="/meta/stnd/ajaxgrid/messagemapchange_dtl.do" />', param);
		}
...
// 	if ('ibsSeq' == colsavename || 'ibsStatus' == colsavename || 'ibsCheck' == colsavename) return;
	...
	var msgMapId = "&msgMapId="+grid_sheet.GetCellValue(row, "msgMapId");
...
function grid_sheet_OnSearchEnd(code, message, stCode, stMsg) {
	...
	if(!isBlankStr("${msgMapId}")) {
		...
		var msgMapId = "";
		msgMapId = grid_sheet.GetCellValue(1, "msgMapId");
		param = "msgMapId="+msgMapId;
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="표준단어조회">
                   ...
                   <tbody>                            
						<tr>
							...
							<td ><input type="text" id="dmnLnm" name="dmnLnm" class="wd98p" /></td>
						</tr>
						<tr>
							<th scope="row" ><label for="isdErrCasCts">전산메시지오류원인내용</label></th>
							<td ><input type="text" id="isdErrCasCts" name="isdErrCasCts" class="wd98p" />
							<th scope="row" ><label for="cdVal">유효값(코드값)</label></th>
							<td ><input type="text" id="cdVal" name="cdVal" class="wd98p" /></td>
						</tr>
						<tr>
							<th scope="row" ><label for="osdErrCasCts">고객메시지오류원인내용</label></th>
							<td colspan="3"><input type="text" id="osdErrCasCts" name="osdErrCasCts" class="wd98p" /></td>
						</tr>
                   </tbody>
                 </table>   
            </div>
...
<!-- 	    <li><a href="#tabs-2">컬럼 목록</a></li> -->
	  ...
	  <div id="tabs-2">
			<!-- 	상세정보 ajax 로드시 이용 -->
			<%@include file="messagemapchange_dtl.jsp" %>
