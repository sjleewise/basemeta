...
function initGrid()
{
    ...
    with(grid_sheet){
		...
		var headers = [
						{Text:"<s:message code='META.HEADER.CODEDMN.LST'/>", Align:"Center"}
					];
					//No.|표준분류|도메인ID|도메인논리명|도메인물리명|논리명기준구분|도메인그룹명|인포타입|데이터타입|데이터갈이|코드값|코드값명|상위도메인ID|주제영역ID|목록엔티티ID|목록엔티티물리명|목록엔티티논리명|목록어트리뷰트ID|목록어트리뷰트물리명|목록어트리뷰트논리명|코드값유형코드|코드값부여방식코드|표준용어자동생성여부|데이터형식|담당사용자ID|도메인출처구분|요청번호|요청일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID
			...
			var cols = [
						...
						{Type:"Combo",   Width:130,   SaveName:"dmngId",	 	Align:"Center", Edit:0, Hidden:0},
						...
						{Type:"Text",   Width:100,   SaveName:"cdId",	 	Align:"Left", Edit:0, Hidden:0},
...
function doAction(sAction)
{
    ...
    {
        case "Search":
//         	if (isBlankStr($("#frmSearch #dmnLnm").val(), 'O') ) {
//                	showMsgBox("INF", "조회할 도메인명을 입력 하십시오.");
//                	return;
//             }
...
// 			grid_sub_dmnvalue.RemoveAll();
			...
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getDomainlistWithCdVal.do" />", param);
        ...
        case "Down2Excel":  //엑셀내려받기
            ...
            grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'코드도메인조회'});
...
<%-- 							 	</select> --%>
						...
						<th scope="row"><label for="cdId"><s:message code="CD.ID" /></label></th> <!-- 코드ID -->
							<td><input type="text" id="cdId" name="cdId" class="wd200"/></td>
