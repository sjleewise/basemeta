...
// 	initSearchButton();	//검색 버튼 초기화
    ...
    $("li#btnNew a").html('<span class="ui-icon ui-icon-folder-open"></span>코드이관요청');
    $("li#btnChangAdd a").html('<span class="ui-icon ui-icon-folder-open"></span>이관삭제요청');
...
// 			$("#frmInput #rvwConts").focus();
    ...
    $("#btnBlank").click( function(){
    	location.href = '<c:url value="/meta/stnd/codetsf_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "No.|<s:message code='META.HEADER.CODETSF_RQST.1'/>";
		headtext += "|<s:message code='META.HEADER.CODETSF_RQST.2'/>";
		headtext += "|<s:message code='META.HEADER.CODETSF_RQST.3'/>";
		headtext += "|<s:message code='META.HEADER.CODETSF_RQST.4'/>";
		headtext += "|<s:message code='META.HEADER.CODETSF_RQST.5'/>";
		headtext += "|<s:message code='META.HEADER.CODETSF_RQST.6'/>";
...
function doAction(sAction)
{
	...
	switch(sAction)
	{
		...
		case "Add": //코드이관 요청
			var url   = "<c:url value="/meta/stnd/popup/codetsf_pop.do"/>";
			var param = "?popRqst=Y";

		    var popup =	OpenWindow(url+param,"codetsfSearch","800","600","yes");
		    //openLayerPop(url+param,800,600,param);
			break;
		case "AddWam": //이관 삭제요청
			var url   = "<c:url value="/meta/stnd/popup/codetsfwam_pop.do"/>";
			var param = "?popRqst=Y";
			var popup = OpenWindow(url+param,"codetsfwamSearch","800","600","yes");
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/delcodetsfrqstlist.do"/>";
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getcodetsfrqstlist.do" />", param);
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/approvecodetsf.do"/>";
...
function loadDetail(param) {
	...
	$('div#detailInfo').load('<c:url value="/meta/stnd/ajaxgrid/codetsf_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/stnd/codetsf_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title"><s:message code="CD.REG.1"/></div><!-- 코드등록 -->
