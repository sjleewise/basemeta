...
// 	$("#mstFrm #bizDtlCd").val('SDITM');
	...
	//요청마스터 요청진행상태가 변경되면, 부모 페이지의 요청마스터도 업데이트 한다.
// 	$("#mstFrm #rqstStepCd").change(function(){
...
// 		doAllApprove(cdval_sheet, "2");
	...
	$("#btnReqApprove").click(function(){
		...
		if (tmprow > 0 ) {
			...
			var tmphtml = '<s:message code="STRD.WORD" /> : ' + param.sditmLnm + ' [' + param.sditmPnm +']';/*표준단어*/
			//$('#stnditem_sel_title').html(tmphtml);
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		if ("${govYn}" == "Y") {
        	headtext += "<s:message code='META.HEADER.STNDITEM.RQST.IFM.4'/>"; //범정부 중앙메타 연계항목
        }

		//headtext  = "No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|";
		//headtext += "표준분류|표준용어논리명|표준용어물리명|논리명기준구분|물리명기준구분|도메인논리명|도메인물리명|도메인그룹|인포타입ID|인포타입논리명|데이터타입|길이|소수점|암호화여부|중복여부|고객정보변환여부|";
		//headtext += "설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호";
		//headtext += "|공개/비공개 여부|비공개사유|개인정보여부|제약조건";
		...
		var cols = [						
					...
					{Type:"Text",   Width:150,  SaveName:"sditmLnm",      Align:"Left", Edit:0, KeyField:1},
					{Type:"Text",   Width:150,  SaveName:"sditmPnm",      Align:"Left", Edit:0, KeyField:1}, 
					...
					{Type:"text",   Width:60,   SaveName:"infotpId",      Align:"Left", Edit:0, Hidden:0},
					{Type:"Text",   Width:90,   SaveName:"infotpLnm",     Align:"Left", Edit:0, Hidden:0},
					...
					{Type:"Combo",  Width:130,  SaveName:"dupYn",	      Align:"Center", Edit:0, Hidden:1},
					{Type:"Combo",  Width:130,  SaveName:"persInfoCnvYn", Align:"Center", Edit:0},
					
					{Type:"Text",   Width:120,  SaveName:"objDescn",	  Align:"Left", 	 Edit:0, KeyField:1},
		...
		if ("${govYn}" == "Y") {
			cols.push(
					...
					{Type:"Text",   Width:100,  SaveName:"constCnd"  ,		Align:"Left",   Edit:0}
			);
		}
...
// 		SetColProperty("dmngId", 	${codeMap.dmngibs});
		...
		if ("${govYn}" == "Y") {
			//범정부연계항목 콤보 목록 설정
			SetColProperty("openYn",		{ComboCode:"N|Y", ComboText:"<s:message code='COMBO.NO.YES'/>"}); /* 아니요|예 */
			SetColProperty("priRsn",		${codeMap.nopenRsnCdibs});
			SetColProperty("prsnInfoYn",	{ComboCode:"N|Y", ComboText:"<s:message code='COMBO.NO.YES'/>"}); /* 아니요|예 */
...
// 			IBSpostJson(url, param, ibscallback);
		...
		case "Search":
// 			alert("search start");
// 			parent.$("#clickBtnNm").val("");
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getitemrqstlist.do" />", param);
...
// 			var url = "<c:url value="/meta/stnd/approveStndItem.do"/>";
			var url = "<c:url value="/meta/stnd/regapproveStndItem.do"/>";
			var param = $('form[name=mstFrm]').serialize();
		...
		//보여지는 컬럼들만 엑셀 다운로드          
	      var downColNms = "";
			...
			grid_sheet.Down2Excel({HiddenColumn:1,DownCols:downColNms, Merge:1, FileName:'표준용어등록요청'});
...
// 	$('div#detailInfo').load('<c:url value="/meta/stnd/ajaxgrid/stnditem_rqst_dtl.do"/>', param, function( response, status, xhr ) {
// 		  if ( status == "error" ) {
...
// 	});
	...
	//alert(grid_sheet.GetCellText(row,"infotpLnm"));
	//도메인그룹/인포타입 셋팅

	setDomainInfo2(grid_sheet.GetCellText(row,"infotpLnm"), grid_sheet.GetCellValue(row,"encYn"), grid_sheet.GetCellValue(row,"dupYn"));
...
// 	$("#dmngId").val(grid_sheet.GetCellValue(row,"dmngLnm"));
// 	set_infoType_select($("#dmngId").val());
// 	ibs2formmapping(row, $("form#frmInput"), grid_sheet);
// 	$("#dmngId").val(grid_sheet.GetCellValue(row,"dmngLnm"));
// 	$("#dmngLnm").val(grid_sheet.GetCellText(row,"dmngLnm"));
// 	$("#infotpLnm").val(grid_sheet.GetCellValue(row,"infotpId"));
	
// 	$("#divTitle").text("표준용어 : " + grid_sheet.GetCellValue(row,"sditmLnm"));

// 	$("#sditmStwdPop").hide();
	
// 	$("#sditmLnm").focus().click();
}
...
function grid_sheet_OnSearchEnd(code, message, stCode, stMsg) {
	...
	if(code < 0) {
		...
		parent.inititemifm = true;	
		if(grid_sheet.SearchRows() > 0){
// 			parent.$("#clickBtnNm").val("");
// 			parent.$("#clickBtnNm").val("ITM");
// 			parent.$("#stnditemradio").click();
		}
		$('#btnReset').click();
	    $("#rqstDcd").val("CU");
	}
...
<div id="tabs">
	<ul>
	<li><a href="#tabs-1"><s:message code="STRD.TERMS.DTL.INFO" /></a></li> <!-- 표준용어 상세정보 -->
...
<!-- 	<div id="detailInfo"></div> -->
		<%@include file="stnditem_rqst_dtl.jsp" %>
	</div>
	<c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S'}">
	<div id="tabs-2">
...
<form name="mstFrm" id="mstFrm" style="display:none;">
	...
	<input type="text" name="bizDtlCd" id="bizDtlCd" value="SDITM" />
