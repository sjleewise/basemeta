...
// 	alert('window.load');
	...
	//	grid_sheet.DoSearch("<c:url value="/meta/stnd/getMessageTsfList.do" />", param);
	//	
//	}
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        SetConfig(cfg);
        var headers = [
                    {Text:"<s:message code='META.HEADER.MESSAGETSF.LST'/>", Align:"Center"}
                ];
                //No.|상태|선택|메시지ID|타겟DB_ID|타겟DB명|타겟스키마ID|타겟스키마명|유형구분코드|업무구분코드|메시지코드|메시지내용|사용여부|요청번호|요청일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID
        ...
        var cols = [                        
                    ...
                    {Type:"Text",   Width:40,  SaveName:"msgId",    Align:"Left", Edit:0, Hidden:1},
                    ...
                    {Type:"Combo",   Width:80,  SaveName:"typDivCd",    Align:"Center", Edit:0, Hidden:0}, 
                    {Type:"Combo",   Width:80,  SaveName:"bizDivCd",    Align:"Center", Edit:0, Hidden:0}, 
                    {Type:"Text",   Width:80,  SaveName:"msgCd",    Align:"Left", Edit:0, Hidden:0}, 
                    {Type:"Text",   Width:300,  SaveName:"msgConts",    Align:"Left", Edit:0},  
                    {Type:"Combo",   Width:80,  SaveName:"useYn",    Align:"Center", Edit:0, Hidden:0},
...
function loadDetail(param) {
	$('div#detailInfo1').load('<c:url value="/meta/stnd/ajaxgrid/messagetsf_dtl.do"/>', param, function(){
		if(!isBlankStr(param)) {
			//그리드 초기화 한다.
			//initDtlGrids();
			
			grid_sub_messagechange.DoSearch('<c:url value="/meta/stnd/ajaxgrid/messagetsfchange_dtl.do" />', param);
...
// 	if ('ibsSeq' == colsavename || 'ibsStatus' == colsavename || 'ibsCheck' == colsavename) return;
	...
	var tmphtml = '<s:message code="MSG.CD.NM" /> : ' + param.msgCd; /*메시지코드명*/
	$('#program_sel_title').html(tmphtml);
	
	var tmpParam = "msgId="+param.msgId+"&tgtDbConnTrgId="+param.tgtDbConnTrgId+"&tgtDbSchId="+param.tgtDbSchId;
	//메뉴ID를 토대로 조회한다....
	//alert(grid_sheet.RowSaveStr(row));
...
//		$('#program_sel_title').html(tmphtml);
	...
	//	var msgId = "";
		//msgId = grid_sheet.GetCellValue(1, "msgId");
		//param = "msgId="+msgId;
        ...
        <form id="frmSearch" name="frmSearch" method="post">
            ...
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='STRD.WORD.INQ' />"> <!-- 표준단어조회 -->
                   <caption><s:message code="MSG.INQ.FORM" /></caption> <!-- 메시지 검색폼 -->
                   <colgroup>
                   <col style="width:15%;" />
                   <col style="width:35%;" />
                   <col style="width:15%;" />
                   ...
                   <tbody>      
                            ...
                            <tr>
                                ...
                                <td><input type="text" id="msgCd" name="msgCd" class="wd95p"/></td>
                                <th scope="row"><label for="msgConts"><s:message code="MSG.CNTN" /></label></th> <!-- 메시지내용 -->
                                <td><input type="text" id="msgConts" name="msgConts" class="wd95p"/></td>
                            </tr>
<!--   <tr>
...
<div style="clear:both; height:10px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1"><s:message code="MSG.CD.INFO" /></a></li> <!-- 메시지코드 정보 -->
...
<!-- 	    <li><a href="#tabs-2">컬럼 목록</a></li> -->
	  ...
	  <div id="tabs-2">
			<!-- 	상세정보 ajax 로드시 이용 -->
			<%@include file="messagechange_dtl.jsp" %>
