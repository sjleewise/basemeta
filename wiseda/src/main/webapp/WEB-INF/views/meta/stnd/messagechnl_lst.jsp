...
// 	alert('window.load');
	...
	if(!isBlankStr("${chnlMsgId}")) {
		var param = "chnlMsgId="+"${chnlMsgId}";
		grid_sheet.DoSearch("<c:url value="/meta/stnd/getMessageChnlList.do" />", param);
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        SetConfig(cfg);
        var headers = [
                    {Text:"<s:message code='META.HEADER.MESSAGE.CHNL.LST'/>", Align:"Center"}
                ];
        //No.|상태|선택|채널오류코드ID|표준오류코드ID|오류코드|채널오류전문언어구분코드|오류유형코드|채널오류메세지내용|채널오류코드|오류코드사용여부|채널유형코드|채널세부코드|오류코드적용일자|조치코드|요청번호|요청일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID
...
function loadDetail(param) {
	$('div#detailInfo1').load('<c:url value="/meta/stnd/ajaxgrid/messagechnl_dtl.do"/>', param, function(){
		if(!isBlankStr(param)) {
			//그리드 초기화 한다.
			//initDtlGrids();
			
			grid_sub_messagechange.DoSearch('<c:url value="/meta/stnd/ajaxgrid/messagechnlchange_dtl.do" />', param);
		}
...
// 	if ('ibsSeq' == colsavename || 'ibsStatus' == colsavename || 'ibsCheck' == colsavename) return;
	...
	var chnlMsgId = "&chnlMsgId="+grid_sheet.GetCellValue(row, "chnlMsgId");
...
function grid_sheet_OnSearchEnd(code, message, stCode, stMsg) {
	...
	if(!isBlankStr("${chnlMsgId}")) {
		...
		var chnlMsgId = "";
		chnlMsgId = grid_sheet.GetCellValue(1, "chnlMsgId");
		param = "chnlMsgId="+chnlMsgId;
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="표준단어조회">
                   ...
                   <tbody>                            
						...
						<tr>
						    ...
						    <td>
						    <select id="chnlTpCd" name="chnlTpCd">
								<option value="">---전체---</option>
						...
						<tr>
						    <th scope="row"><label for="chnlErrMsgCts">메시지내용</label></th>
						    <td colspan="3"><input type="text" id="chnlErrMsgCts" name="chnlErrMsgCts" class="wd95p"/></td>
						</tr>
                   </tbody>
                 </table>   
            </div>
...
<!-- 	    <li><a href="#tabs-2">컬럼 목록</a></li> -->
	  ...
	  <div id="tabs-2">
			<!-- 	상세정보 ajax 로드시 이용 -->
			<%@include file="messagechnlchange_dtl.jsp" %>
