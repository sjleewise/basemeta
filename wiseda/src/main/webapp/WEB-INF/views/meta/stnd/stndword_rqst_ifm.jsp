...
<html>
<head>
<title><s:message code="STRD.WORD.REG.DEMD"/></title> <!-- 표준단어등록요청 -->
...
// 	$("#btnReset").click( function(){ doAction("Reset"); } );
	...
	//유사어 금칙어 조회 버튼 추가
	$("#btnExcelDown").before('<button class="btn_search"  id="btnSymnPop"  name="btnSymnPop"><s:message code="STND.SYMN.SEARCH" /></button>')
	
	$('#btnSymnPop').click(function(){
// 		alert("유사어/금지어 조회 팝업 작업중");
		openLayerPop2("<c:url value="/meta/stnd/popup/symn_lst.do"/>","1100","600","");
	});
	}
);
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headers = [
						{Text:"<s:message code='META.HEADER.STNDWORD.RQST.IFM'/>", Align:"Center"}
					];
					//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|표준분류|표준단어논리명|표준단어물리명|영문의미|한자|출처구분|표준단어구분|도메인여부|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
			...
			var cols = [						
						...
						{Type:"Text",   Width:150,  SaveName:"stwdLnm",   	Align:"Left", Edit:0, KeyField:1},
						{Type:"Text",   Width:150,  SaveName:"stwdPnm",   	Align:"Left", Edit:0, KeyField:1}, 
						{Type:"Text",   Width:200,   SaveName:"engMean", 	Align:"Left", Edit:0, Hidden:0},
						{Type:"Text",   Width:100,   SaveName:"cchNm",	 	Align:"Left", Edit:0, Hidden:0},
						{Type:"Text",   Width:60,   SaveName:"orgDs",	 	Align:"Left", Edit:0, Hidden:0},
						{Type:"Combo",   Width:60,   SaveName:"wdDcd",	 	Align:"Center", Edit:0, Hidden:1},
						{Type:"Combo",   Width:60,   SaveName:"dmnYn",	 	Align:"Center", Edit:0, Hidden:1},
						{Type:"Text",   Width:350,  SaveName:"objDescn",	Align:"Left", 	 Edit:0, KeyField:1},
...
// 			loadDetail();
	    	...
	    	//신규 추가시 변경 가능하도록 readonly 해제
            formElementChgReadOnly("stndAsrt|stwdLnm|stwdPnm",false);
...
// 			var popup = OpenWindow("<c:url value="/meta/stnd/popup/stndword_pop.do"/>","stndwordSearch","800","600","yes");
			...
			var url   = "<c:url value="/meta/stnd/popup/stndword_pop.do"/>";
// 			var param = "?popRqst=Y";
...
// 			var url = "<c:url value="/meta/stnd/approveStndWord.do"/>";
			var url = "<c:url value="/meta/stnd/regapproveStndWord.do"/>";
			var param = $('form[name=mstFrm]').serialize();
		...
		//보여지는 컬럼들만 엑셀 다운로드          
	      var downColNms = "";
			...
			grid_sheet.Down2Excel({HiddenColumn:1,DownCols:downColNms, Merge:1, FileName:'표준단어등록요청'});
			
			break;
		case "LoadExcel":  //엑셀업로드 팝업 호출
// 			grid_sheet.LoadExcel();
			var url = "<c:url value="/meta/stnd/popup/stndword_xls.do" />";
...
// 	$('div#detailInfo').load('<c:url value="/meta/stnd/ajaxgrid/stndword_rqst_dtl.do"/>', param, function( response, status, xhr ) {
// 		  if ( status == "error" ) {
...
// 	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
				doAction("Search");
				$('#btnReset').click();
				if(parent.initdmnifm)
				$('#stnddmn_ifm', parent.document)[0].contentWindow.doAction("Search");
...
function grid_sheet_OnLoadExcel() {
	...
	var url = "<c:url value="/meta/stnd/regstwdrqstlist.do"/>";
...
// 	};
	...
	if(param.regTypCd == 'U'||param.vrfCd == '5') {
		getRqstChg(param1);
		//변경일 경우 표준구분 논리명, 물리명을 변경 못하도록 
		
// 		$("#stndAsrt > option").prop("disabled", true);
// 		$("#stwdLnm").attr("readonly",true);
// 		$("#stwdPnm").attr("readonly",true);
		formElementChgReadOnly("stndAsrt|stwdLnm|stwdPnm",true);
...
// 		$("#stwdPnm").attr("readonly",true);
	...
	else if(param.regTypCd == 'C'){
    	//신규 추가시 변경 가능하도록 
    	formElementChgReadOnly("stndAsrt|stwdLnm|stwdPnm",false);
// 		$("#stndAsrt > option").prop("disabled", false);
// 		$("#stwdLnm").attr("readonly",false);
// 		$("#stwdPnm").attr("readonly",false);
	}
...
// 	$("#divTitle").text("표준단어 : " + grid_sheet.GetCellValue(row,"stwdLnm"));
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title"><s:message code="STRD.WORD.REG"/></div> <!-- 표준단어등록 -->
...
<div id="tabs">
	<ul>
		<li><a href="#tabs-1"><s:message code="STRD.WORD.DTL.INFO"/></a></li>  <!-- 표준단어 상세정보 -->
...
<!-- 		<div id="detailInfo"></div> -->
		<%@include file="stndword_rqst_dtl.jsp" %>
	</div>
	<c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S'}">
	<div id="tabs-2">
...
<form name="mstFrm" id="mstFrm" style="display:none;">
	...
	<input type="text" name="bizDtlCd" id="bizDtlCd" value="STWD" />
