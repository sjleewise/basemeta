...
<html>
<head>
<title>표준오류코드 등록요청</title>
<script type="text/javascript">

$(document).ready(function() { 

	//업무구분상세 초기화...
	$("#mstFrm #bizDtlCd").val('SEM');
...
//         	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
    ...
    $("#btnBlank").click( function(){
    	location.href = '<c:url value="/meta/stnd/messagestd_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.MESSAGE.STD.RQST'/>";
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|AS-IS오류코드|오류전문언어구분코드|오류유형코드|메시지유형|메시지구분|업무구분|표준오류코드여부|오류코드|전산메세지오류원인내용|고객메세지오류원인내용|오류코드적용일자|조치코드|담당파트|담당자명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
		var headers = [
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/delmsgstdrqstlist.do"/>";
...
// 			if(ibsSaveJson.data.length == 0) return;
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getmsgstdrqstlist.do" />", param);
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/approveMsgStd.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 			grid_sheet.LoadExcel({Mode:'HeaderMatch'});
			var url = "<c:url value="/meta/stnd/popup/messagestd_xls.do" />";
...
// 	alert( JSON.stringify(param));
	$('div#detailInfo').load('<c:url value="/meta/stnd/ajaxgrid/messagestd_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/stnd/messagestd_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
function doSave()
{
	...
	if($("#frmInput #regTypCd").val() != 'D' && $("#frmInput #errCd").val() == ""){
		errCdstr = $("#frmInput #msgTyp").val() + $("#frmInput #msgDit").val() + $("#frmInput #bizDit").val() + "0001";
		$("#frmInput #errCd").val(errCdstr);
	}else if($("#frmInput #regTypCd").val() == 'D'){
	}else{
		$("#frmInput #regTypCd").val("U");
	}
	if($("#frmInput #regTypCd").val() != 'D' && $("#frmInput #msgTyp").val() == "D"  && $("#frmInput #bizDit").val() != "COR")
	{ 	
		alert("화면메시지 인경우 업무구분을 'COR'만 등록가능합니다."); 
		return;
	} 
	if($("#frmInput #errCd").val() == "" && $("#frmInput #regTypCd").val() !="C"){
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title">메시지등록</div>
