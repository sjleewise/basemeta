...
// 		doAllApprove(cdval_sheet, "2");
	...
	$("#btnReqApprove").click(function(){
		...
		if (tmprow > 0 ) {
			...
			var tmphtml = '표준단어 : ' + param.sditmLnm + ' [' + param.sditmPnm +']';
...
$(window).on('load',function() {
	...
	//$('#stnditem_ifm')[0].contentWindow.$("#btnRegRqst").hide();  		
	//$('#stnddmn_ifm')[0].contentWindow.$("#btnRegRqst").hide();
	//$('#stndword_ifm')[0].contentWindow.$("#btnRegRqst").hide();
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.NOT.STNDITEM.RQST'/>";
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|비표준용어아이디|비표준용어논리명|비표준용어물리명|논리명기준구분|물리명기준구분|도메인논리명|도메인물리명|도메인그룹|인포타입ID|인포타입논리명|데이터타입|길이|소수점|기준정보|암호화여부|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호
		var headers = [
		...
		var cols = [						
					...
					{Type:"Combo",  Width:130,  SaveName:"stndInfo",	  Align:"Center", Edit:0},						
...
// 		SetColProperty("dmngId", 	${codeMap.dmngibs});
		...
		SetColProperty("stndInfo", 	{ComboCode:"N|Y", ComboText:"아니요|예"});
...
// 	});
	...
	setDomainInfo2(grid_sheet.GetCellText(row,"infotpLnm"));
	
	param +="&sditmId="+grid_sheet.GetCellValue(row, "sditmId");
...
function grid_sheet_OnClick(row, col, value, cellx, celly) {
	...
	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '비표준용어 : ' + param.sditmLnm + ' [' + param.sditmPnm +']';
	//$('#stnditem_sel_title').html(tmphtml);
...
// 	$("#divTitle").text("비표준용어 : " + grid_sheet.GetCellValue(row,"sditmLnm"));
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title">비표준용어등록</div>
...
<div id="tabs">
	<ul>
		<li><a href="#tabs-1">비표준용어 상세정보</a></li>
		...
		<li><a href="#tabs-3">변경항목</a></li>   
	
		<li><a href="#tabs-4">Where Used</a></li>
	</ul>
	
	<div id="tabs-1">
	   <!--  상세정보 ajax 로드시 이용 -->
 	   <!-- <div id="detailInfo"></div> -->
		<%@include file="notstnditem_rqst_dtl.jsp" %>
	</div>
