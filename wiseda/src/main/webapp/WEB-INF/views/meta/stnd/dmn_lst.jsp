...
//         	$("#"+firstSelectBoxId[0] +" option:eq(0)").attr("selected", "selected");
		...
		$("#tabs").click(function(){
			...
			if(tabidx == 2){
				$("#btnExcelDownSub").show();

				$("#btnExcelDownSub").click(function(){

				   //=======보여지는 컬럼들만 엑셀 다운로드===========          
			       var downColNms = "";

			       for(var i = 0; i <= grid_sub_dmnvalue.LastCol(); i++ ){
				       
			     		if(grid_sub_dmnvalue.GetColHidden(i) != 1){
				     		
			     			downColNms += grid_sub_dmnvalue.ColSaveName(0,i)+ "|";
			     		}
			       }
			       //=======================================
		          
			       grid_sub_dmnvalue.Down2Excel({HiddenColumn:1,DownCols:downColNms, Merge:1, FileName:'유효값조회'});
...
function initGrid()
{
    ...
    with(grid_sheet){
		...
		var headers = [
					...
					//목록어트리뷰트논리명|코드값유형코드|코드값부여방식코드|부모도메인명|표준용어자동생성여부|암호화여부|데이터형식|담당사용자ID|도메인출처구분
			...
			var cols = [
						...
						{Type:"Combo",   Width:130,   SaveName:"infotpId",	 	Align:"Left", Edit:0, Hidden:0},
						...
						{Type:"Text",   Width:80,   SaveName:"cdId",	 	Align:"Left", Edit:0, Hidden:0},
...
// 						{Type:"Combo",  Width:80,   SaveName:"dupYn",   	Align:"Center", Edit:0},
						...
						{Type:"Text",   Width:100,   SaveName:"dmnEngMean",	 	Align:"Left", Edit:0, Hidden:0},
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "Search":
			...
			grid_sub_dmninit.RemoveAll();
			grid_sub_dmnvalue.RemoveAll();
			grid_sub_dmnwhereused.RemoveAll();
			grid_sub_dmnvaluechange.RemoveAll();
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getDomainlist.do" />", param);
        ...
        case "Down2Excel":  //엑셀내려받기
            ...
            grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'도메인조회'});
...
function loadDetail(param) {
	$('div#detailInfo1').load('<c:url value="/meta/stnd/ajaxgrid/dmninfo_dtl.do"/>', param, function(){
		...
		if(!isBlankStr(param)) {
			initDtlGrids();
			grid_sub_dmnchange.DoSearch('<c:url value="/meta/stnd/ajaxgrid/dmnchange_dtl.do" />', param);
			grid_sub_dmninit.DoSearch('<c:url value="/meta/stnd/ajaxgrid/dmninit_dtl.do" />', param);
			...
			grid_sub_diccomparisonDmn.DoSearch('<c:url value="/meta/stnd/ajaxgrid/dmncomparison_dtl.do" />', param);
		}
...
<%-- 							 	</select> --%>
							...
							<th scope="row"><label for="cdValTypCd"><s:message code="CD.VAL.PTRN" /></label></th> <!-- 코드값유형 -->
						...
						<tr>
							...
							<th scope="row"><label for="cdId"><s:message code="CD.ID" /></label></th> <!-- 코드ID -->
							<td><input type="text" id="cdId" name="cdId" class="wd95p"/></td>
						</tr>
                   </tbody>
                 </table>   
            </div>
