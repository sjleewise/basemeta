...
// 	alert('window.load');
	...
	if(!isBlankStr("${msgId}")) {
		...
		grid_sheet.DoSearch("<c:url value="/meta/stnd/getMessageList.do" />", param);
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        SetConfig(cfg);
        var headers = [
                    {Text:"<s:message code='META.HEADER.MESSAGE.LST'/>", Align:"Center"}
                ];
                //No.|상태|선택|메시지ID|유형구분코드|업무구분코드|메시지코드|메시지내용|사용여부|요청번호|요청일련번호|설명|버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID
...
function loadDetail(param) {
	$('div#detailInfo1').load('<c:url value="/meta/stnd/ajaxgrid/message_dtl.do"/>', param, function(){
		if(!isBlankStr(param)) {
			//그리드 초기화 한다.
			//initDtlGrids();
			
			grid_sub_messagechange.DoSearch('<c:url value="/meta/stnd/ajaxgrid/messagechange_dtl.do" />', param);
...
function grid_sheet_OnSearchEnd(code, message, stCode, stMsg) {
	...
	if(!isBlankStr("${msgId}")) {
		...
		//선택한 그리드의 row 내용을 보여준다.....
		var tmphtml = '<s:message code="MSG.CD" /> : ' + param.msgCd; /*메시지코드*/
		$('#program_sel_title').html(tmphtml);
