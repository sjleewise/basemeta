...
<html>
<head>
<title>채널오류코드 등록요청</title>
<script type="text/javascript">

$(document).ready(function() { 

	//업무구분상세 초기화...
	$("#mstFrm #bizDtlCd").val('CEM');
...
//         	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
    ...
    $("#btnBlank").click( function(){
    	location.href = '<c:url value="/meta/stnd/messagechnl_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.MESSAGE.CHNL.RQST'/>";
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|오류코드|채널오류전문언어구분코드|오류유형코드|채널오류메세지내용|채널오류코드|오류코드사용여부|채널유형코드|채널세부코드|오류코드적용일자|조치코드|요청일시|요청자ID|요청자명|요청번호|요청일련번호
		var headers = [
...
function doAction(sAction)
{
	...
	switch(sAction)
	{
		...
		case "AddWam": //기존  추가
			var url   = "<c:url value="/meta/stnd/popup/messagechnl_pop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/delmsgchnlrqstlist.do"/>";
...
// 			if(ibsSaveJson.data.length == 0) return;
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getmsgchnlrqstlist.do" />", param);
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/stnd/approveMsgChnl.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 			grid_sheet.LoadExcel({Mode:'HeaderMatch'});
			var url = "<c:url value="/meta/stnd/popup/messagechnl_xls.do" />";
...
// 	alert( JSON.stringify(param));
	$('div#detailInfo').load('<c:url value="/meta/stnd/ajaxgrid/messagechnl_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/stnd/messagechnl_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
