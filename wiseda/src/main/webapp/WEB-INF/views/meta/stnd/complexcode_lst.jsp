...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        //SetMergeSheet(msPrevColumnMerge);
		var headers = [
						{Text:"<s:message code='META.HEADER.COMPLEXCODE.LST'/>", Align:"Center"}
					];
                    //No.|도메인ID|도메인논리명|도메인물리명|코드|상위코드|코드명|레벨|사용여부|표시순서|기타1|기타명1|기타2|기타명2|기타3|기타명3|기타4|기타명4|기타5|기타명5|비고|적요1|적요2|코드ID
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "Search":
			...
			//$('#dmn_sel_title').html('');
			grid_sheet.DoSearch("<c:url value="/meta/stnd/getComplexCodeList.do" />", param);
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='STRD.TERMS.INQ' />"> <!-- 표준용어조회 -->
                   ...
                   <tbody>                            
						<tr>
							...
							<th scope="row"><label for="cdVal"><s:message code="CD" /></label></th> <!-- 코드 -->
							<td><input type="text" id="cdVal" name="cdVal" class="wd200"/></td>
						</tr>
						<tr>
							<th scope="row"><label for="cdValNm"><s:message code="CD.NM" /></label></th> <!-- 코드명 -->
							<td><input type="text" id="cdValNm" name="cdValNm" class="wd200"/></td>
							<th scope="row"><label for="uppCdVal"><s:message code="UPRN.CD" /></label></th> <!-- 상위코드 -->
							<td><input type="text" id="uppCdVal" name="uppCdVal" class="wd200"/></td>
						</tr>
					<!--  	<tr>
