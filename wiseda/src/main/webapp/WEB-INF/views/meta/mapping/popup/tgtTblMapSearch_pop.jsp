...
<html>
<head>
<title><s:message code="TARG.TBL.MAPG.INQ"/></title> <!-- 타겟테이블매핑 검색 -->
...
//     	alert(1);
  ...
  //파라미터 : (자동완성 대상 오브젝트, 검색할 단어종류, 최대표시 갯수(default-10개))
    setautoComplete($("#frmSearch #tgtTblPnm"), "TBLMAP");
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headers = [
                       {Text: "<s:message code='META.HEADER.TGTBLMAPSEARCH.POP1'/>"},
                       /* No.|상태|선택|테이블매핑ID|매핑정의서유형|매핑정의서ID|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|매핑일련번호|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|테이블매핑유형|선택/스킵조건|조인조건|사전이행 고려사항|응용담당자ID|응용담당자명|전환담당자ID|전환담당자명|참고사항 */
                       {Text: "<s:message code='META.HEADER.TGTBLMAPSEARCH.POP2'/>", Align:"Center"}
                       /* No.|상태|선택|테이블매핑ID|매핑정의서유형|매핑정의서ID|DB명|테이블ID|테이블명(물리명)|테이블명(논리명)|이행유형|주제영역ID|주제영역명|매핑일련번호|DB명|테이블명(물리명)|시스템명|업무명|테이블매핑유형|선택/스킵조건|조인조건|사전이행 고려사항|응용담당자ID|응용담당자명|전환담당자ID|전환담당자명|참고사항 */
                   ];
...
//					{Type:"Combo",  Width:60,  SaveName:"vrfCd",	Align:"Center", Edit:0},
					{Type:"Text",   Width:100,  SaveName:"tblMapId", 	 Align:"Left",   Edit:0, Hidden:1},
					{Type:"Combo",   Width:100,  SaveName:"mapDfType", 	 Align:"Left",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"mapDfId", 	 Align:"Left",   Edit:0, KeyField:1},
					{Type:"Text",   Width:100,  SaveName:"tgtDbPnm", 	 Align:"Left",   Edit:0, KeyField:1},
					{Type:"Text",   Width:0,  SaveName:"tgtTblId",    Align:"Left", Edit:0, Hidden:1},
					{Type:"Text",   Width:100,  SaveName:"tgtTblPnm", 	 Align:"Left",   Edit:0, KeyField:1},
					{Type:"Text",   Width:100,  SaveName:"tgtTblLnm", 	 Align:"Left",   Edit:0},
					{Type:"Combo",   Width:100,  SaveName:"tgtFlfType", 	 Align:"Center",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"tgtSubjId", 	 Align:"Left",   Edit:0, Hidden:1},
					{Type:"Text",   Width:100,  SaveName:"tgtSubjAllPath", 	 Align:"Left",   Edit:0},
					
					{Type:"Text",   Width:100,  SaveName:"mapSno", 	 Align:"Center",   Edit:1},
					...
					{Type:"Text",   Width:100,  SaveName:"preFlfDescn", 	 Align:"Left",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"appCrgpId", 	 Align:"Left",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"appCrgpNm", 	 Align:"Left",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"cnvsCrgpId", 	 Align:"Left",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"cnvsCrgpNm", 	 Align:"Left",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"refDescn", 	 Align:"Left",   Edit:0}
                ];
...
//		SetColProperty("regTypCd", 	${codeMap.regTypCdibs});
		...
		//SetColProperty("tgtSubjId", 	${codeMap.subjibs});
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "Search":
        	...
        	grid_sheet.DoSearch("<c:url value="/meta/mapping/popup/gettblmaplist.do" />", param);
        	break;
...
// 			alert(saveJson.Code); 처리대상 행이 없는 경우 리턴한다.
        	...
        	var url = "<c:url value='/meta/mapping/regWam2WaqTblMap.do' />";
...
function grid_sheet_OnDblClick(row, col, value, cellx, celly) {
	...
	//iframe 형태의 팝업일 경우
	if ("${search.popType}" == "I") {
		parent.returnTgtTblMapInfoPop(JSON.stringify(retjson));
	} else {
		opener.returnTgtTblMapInfoPop(JSON.stringify(retjson));
	}
...
<div class="pop_tit"> <!-- 팝업가로사이즈 여기서 조절하면 됩니다 기본은 100% -->
    ...
    <div class="pop_tit_txt"><s:message code="TARG.TBL.MAPG.INQ"/></div><!-- 타겟테이블매핑 검색 -->
...
<!--                             </td> -->
                           <th scope="row"><label for="mapDfId"><s:message code="MAPG.DFNT.P.ID" /></label></th> <!-- 매핑정의서ID -->
                            <td>
                                ...
                                <input type="text" name="mapDfId" id="mapDfId" value="${search.mapDfId}"/>
                                </span>
                            </td>
                            <th scope="row"><label for="tgtTblPnm"><s:message code="TARG.TBL.PHYC.NM"/></label></th> <!-- 타겟테이블물리명 -->
                            <td>
                                <span class="input_file">
                                <input type="text" name="tgtTblPnm" id="tgtTblPnm" value="${search.tgtTblPnm}"/>
                                </span>
                            </td>
                            <th scope="row"><label for="tgtSubjAllPath"><s:message code="SUBJ.TRRT" /></label></th> <!-- 주제영역 -->
	                       	<td >
		                       	<select id="tgtSubjAllPath"  name="tgtSubjAllPath" >
		                       		 <option value=""><s:message code="WHL" /></option> <!-- 전체 -->
									 <c:forEach var="code" items="${codeMap.subj}" varStatus="status">
