...
<html>
<head>
<title><s:message code="CD.MAPG.DFNT.P.REG"/></title> <!-- 코드매핑정의서 등록 -->
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headers = [
                       {Text: "<s:message code='META.HEADER.CODMAP.RQST1'/>"},
                       /* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|매핑정의서유형|코드전환유형|코드매핑유형|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|응용담당자ID|응용담당자명|전환담당자ID|전환담당자명|요청일시|요청자ID|요청자명|요청번호|요청일련번호 */
                       {Text: "<s:message code='META.HEADER.CODMAP.RQST2'/>", Align:"Center"}
                       /* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|매핑정의서유형|코드전환유형|코드매핑유형|코드도메인ID|코드도메인명|코드|코드명|시스템명|업무명|DB명|테이블물리명|테이블논리명|컬럼물리명|컬럼논리명|코드|코드명|상위코드|상위코드명|응용담당자ID|응용담당자명|전환담당자ID|전환담당자명|요청일시|요청자ID|요청자명|요청번호|요청일련번호 */
                  ];
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "NewRqst": //신규요청서
       	 var url = '<c:url value="/meta/mapping/codmap_rqst.do" />';
		 location.href = url;
        break;
        
        case "AddWam": //기존 테이블 추가
			var url   = "<c:url value="/meta/mapping/popup/codMapSearchPop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/mapping/delcodmaprqstlist.do"/>";
...
// 			if (saveJson.Code == "IBS000") return;
			...
			var url = "<c:url value="/meta/mapping/regcodmaprqstlist.do"/>";
...
//         	ibsSaveJson = grid_sheet.GetSaveJson(1);	//DoAllSave와 동일...
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/mapping/getcodmaprqstlist.do" />", param);
			//코드도메인 조회...
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/mapping/approvecodmap.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
        ...
        case "LoadExcel":  //엑셀업로드
			var url = "<c:url value="/meta/mapping/popup/codmap_xls.do" />";
...
function loadDetail(param) {
	//alert("여기??");
	$('div#detailInfo').load('<c:url value="/meta/mapping/ajaxgrid/codmap_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
function grid_sheet_OnClick(row, col, value, cellx, celly) {
	...
	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '<s:message code="TARG.DMN"/> : ' + param.tgtDmnLnm //타겟도메인
	$('#tbl_sle_title').html(tmphtml);
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title"><s:message code="CD.MAPG.DFNT.P.REG"/></div><!-- 코드매핑정의서 등록 -->
	</div>
</div>
<!-- 메뉴 메인 제목 -->
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div class="selected_title_area">
		    <div class="selected_title" id="tbl_sle_title"><s:message code="CD.DMN.CHC"/></div> <!-- 코드도메인을 선택하세요. -->
	</div>
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1"><s:message code="CD.MAPG.DFNT.P"/></a></li> <!-- 코드매핑정의서 -->
