...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headers = [
                       {Text: "<s:message code='META.HEADER.COLMAP.RQST1'/>"},
                    	   /* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|테이블매핑ID|매핑정의서ID|매핑정의서유형|컬럼매핑유형|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|매핑조건|컬럼참조DB명|컬럼참조테이블명|조인조건|응용담당자ID|응용담당자명|전환담당자ID|전환담당자명|요청일시|요청자ID|요청자명|요청번호|요청일련번호 */
                       {Text: "<s:message code='META.HEADER.COLMAP.RQST2'/>", Align:"Center"}
                       /* No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|테이블매핑ID|매핑정의서ID|매핑정의서유형|컬럼매핑유형|DB명|테이블명(물리명)|테이블명(논리명)|컬럼순서|컬럼명(물리명)|컬럼명(논리명)|데이터타입|PK여부|NotNull여부|도메인명(논리명)|시스템명|업무명|DB명|테이블명(물리명)|테이블명(논리명)|컬럼명(물리명)|컬럼명(논리명)|데이터타입|Default값|컬럼설명|매핑조건|컬럼참조DB명|컬럼참조테이블명|조인조건|응용담당자ID|응용담당자명|전환담당자ID|전환담당자명|요청일시|요청자ID|요청자명|요청번호|요청일련번호 */
                   ];
        ...
        var cols = [                        
					...
					{Type:"Text",   Width:100,  SaveName:"tgtColPnm", 	 Align:"Center",   Edit:0, KeyField:1},
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "NewRqst": //신규요청서
       	 var url = '<c:url value="/meta/mapping/colmap_rqst.do" />';
		 location.href = url;
        break;
        
        case "AddWam": //기존 테이블 추가
			var url   = "<c:url value="/meta/mapping/popup/colMapSearchPop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/mapping/delcolmaprqstlist.do"/>";
...
//         	ibsSaveJson = grid_sheet.GetSaveJson(1);	//DoAllSave와 동일...
		...
		case "Search":
			...
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/mapping/getcolmaprqstlist.do" />", param);
			//코드도메인 조회...
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/mapping/approvecolmap.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
        ...
        case "LoadExcel":  //엑셀업로드
			var url = "<c:url value="/meta/mapping/popup/colmap_xls.do" />";
...
function loadDetail(param) {
	//alert("여기??");
	$('div#detailInfo').load('<c:url value="/meta/mapping/ajaxgrid/colmap_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title"><s:message code="CLMN.MAPG.DFNT.P.REG"/></div> <!-- 컬럼매핑정의서 등록 -->
	</div>
</div>
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1"><s:message code="CLMN.MAPG.DFNT.P"/></a></li> <!-- 컬럼매핑정의서 -->
