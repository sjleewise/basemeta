...
<html>
<head>
<title><s:message code="CLMN.MAPG.DFNT.P.SRH"/></title> <!-- 컬럼매핑정의서 검색 -->
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headers = [
                       {Text: "<s:message code='META.HEADER.COLMAPSEARCH.POP1'/>"},
                       /* No.|상태|선택|테이블매핑ID|매핑정의서유형|매핑정의서ID|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|타겟(TOBE)|매핑일련번호|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|소스(ASIS)|테이블매핑유형|선택/스킵조건|조인조건|사전이행 고려사항|응용담당자ID|응용담당자명|전환담당자ID|전환담당자명|참고사항 */
                       {Text: "<s:message code='META.HEADER.COLMAPSEARCH.POP2'/>", Align:"Center"}
                       /* No.|상태|선택|테이블매핑ID|매핑정의서유형|매핑정의서ID|DB명|테이블명(물리명)|테이블명(논리명)|이행유형|주제영역|매핑일련번호|시스템명|업무명|DB명|테이블명(물리명)|테이블명(논리명)|테이블매핑유형|선택/스킵조건|조인조건|사전이행 고려사항|응용담당자ID|응용담당자명|전환담당자ID|전환담당자명|참고사항 */
                   ];
        ...
        var cols = [                        
					...
					{Type:"Combo",  Width:100,  SaveName:"tgtSubjId", 	 Align:"Left",   Edit:0},					
					{Type:"Text",   Width:100,  SaveName:"mapSno", 	     Align:"Center",   Edit:1, Hidden:1},
					...
					{Type:"Text",   Width:100,  SaveName:"sltSkpCndNm",  Align:"Center",   Edit:0, Hidden:1},
					{Type:"Text",   Width:100,  SaveName:"jnCndNm", 	 Align:"Center",   Edit:0, Hidden:1},
					{Type:"Text",   Width:100,  SaveName:"preFlfDescn",  Align:"Center",   Edit:0, Hidden:1},
					{Type:"Text",   Width:100,  SaveName:"appCrgpId", 	 Align:"Center",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"appCrgpNm", 	 Align:"Center",   Edit:0},
					{Type:"Text",   Width:100,  SaveName:"cnvsCrgpId", 	 Align:"Center",   Edit:0, Hidden:1},
					{Type:"Text",   Width:100,  SaveName:"cnvsCrgpNm", 	 Align:"Center",   Edit:0, Hidden:1},
					{Type:"Text",   Width:100,  SaveName:"refDescn", 	 Align:"Center",   Edit:0, Hidden:1}
                ];
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "Search":
        	...
        	grid_sheet.DoSearch("<c:url value="/meta/mapping/popup/getcolmaplist.do" />", param);
        	break;
...
// 			alert(saveJson.Code); 처리대상 행이 없는 경우 리턴한다.
        	...
        	var url = "<c:url value='/meta/mapping/regWam2WaqColMap.do' />";
...
<div class="pop_tit"> <!-- 팝업가로사이즈 여기서 조절하면 됩니다 기본은 100% -->
    ...
    <div class="pop_tit_txt"><s:message code="CLMN.MAPG.DFNT.P.SRH"/></div><!-- 컬럼매핑정의서 검색 -->
...
<!--                             </td> -->
                           <th scope="row"><label for="mapDfId"><s:message code="MAPG.DFNT.P.ID" /></label></th> <!-- 매핑정의서ID -->
                            <td>
                                ...
                                <input type="text" name="mapDfId" id="mapDfId" style="width:98%;" value="${search.mapDfId}"/>
                                </span>
                            </td>
                            <th scope="row"><label for="tgtTblPnm"><s:message code="TARG.TBL.PHYC.NM"/></label></th> <!-- 타겟테이블물리명 -->
                            <td>
                                <span class="input_file">
                                <input type="text" name="tgtTblPnm" id="tgtTblPnm" style="width:98%;" value="${search.tgtTblPnm}"/>
                                </span>
                            </td>
                            <th scope="row"><label for="tgtSubjAllPath"><s:message code="SUBJ.TRRT" /></label></th> <!-- 주제영역 -->
	                       	<td >
		                       	<select id="tgtSubjAllPath"  name="tgtSubjAllPath" style="width:98%;" >
