...
// 	grid_sub_dbcidxcolchange.FitColWidth();
});


function initsubgrid_dbcidxcolchange() {

    with(grid_sub_dbcidxcolchange){
...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        var headers = [
                    {Text:"<s:message code='META.HEADER.DBCIDXCOLCHANGE.DTL'/>", Align:"Center"}
                    /* No.|DB스키마ID|만료일시|시작일시|DB스키마명|DBC테이블명|DBC인덱스명|DBC인덱스컬럼명|DBC인덱스컬럼한글명|순서|정렬타입|데이터타입|데이터자리수|데이터길이|데이터소수점|버전|설명 */
                ];
        ...
        var cols = [                        
                    {Type:"Seq",	Width:50,   SaveName:"ibsSeq",		Align:"Center", Edit:0},
						...
						{Type:"Text",   Width:100,   SaveName:"vers",	 	Align:"Right", Edit:0, Hidden:0},
   	...
   	//==시트설정 후 아래에 와야함=== 
    init_sheet(grid_sub_dbcidxcolchange);    
    //===========================
  	
    //저장 처리 과정을 디버깅 메시지를 팝업으로 표시 (-1)
//     grid_sub_dbcidxcolchange.ShowDebugMsg(-1);	
    	
}

$(window).on('load',function() {
	initsubgrid_dbcidxcolchange();
...
y : y좌표
*/
function grid_sub_dbcidxcolchange_OnDblClick(row, col, value, cellx, celly) {
...
function grid_sub_dbcidxcolchange_OnClick(row, col, value, cellx, celly) {
...
function grid_sub_dbcidxcolchange_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<!-- 그리드 입력 입력 -->
	<div class="grid_01">
	     <script type="text/javascript">createIBSheet("grid_sub_dbcidxcolchange", "100%", "250px")</script>
	</div>
	<!-- 그리드 입력 입력 End -->
