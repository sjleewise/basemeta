...
//         initSearchButton();	//검색 버튼 초기화
		...
		$("#btnRegRqst").click(function(){
			...
			var message = "";
			if (cntrqst > 0) message += "[DDL 테이블:"+cntrqst+"건]";
...
// 			doAllApprove(grid_sheet, "2");
		...
		$("#btnReqApprove").click(function(){
			...
			if (cntappr > 0) message += "[기타오브젝트:"+cntappr+"건 중 승인 "+cntAppr+"건, 반려 "+cntReject+"건]<br>";
        ...
        $("#btnBlank").click( function(){
    		location.href = '<c:url value="/meta/ddletc/ddletc_rqst.do" />';
        } );
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        headtext += "<s:message code='META.HEADER.DDLETC.RQST'/>";
        //No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|DB명|스키마명|오브젝트구분|기타오브젝트명|기타오브젝트한글명|DDL|스크립트정보|요청일시|요청자ID|요청자명|요청번호|요청일련번호
        ...
        var cols = [                        
                    ...
                    {Type:"Text",     Width:90,   SaveName:"dbConnTrgPnm",	Align:"Center", Edit:0},
                    {Type:"Text",     Width:90,   SaveName:"dbSchPnm",	    Align:"Center", Edit:0},
                    {Type:"Combo",    Width:90,   SaveName:"etcObjDcd",	    Align:"Center", Edit:0},
                    {Type:"Text",     Width:150,  SaveName:"ddlEtcPnm",     Align:"Left",   Edit:0, KeyField:1}, 
                    {Type:"Text",     Width:150,  SaveName:"ddlEtcLnm", 	Align:"Left",   Edit:0, Hidden:0},
                    {Type:"CheckBox", Width:70,   SaveName:"ddlCheck", 	    Align:"Left",   Edit:1, Hidden:1},                                        
                    {Type:"Text",     Width:500,  SaveName:"scrtInfo",      Align:"Left",   Edit:0, Hidden:0},
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "AddWam": //기존 테이블 추가
			var url   = "<c:url value="/meta/ddletc/popup/ddletc_pop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/ddletc/delDdlEtcRqstList.do"/>";
		...
		case "SaveRow" :
			...
			if( $("#dbConnTrgId").val() != ""){
				dbConnTrgPnm = $("#dbConnTrgId option:selected").text();
			}

		    if( $("#dbSchId").val() != ""){
		    	dbSchPnm     = $("#dbSchId option:selected").text();
			}
					
			$("#dbConnTrgPnm").val(dbConnTrgPnm);
			$("#dbSchPnm").val(dbSchPnm);
			
			var saveJson = getform2IBSjson($("#frmInput"));

			if(saveJson.data.length == 0) return;
			
			var url = "<c:url value="/meta/ddletc/regDdlEctRqstList.do"/>";
...
//         	ibsSaveJson = grid_sheet.GetSaveJson(1);	//DoAllSave와 동일...
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/ddletc/getDdlEtcRqstList.do" />", param);
			//컬럼 리스트 조회...
// 			doActionCol("Search");
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/ddletc/approveddletc.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
        ...
        case "LoadExcel":  //엑셀업로드
			var url = "<c:url value="/meta/ddletc/popup/ddletc_xls.do" />";
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/meta/ddletc/ajaxgrid/ddletc_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
// 				doActionCol("Search");
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/ddletc/ddletc_rqst.do" />';
			if(!isBlankStr(res.resultVO.rqstNo)) {
...
//     tblClick(row);
	...
	var tmphtml = '기타오브젝트: ' + param.ddlEtcPnm + ' [' + param.ddlEtcLnm +']';
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title">기타오브젝트 등록</div>
	</div>
</div>
<!-- 메뉴 메인 제목 -->
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1">기타오브젝트</a></li> <!-- 테이블정의서 -->
	    
	    <c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S'}">
	    	<li id="tabs-rqstvrf"><a href="#tabs-2">검증결과</a></li>  <!-- 검증결과 --> 
	    </c:if>
