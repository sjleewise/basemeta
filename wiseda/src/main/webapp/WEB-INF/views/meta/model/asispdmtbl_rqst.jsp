...
// 		//imgConvert($('div.tab_navi a img'));
		...
		var checkrequstcount = '<c:out value="${checkrequstcount}"/>';		 
		if(checkrequstcount==1){
		...
		$("#btnRegRqst").click(function(){
			...
			var message = "";
			if (cntrqst > 0) message += "[물리모델:"+cntrqst+"건]";
...
//	 			$("#frmInput #rvwConts").focus();
			...
			if (cntappr > 0) message += "[물리모델:"+cntappr+"건 중 승인 "+cntAppr+"건, 반려 "+cntReject+"건]<br>";
...
//         create_selectbox(sysareaJson, $("#sysAreaId"));
      ...
      //신규요청서
        $("#btnBlank").click( function(){
    		location.href = '<c:url value="/meta/model/asispdmtbl_rqst.do" />';
        } );
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.ASIS.PDM.TBL.RQST'/>";
        //No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|주제영역|시스템명|DB유형|DB명|테이블ID|테이블(물리명)|테이블(논리명)|이전테이블(물리명)|모델논리명|상위주제영역논리명|주제영역ID|전환대상여부|미전환대상사유|표준적용여부|담당자ID|담당자명|비고|요청일시|요청자ID|요청자명|요청번호|요청일련번호
        ...
        var cols = [                        
					...
					{Type:"Combo",  Width:110,  SaveName:"vrfCd",	  Align:"Center", Edit:0},
                    
                    {Type:"Text",   Width:200, SaveName:"fullPath",	 	 Align:"Left", Edit:0, KeyField:1}, 
                    {Type:"Text",   Width:150, SaveName:"systemNm",	 	 Align:"Left", Edit:0, KeyField:1}, 
                    {Type:"Text",   Width:100, SaveName:"dbmsTypCd",	 	 Align:"Left", Edit:0, KeyField:1}, 
                    {Type:"Text",   Width:150, SaveName:"dbmsNm",	 	 Align:"Left", Edit:0, KeyField:1}, 
                    {Type:"Text",   Width:80,  SaveName:"pdmTblId",   	 Align:"Left",   Edit:0},
                    {Type:"Text",   Width:190, SaveName:"pdmTblPnm",     Align:"Left",   Edit:0, KeyField:1},   
                    {Type:"Text",   Width:190, SaveName:"pdmTblLnm", 	 Align:"Left",   Edit:0, KeyField:0},
                    ...
                    {Type:"Text",   Width:300, SaveName:"nswtTrgtRsn"	,    Align:"Center",   Edit:0 },
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "AddWam": //기존 테이블 추가
			var url   = "<c:url value="/meta/model/popup/asispdmtblSearchPop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/model/delasispdmtblrqstlist.do"/>";
...
// 			if (saveJson.Code == "IBS000") return;
			...
			var url = "<c:url value="/meta/model/regasispdmtblrqstlist.do"/>";
...
//         	ibsSaveJson = grid_sheet.GetSaveJson(1);	//DoAllSave와 동일...
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/model/getasispdmtblrqstlist.do" />", param);
			//코드도메인 조회...
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/model/approveasispdmtbl.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
		...
		case "RegRqstCancel":  
			var rqstno     = "${waqMstr.rqstNo}";
	  		var rqstbizdcd = "${waqMstr.bizDcd}"; 
	  		
    		var url = "<%=request.getContextPath()%>/commons/damgmt/approve/requestasisCancel.do";
    				  
    		location.href=url+"?rqst_no="+rqstno+"&biz_dcd="+rqstbizdcd;
        ...
        case "LoadExcel":  //엑셀업로드
			var url = "<c:url value="/meta/model/popup/asispdmtbl_xls.do" />";
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/model/regasispdmcolrqstlist.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/model/delasispdmcolrqstlist.do"/>";
			var param = $("#mstFrm").serialize();
			IBSpostJson2(url, DelJson, param, ibscallback);
        	break;
...
//         	var nrow = col_sheet.DataInsert();
        ...
        case "SaveRel": //관계 리스트 저장
			...
			var url = "<c:url value="/meta/model/regasispdmrelrqstlist.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/model/delasispdmrelrqstlist.do"/>";
			var param = $("#mstFrm").serialize();
			IBSpostJson2(url, DelJson, param, ibscallback);
        	break;
        	
        case "RegRqstCancel":  
    		var rqstno='<c:out value="${rqstno}"/>';
    		var rqstbizdcd='<c:out value="${rqstbizdcd}"/>';
    		var url = "<%=request.getContextPath()%>/commons/damgmt/approve/requestasisCancel.do";
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/meta/model/ajaxgrid/asispdmtbl_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/model/asispdmtbl_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
//     tblClick(row);
	...
	var param1 = $("#mstFrm").serialize();
	param1 += "&rqstSno=" + param.rqstSno;
	param1 += "&pdmTblId=" + param.pdmTblId;
	param1 += "&pdmTblPnm=" + param.pdmTblPnm;
	param1 += "&regTypCd=" + param.regTypCd;
 	 
	col_sheet.DoSearch("<c:url value="/meta/model/getasispdmcolrqstlist.do" />", param1);
	
	<c:if test="${ pdmrelyn == 'Y'}">
	rel_sheet.DoSearch("<c:url value="/meta/model/getasispdmrelrqstlist.do" />", param1);
	</c:if>
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title">물리모델 등록</div>
	</div>
</div>
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs" style="display: none;">
	  <ul>
	    <li><a href="#tabs-1">테이블 정의서</a></li>
	    <li><a href="#tabs-2">컬럼 목록</a></li>
	    <c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S' or waqMstr.rqstStepCd == 'Q'}">
	    <li id="tabs-rqstvrf"><a href="#tabs-4">검증결과</a></li>
	    <li id="tabs-rqstchg"><a href="#tabs-5">테이블변경항목</a></li>
	    <li id="tabs-rqstchg"><a href="#tabs-6" id="colinfo">컬럼변경항목</a></li>
	    <c:if test="${ pdmrelyn == 'Y'}">
	    <li><a href="#tabs-3" >관계 목록</a></li> 
	    <li id="tabs-rqstchg" ><a href="#tabs-7" id="relinfo">관계변경항목</a></li>
	    </c:if>
	    </c:if>
	  </ul>
	  <div id="tabs-1">
		...
		<!-- 컬럼 목록 탭 -->
	  <div id="tabs-2">
		<%@include file="asispdmcol_rqst.jsp" %>
	  </div>
	  <c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S' or waqMstr.rqstStepCd == 'Q'}">
	  <div id="tabs-4">
	  ...
	  <c:if test="${ pdmrelyn == 'Y'}">
	  <div id="tabs-3">
		<%@include file="asispdmrel_rqst.jsp" %>
	  </div>
	  <div id="tabs-7">
