...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.ASIS.PDM.COL.XLS'/>";
        //No.|상태|삭제|요청구분|등록유형|주제영역|테이블(물리명)|컬럼(물리명)|컬럼(논리명)|이전컬럼(물리명)|컬럼순서|데이터타입|길이|소수점길이|PK 여부|PK 순서|NOT NULL여부|DEFAULT 값|암호화여부|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호|요청상세일련번호
        var headers = [
...
//  	 	   	if(SaveJson.data.length == 0) return;
 	 	    ...
 	 	    var url = "<c:url value="/meta/model/regasispdmcolxlsrqstlist.do"/>";
	 	 	if ("${search.popType}" == "I") {
...
<div class="pop_tit" >
    ...
    <div class="pop_tit_txt">엑셀업로드-AS-IS물리모델컬럼</div>
