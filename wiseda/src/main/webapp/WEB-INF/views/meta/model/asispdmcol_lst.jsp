...
function initColGrid()
{
    ...
    with(col_sheet){
        ...
        var headtext = "<s:message code='META.HEADER.ASIS.PDM.COL.LST'/>";
        //No.|컬럼ID|물리모델테이블ID|테이블논리명|테이블물리명|표준용어ID|표준용어명|표준용어물리명|도메인ID|도메인명|도메인물리명|인포타입ID|인포타입명|컬럼명|컬럼한글명|컬럼순서|데이터타입|PK여부|PK순서|NOTNULL여부|DEFAULT값|암호화여부|등록유형|최초요청자ID|최초요청자|최초요청일시|요청자ID|요청자|요청일시|승인자ID|승인자|승인일시|버전|설명
...
function doActionCol(sAction)
{
    switch(sAction)
    {
        case "Search":	//요청서 재조회...
        	...
        	col_sheet.DoSearch("<c:url value="/meta/model/getasispdmcollist.do" />", param);
