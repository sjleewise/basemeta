...
//	     	  $(this).addClass('search_button');
		...
		//$('div#popSearch').dialog("open");
			...
			var popwin = OpenModal(url+"?"+param, "subjR7Pop",  700, 400, "no");  
			popwin.focus();
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext = "<s:message code='META.HEADER.R9.PDMTBLSEARCH.POP.SOFT'/>";
        //No.|상태|선택|요청구분|등록유형|주제영역ID|주제영역|테이블ID|테이블(물리명)|테이블(논리명)|표준적용여부|담당사용자ID|설명|category_id|model_id|subjectarea_id|entity_id|category_name|model_name|subjectarea_name|entity_name|entity_name_physical|entity_name_logical
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "Search":
        	...
        	if($("#rqstDcd").val() == "CU") {
        		//grid_sheet.DoSearch("<c:url value="/meta/model/getmarttbllist.do" />", param);	
        		
        		grid_sheet.DoSearch("<c:url value="/meta/model/getmart9tbllistsoft.do" />", param);
        	}else{
...
function grid_sheet_OnSearchEnd(code, message) {
	...
	if(code < 0) {
		...
		//grid_sheet.ColumnSort("pdmTblPnm","ASC");
...
<div class="pop_tit"> <!-- 팝업가로사이즈 여기서 조절하면 됩니다 기본은 100% -->
    ...
    <div class="pop_tit_txt">테이블 검색</div>  <!-- 테이블 검색 -->
        ...
        <form id="frmSearch" name="frmSearch" method="post">
            ...
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="주제영역조회">
                   ...
                   <tbody>                            
                       <tr> 
                           <th scope="row" class="th_require"><label for="rqstDcd">요청구분</label></th>  <!-- 요청구분 -->                       
						   <td>
						   ...
						   <td>
                                <span class="input_file"> 
                                	<input type="text" name="subjLnm" id="subjLnm" style="width:85%;" />
