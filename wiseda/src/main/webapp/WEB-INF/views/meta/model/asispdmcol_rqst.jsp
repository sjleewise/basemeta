...
function initColGrid()
{
    ...
    with(col_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.ASIS.PDM.COL.RQST'/>";
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|주제영역|테이블(물리명)|컬럼ID|컬럼(물리명)|컬럼(논리명)|이전컬럼(물리명)|테이블ID|모델명|상위주제영역명|주제영역ID|주제영역명|표준용어ID|컬럼순서|데이터타입|길이|소수점길이|PK 여부|PK 순서|NOT NULL여부|DEFAULT 값|암호화여부|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호|요청상세일련번호
	    var headers = [
        ...
        var cols = [                        
                    ...
                    {Type:"Text",    Width:180,  SaveName:"pdmColPnm"   ,Align:"Left",   Edit:0, KeyField:1},
                    {Type:"Text",    Width:180,  SaveName:"pdmColLnm"   ,Align:"Left",   Edit:0, KeyField:1},
                    ...
                    {Type:"Text",    Width:70,   SaveName:"dataLen"     ,Align:"Center",   Edit:0},
                    {Type:"Text",    Width:70,   SaveName:"dataScal"    ,Align:"Center",   Edit:0},
                    {Type:"Combo",   Width:70,   SaveName:"pkYn"        ,Align:"Left",   Edit:0},
                    {Type:"Int",     Width:70,   SaveName:"pkOrd"       ,Align:"Left",   Edit:0},
                    {Type:"Combo",   Width:70,   SaveName:"nonulYn"     ,Align:"Left",   Edit:0},
                    {Type:"Text",    Width:70,   SaveName:"defltVal"    ,Align:"Left",   Edit:0},
                    {Type:"Combo",   Width:100,  SaveName:"encYn"       ,Align:"Center",   Edit:0, Hidden:1},
...
//         	var param = $('#colfrmSearch').serialize();
        	...
        	col_sheet.DoSearch("<c:url value="/meta/model/getasispdmcolrqstlist.do" />", param);
//         	col_sheet.DoSearchScript("testJsonlist");
        ...
        case "SearchRow": //단일 조회...
        	...
        	col_sheet.DoRowSearch(crow, "<c:url value="/meta/model/getasispdmcolrqstinfo.do" />",  param ,0);
        break;
        ...
        case "LoadExcel":  //엑셀업로드
			var url = "<c:url value="/meta/model/popup/asispdmcol_xls.do" />";
...
function loadDetailCol(param) {
	$('div#detailInfocol').load('<c:url value="/meta/model/ajaxgrid/asispdmcol_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
// 	alert($("#mstFrm #bizDcd").val());
	if($("#mstFrm #bizDcd").val() == "ASP"){
	...
	var param1  = "bizDtlCd=COL";
	    ...
	    param1 += "&searchObj="+param.pdmTblPnm;	    
	    param1 += "&bizDcd=ASP";
...
<!-- 			    <button class="btn_search" id="btnSearch" 	name="btnSearch">재조회</button>  -->
                <button class="btn_rqst_new" id="btnColRqstNew" name="btnColRqstNew">추가</button>                                                         
				  <ul class="add_button_menu">
				    <li class="btn_new" id="btnColNew"><a><span class="ui-icon ui-icon-pencil"></span>신규 추가</a></li>
