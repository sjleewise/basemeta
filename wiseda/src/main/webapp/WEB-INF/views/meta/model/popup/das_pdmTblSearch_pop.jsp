    ...
    switch(sAction)
    {
        ...
        case "Search":
        	...
        	if($("#rqstDcd").val() == "CU") {
        		...
        		grid_sheet.DoSearch("<c:url value="/meta/model/getMartDasTblList.do" />", param);
        	}else{
...
// 			alert(saveJson.Code); 처리대상 행이 없는 경우 리턴한다.
        	...
        	if($("#rqstDcd").val() == "CU"){
        		...
        		url = "<c:url value="/meta/model/regMartDasWaqPdmTbl.do" />";
        	}else{
        		param += "&rqstDcd=DD";
    	        url = "<c:url value="/meta/model/regWamDasWaqPdmTbl.do" />";        		
        	}
...
//	alert(ret);
	...
	if(${maxSubjLvl} != subjLvl){
		showMsgBox("ERR", "주제영역을 "+ "${maxSubjLvl + 1}" +"단계로 선택해주세요.");
	}else{
