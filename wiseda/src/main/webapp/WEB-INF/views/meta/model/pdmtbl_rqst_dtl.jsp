...
// 	$("#frmInput #stwdLnm").focus();
	...
	$("#frmInput #openRsnCd").val('${result.openRsnCd}');
	$("#frmInput #nopenRsn").val('${result.nopenRsn}');
	$("#frmInput #occrCyl").val('${result.occrCyl}');
	$("#frmInput #dqDgnsYn").val('${result.dqDgnsYn}');
	$("#frmInput #nopenDtlRelBss").val('${result.nopenDtlRelBss}');
	$("#frmInput #openDataLst").val('${result.openDataLst}');
...
// 	chkformsetup($("#frmInput #dwTrgTblYn"), '${result.dwTrgTblYn}');	
	if("${result.stdAplYn}" == "Y") $("#frmInput #stdAplYn").prop('checked', true);
	if("${result.partTblYn}" == "Y") $("#frmInput #partTblYn").prop('checked', true);
	if("${result.dwTrgTblYn}" == "Y") $("#frmInput #dwTrgTblYn").prop('checked', true);
...
//범정부항목 - 공개/비공개여부에 따른 비공개사유 활성화
function OpenRsn(){
	if($('#openRsnCd').val() == '02'){
		$('#nopenRsn').attr("disabled",false);
	}else{
		$('#nopenRsn').attr("disabled",true);
		$('#nopenRsn').val('');
	}
}
...
<div id="input_form_div">
	...
	<div class="tb_basic">
		<table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='MSG.TBL.SMRY' />"> <!-- 테이블 서머리입니다. -->
			...
			<tbody>
				<tr>
					<th scope="row"  class="th_require"><s:message code="SUBJ.TRRT.NM" /></th> <!-- 주제영역명 -->
					<td colspan="3">
...
<!--                    </tr> -->
                   <tr>
                       ...
                       <td colspan="3"><input type="text" id="pdmTblPnm" name="pdmTblPnm" class="wd500" value="${result.pdmTblPnm }" title="<s:message code='MSG.TBL.NM.INPT' />" /></td> <!-- 테이블명은 반드시 입력해야 합니다. -->
                       <th scope="row" style="display:none"><label for="bfPdmTblPnm"><s:message code="BFR.TBL.NM"/></label></th> <!--이전 테이블명 -->
                       <td style="display:none"><input type="text" id="bfPdmTblPnm" name="bfPdmTblPnm" class="wd200" value="${result.bfPdmTblPnm }"  /></td>
                   </tr>
                   <tr>
                       <th scope="row" class="th_require"><label for="pdmTblLnm"><s:message code="TBL.NM.LGC.NM" /></label></th> <!-- 테이블명(논리명) -->
                       <td colspan = "3"><input type="text" id="pdmTblLnm" name="pdmTblLnm" class="wd500" value="${result.pdmTblLnm }"  /></td>
                   ...
                   <tr class="ht50">
                       ...
                       <td colspan="3"><textarea id="userRqstCntn" name="userRqstCntn" class = "wd98p" >${result.userRqstCntn }</textarea></td>
                   </tr>
                   <tr>
                       <th scope="row" ><s:message code="STRD.APL.YN"/></th> <!-- 적용 여부 -->
                       <td colspan="3">
                   ...
                   <tr>
                      ...
                      <td><input type="text" id="ctdFcy" name="ctdFcy" class="wd98p" value="${result.ctdFcy }" /></td>
                   	  <th scope="row"><label for="bckFcy"><s:message code="BAUP.CYCL"/></label></th> <!-- 백업주기 -->
                      <td><input type="text" id="bckFcy" name="bckFcy" class="wd98p" value="${result.bckFcy }" /></td>
                   </tr>
                   <tr>
                   	  <th scope="row"><label for="delCri"><s:message code="DEL.BASE"/></label></th> <!-- 삭제기준 -->
                      <td><input type="text" id="delCri" name="delCri" class="wd98p" value="${result.delCri }" /></td>
                   	  <th scope="row"><label for="delMtd"><s:message code="DEL.MTH"/></label></th> <!-- 삭제방법 -->
                      <td><input type="text" id="delMtd" name="delMtd" class="wd98p" value="${result.delMtd }" /></td>
                   </tr>
               </tbody>
           </table>
       </div>
...
<!-- 				<tbody> -->
<!-- 					<tr> -->
<%-- 						<th scope="row"><s:message code="SUBJ.NM" /></th> <!-- 업무분류체계명 --> --%>
<%-- 						<td colspan="3"><input type="text" id="subjNm" name="subjNm" class="wd98p" value="${result.subjNm }" /></td> --%>
<!-- 					</tr> -->
<!-- 					<tr> -->
<%-- 						<th scope="row"><s:message code="PRSV.TERM" /></th> <!-- 보존기간 --> --%>
<!-- 						<td> -->
<%-- 							<select  id="prsvTerm" name="prsvTerm"  class="wd300"> --%>
<%-- 				                <c:forEach var="code" items="${codeMap.prsvTerm}" varStatus="status"> --%>
<%-- 				        		  <option value="${code.codeCd}">${code.codeLnm}</option> --%>
<%-- 				        		</c:forEach>  --%>
<%-- 					        </select> --%>
<!-- 						</td> -->
<%-- 						<th scope="row"><s:message code="TBL.VOL" /></th> <!-- 테이블볼륨 --> --%>
<%-- 						<td><input type="text" id="tblVol" name="tblVol" class="wd98p" value="${result.tblVol }" /></td> --%>
<!-- 					</tr> -->
<!-- 					<tr> -->
<%-- 						<th scope="row"><s:message code="OPEN.RSN.CD" /></th> <!-- 공개/비공개여부 --> --%>
<!-- 						<td> -->
<%-- 							<select  id="openRsnCd" name="openRsnCd"  class="wd300" onchange="OpenRsn()"> --%>
...
<!-- 						<td> -->
<%-- 							<select  id="nopenRsn" name="nopenRsn"  class="wd300"> --%>
<%-- 								<option value=""><s:message code="CHC"/></option> --%>
...
<!-- 					</tr> -->
<!-- 					<tr> -->
<%-- 						<th scope="row"><s:message code="NOPEN.DTL.REL.BSS" /></th> <!-- 상세관련근거 --> --%>
<!-- 						<td colspan="3"><textarea id="nopenDtlRelBss" name="nopenDtlRelBss" class = "wd98p" ></textarea></td> -->
<!-- 					</tr> -->
<!-- 					<tr> -->
<%-- 						<th scope="row"><s:message code="OPEN.DATA.LST" /></th> <!-- 개방데이터목록 --> --%>
<!-- 						<td colspan="3"><textarea id="openDataLst" name="openDataLst" class = "wd98p" ></textarea></td> -->
<!-- 					</tr> -->
<!-- 					<tr> -->
<%-- 						<th scope="row"><s:message code="OCCR.CYL" /></th> <!-- 발생주기 --> --%>
<!-- 						<td> -->
<%-- 							<select  id="occrCyl" name="occrCyl"  class="wd300"> --%>
<%-- 				                <c:forEach var="code" items="${codeMap.occrCyl}" varStatus="status"> --%>
<%-- 				        		  <option value="${code.codeCd}">${code.codeLnm}</option> --%>
<%-- 				        		</c:forEach>  --%>
<%-- 					        </select> --%>
<!-- 						</td> -->
<%-- 						<th scope="row"><s:message code="DQ.DGNS.YN" /></th> <!-- 품질진단여부 --> --%>
<!-- 						<td> -->
