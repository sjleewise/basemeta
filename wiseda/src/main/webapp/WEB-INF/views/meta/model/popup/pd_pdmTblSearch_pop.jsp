...
<html>
<head>
<title>POWER DESIGNER <s:message code="TBL.INQ" /></title><!-- 테이블 검색 -->
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headtext = "<s:message code='META.HEADER.PD.PDMTBLSEARCH.POP'/>";
// 	        +"객체버전|등록유형코드|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|";

          //No.|상태|선택|"
          //+"주제영역ID|주제영역|"
          //+"테이블ID|테이블(물리명)|테이블(논리명)|"
          //+"표준적용여부|모델등록여부|"
          //+"담당사용자ID|객체설명|
        ...
        var cols = [                        
                    ...
                    {Type:"Combo",  Width:60,  SaveName:"pdmRegYn"	,    Align:"Center",   Edit:0},
...
// 	     SetColProperty("regTypCd", 	{ComboCode:"C|U|D", ComboText:"<s:message code='NEW.CHG.DEL' />"});/* 신규|변경|삭제 */
	     SetColProperty("pdmRegYn", 	{ComboCode:"N|Y", 	ComboText:"N|Y"});
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        ...
        case "Search":
        	...
        	grid_sheet.DoSearch("<c:url value="/meta/model/getpdmarttbllist.do" />", param);
...
// 			alert(saveJson.Code); 처리대상 행이 없는 경우 리턴한다.
        	...
        	param += "&rqstDcd=CU";
        	var url = "<c:url value="/meta/model/regpdMart2WaqPdmTbl.do" />";
			//alert(saveJson);
...
// 	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
 			...
 			//opener.doAction("Search");
 			//alert($(opener.document).find("#rqstNo").val()+33);
                ...
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="<s:message code='SUBJ.TRRT.INQ' />"> <!-- 주제영역조회 -->
                   ...
                   <tbody>                            
                       <tr>                               
                            ...
                            <td><select id="subjLnm" class="" name="subjLnm">
