...
function initColHlGrid()
{
    ...
    with(colh_sheet){
        ...
        SetConfig(cfg);
        SetMergeSheet(1);
        var headtext = "<s:message code='META.HEADER.ASIS.COL.HIST.DTL'/>";
		//No.|등록유형|버전|컬럼ID|만료일시|시작일시|물리모델테이블ID|물리모델테이블|표준용어ID|표준용어(논리명)|표준용어(물리명)|도메인ID|도메인(논리명)|도메인(물리명)|인포타입ID|인포타입명|컬럼(물리명)|컬럼(논리명)|컬럼순서|데이터타입|길이|소수점|PK여부|PK순서|NOTNULL여부|DEFAULT값|최초요청자|최초요청일시|요청자|요청일시|승인자|승인일시|설명        					
        ...
        var cols = [                        
                    ...
                    {Type:"Text",    Width:150,  SaveName:"sditmLnm"     ,Align:"Left",   Edit:0, ColMerge:1},
                    {Type:"Text",    Width:150,  SaveName:"sditmPnm"     ,Align:"Left",   Edit:0, Hidden:0, ColMerge:1},
                    {Type:"Text",    Width:100,  SaveName:"dmnId"     ,Align:"Left",   Edit:0, Hidden:1},
                    {Type:"Text",    Width:150,  SaveName:"dmnLnm"     ,Align:"Left",   Edit:0, ColMerge:0},
                    ...
                    {Type:"Text",    Width:150,  SaveName:"infotpLnm"     ,Align:"Left",   Edit:0, ColMerge:0},
...
//         	alert(param);
        	colh_sheet.DoSearch("<c:url value="/meta/model/getasishistcollist.do" />", param);
//         	colh_sheet.DoSearchScript("testJsonlist");
        break;
        case "SearchOne":
        	var param =  "pdmColId=" + col_sheet.GetCellValue(col_sheet.GetSelectRow(), "pdmColId");
        	colh_sheet.DoSearch("<c:url value="/meta/model/getasishisttblcollist.do" />", param);
        break;
    }
}
