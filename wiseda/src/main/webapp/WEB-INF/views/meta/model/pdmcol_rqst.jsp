...
$(window).on('load',function() {
	...
	//erwin 등록 버튼 숨긴다.
	if("${waqMstr.bizDcd}" == "R9P" || "${waqMstr.bizDcd}" == "R7P" || "${waqMstr.bizDcd}" == "DAS" || "${waqMstr.bizDcd}" == "EXP"){
...
function initColGrid()
{
    ...
    with(col_sheet){
        ...
        var headtext  = "<s:message code='META.HEADER.PDMCOL.RQST.1'/>";
		    ...
		    headtext += "<s:message code='META.HEADER.PDMCOL.RQST.8'/>";

		 // 범정부 항목
			if ("${govYn}" == "Y") {
				headtext += "<s:message code='META.HEADER.PDMCOL.RQST.8'/>"; //범정부 중앙메타 연계항목
		    }
        ...
        var cols = [                        
                    ...
                    {Type:"Combo",     Width:40,  SaveName:"fkYn"       ,Align:"Left",   Edit:0},
                    {Type:"Combo",     Width:40,  SaveName:"akYn"       ,Align:"Left",   Edit:0},
        ...
        if ("${govYn}" == "Y") {
        	cols.push(
					...
					{Type:"Combo",    Width:80,  SaveName:"prsnInfoYn" ,Align:"Center",   Edit:0},
					{Type:"Text",     Width:80,  SaveName:"constCnd" ,Align:"Center",   Edit:0}
				);
        }
...
//         	var param = $('#colfrmSearch').serialize();
        	...
        	col_sheet.DoSearch("<c:url value="/meta/model/getpdmcolrqstlist.do" />", param);
...
function loadDetailCol(param) {
	$('div#detailInfocol').load('<c:url value="/meta/model/ajaxgrid/pdmcol_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
function col_sheet_OnDblClick(row, col, value, cellx, celly) {
	...
	param1 += "&rqstSno=" + param.rqstSno + "&rqstDtlSno=" + param.rqstDtlSno   + "&searchObj=" + param.pdmTblPnm ;
	$("#tabs #colinfo").click();
