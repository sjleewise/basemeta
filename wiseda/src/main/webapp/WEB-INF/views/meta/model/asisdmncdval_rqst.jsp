...
<html>
<head>
<title>ASIS유효값 등록요청</title>
<script type="text/javascript">

$(document).ready(function() { 

	//업무구분상세 초기화...
	$("#mstFrm #bizDtlCd").val('DCV');
...
// 	$("#btnDmnSch").click(function(){ setDmn($("#dmnLnm").val());  });
	...
	$("#btnChangAdd").click(function(){
		doAction("AddWam");
// 		var popup = OpenWindow("<c:url value="/meta/model/sditmlst_pop.do"/>","sditmSch","800","600","yes");
// 		popup.focus();
	});
...
//         	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
    ...
    $("#btnBlank").click( function(){
    	location.href = '<c:url value="/meta/model/asisdmncdval_rqst.do" />';
    } );
...
function initGrid()
{
	...
	with(grid_sheet){
		...
		var headtext  = "<s:message code='META.HEADER.ASIS.DMN.CD.VAL.RQST'/>";
		//No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|시스템명|시스템구분|업무구분|ASISDB명|테이블명|컬럼명|코드명|참조테이블명|참조컬럼명|코드값|표시순번|코드값설명|응용팀|KB담당자|SI담당자|전환팀|전환담당자|최종수정일자|비고1|비고2|비고3|비고4|요청일시|요청자ID|요청자명|요청번호|요청일련번호
		var headers = [
		...
		var cols = [						
					...
					{Type:"Text",   Width:100,  SaveName:"asisDbNm",  Align:"Left", Edit:0, KeyField:1},
					{Type:"Text",   Width:100,  SaveName:"asisTblNm",  Align:"Left", Edit:0, KeyField:1},
					{Type:"Text",   Width:100,  SaveName:"asisColNm",  Align:"Left", Edit:0, KeyField:1},
					...
					{Type:"Text",   Width:100,  SaveName:"codeValue",  Align:"Left", Edit:0, KeyField:1},
...
function doAction(sAction)
{
	...
	switch(sAction)
	{
		...
		case "AddWam": //기존  추가
			var url   = "<c:url value="/meta/model/popup/asisdmncdvalpop.do"/>";
			var param = "popRqst=Y";
...
// 	    	if(DelJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/model/delasisdmncdvalrqstlist.do"/>";
...
// 			if(ibsSaveJson.data.length == 0) return;
			...
			//var url = "<c:url value="/meta/model/regitemrqstlist.do"/>";
			var url = "<c:url value="/meta/model/regasisdmncdvalrqstlist.do"/>";
		...
		case "Search":
			var param = $("#mstFrm").serialize();
			grid_sheet.DoSearch("<c:url value="/meta/model/getasisdmncdvalrqstlist.do" />", param);
...
// 			if(saveJson.data.length == 0) return;
			...
			var url = "<c:url value="/meta/model/approveAsisDmnCdVal.do"/>";
			var param = $('form[name=mstFrm]').serialize();
			IBSpostJson2(url, saveJson, param, ibscallback);
...
// 			grid_sheet.LoadExcel({Mode:'HeaderMatch'});
			var url = "<c:url value="/meta/model/popup/asisdmncdval_xls.do" />";
...
// 	alert( JSON.stringify(param));
	$('div#detailInfo').load('<c:url value="/meta/model/ajaxgrid/asisdmncdval_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/model/asisdmncdval_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
// 	var url = "<c:url value="/meta/model/regsditmrqstlist.do"/>";
...
function grid_sheet_OnClick(row, col, value, cellx, celly) {
	...
	//선택한 그리드의 row 내용을 보여준다.....
// 	var tmphtml = '표준용어 : ' + param.msgCd + ' [' + param.msgConts +']';
...
<div class="menu_subject">
	<div class="tab">
	<div class="menu_title">유효값등록</div>
