...
//             	showMsgBox("CNF", "선택한 테이블에 속한 컬럼도 삭제됩니다.<br>삭제 하시겠습니까?", "Delete");
        ...
        $("#btnBlank").click( function(){
    		location.href = '<c:url value="/meta/model/das_pdmtbl_rqst.do" />';
        } );
...
//         create_selectbox(sysareaJson, $("#sysAreaId"));
        ...
        if( "${waqMstr.rqstStepCd}" == "S" || "${waqMstr.rqstStepCd}" == "N") {
         
        	$("#btnMartSch").show();
        }	         
        
        $("#btnMartSch").click(function(){
...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        //headtext += "No.|상태|선택|검토상태|검토내용|요청구분|등록유형|검증결과|주제영역|테이블ID|테이블(물리명)|테이블(논리명)|모델논리명|상위주제영역논리명|주제영역ID|표준적용여부|파티션테이블여부|정보계대상여부|파기대상여부|CC반영여부|GA반영여부|VOC반영여부|EP반영여부|담당자ID|담당자명|테이블유형|설명|요청일시|요청자ID|요청자명|요청번호|요청일련번호";
        
        var headtext  = "<s:message code='META.HEADER.R7.PDMTBL.RQST.1'/>";
        	headtext += "<s:message code='META.HEADER.R7.PDMTBL.RQST.2'/>"; 
        	headtext += "<s:message code='META.HEADER.R7.PDMTBL.RQST.3'/>";
        	headtext += "<s:message code='META.HEADER.R7.PDMTBL.RQST.4'/>";
        ...
        var cols = [                        
					...
					{Type:"Combo",  Width:90,   SaveName:"rvwStsCd",	Align:"Center", Edit:0, Hidden:1},						
					{Type:"Text",   Width:90,   SaveName:"rvwConts",	Align:"Left", Edit:0, Hidden:1},						
					{Type:"Combo",  Width:90,   SaveName:"rqstDcd",	Align:"Center", Edit:0},						
					{Type:"Combo",  Width:90,   SaveName:"regTypCd",	Align:"Center", Edit:0},						
					{Type:"Combo",  Width:90,   SaveName:"vrfCd",	Align:"Center", Edit:0},
		            ...
		            {Type:"Text",   Width:60,   SaveName:"crgUserId",	 Align:"Left", Edit:0, Hidden:0},
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "New":        //신규 추가...
        	...
        	var url   = "<c:url value="/meta/model/popup/das_pdmtblSearchPop.do"/>";
			var param = "popRqst=Y";
			//var popup = OpenWindow(url+param,"pdmtblSearch","800","600","yes");
			
			openLayerPop(url, 850, 650, param);
...
// 			if(saveJson.data.length == 0) return;
        ...
        case "Down2Excel":  //엑셀내려받기
            grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'DA#연계모델등록요청'});
            break;
	...
	$('div#detailInfo').load('<c:url value="/meta/model/ajaxgrid/pdmtbl_rqst_dtl.do"/>', param, function( response, status, xhr ) {
		  if ( status == "error" ) {
		    ...
		    alert( msg + xhr.status + " " + xhr.statusText );
			  ...
			  $("#frmInput #pdmTblPnm").attr("disabled", true);
		      $("#frmInput #pdmTblLnm").attr("disabled", true);
		      $("#frmInput #objDescn").attr("disabled", true);
		      
		      $("#frmInput #subjSearchPop").hide();
		  }
	});
}
...
//	    		$("form#frmSearch input[name=rqstSno]").val(res.ETC.rqstSno);
		...
		case "<%=WiseMetaConfig.RqstAction.APPROVE%>":
			var url = '<c:url value="/meta/model/das_pdmtbl_rqst.do" />';
			var param = $('form[name=mstFrm]').serialize();
...
function grid_sheet_OnSaveEnd(code, message) {
	//alert(code);
	if (code == 0) {
		showMsgBox("INF", "저장 성공했습니다.");
	} else {
		showMsgBox("ERR", "저장 실패했습니다.");
	}
}
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1"><s:message code="TBL.DFNT.P" /></a></li> <!--테이블정의  -->
	    ...
	    <c:if test="${waqMstr.rqstStepCd == 'N' or waqMstr.rqstStepCd == 'S'  or waqMstr.rqstStepCd == 'Q'}">
		    ...
		    <li id="tabs-rqstchg"><a href="#tabs-5" id="colinfo"><s:message code="CLMN.CHG.ITEM" /></a></li> 	<!-- 컬럼변경항목  -->    
	    </c:if>
	  </ul>
	  <div id="tabs-1">
		<!-- 	상세정보 ajax 로드시 이용 -->
