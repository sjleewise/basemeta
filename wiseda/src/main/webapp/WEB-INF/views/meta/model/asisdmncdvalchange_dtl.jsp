...
//     	grid_sub_dmncdvalchange.Down2Excel({HiddenColumn:1});
    });
...
// 	grid_sub_dmncdvalchange.FitColWidth();
});


function initsubgrid_dmncdvalchange() {

    with(grid_sub_dmncdvalchange){
...
//     	var cfg = {SearchMode:2,Page:100};
        ...
        SetMergeSheet(1);
        var headers = [
                    {Text:"<s:message code='META.HEADER.ASIS.DMN.CD.VAL.CHG.DTL'/>", Align:"Center"}
                ];
        //No.|ASIS유효값ID|만료일자|시작일자|시스템명|시스템구분|업무구분|ASISDB명|테이블명|컬럼명|코드명|참조테이블명|참조컬럼명|코드값|표시순번|코드값설명|응용팀|KB담당자|SI담당자|전환팀|전환담당자|최종수정일자|비고1|비고2|비고3|비고4|요청번호|요청일련번호|설명|버전|등록유형|최초요청일시|최초요청사용자ID|요청일시|요청사용자ID|승인일시|승인사용자ID|비고
...
//         FitColWidth();
   	...
   	//==시트설정 후 아래에 와야함=== 
    init_sheet(grid_sub_dmncdvalchange);    
    //===========================
  	
    //저장 처리 과정을 디버깅 메시지를 팝업으로 표시 (-1)
//     grid_sub_dmncdvalchange.ShowDebugMsg(-1);	
...
y : y좌표
*/
function grid_sub_dmncdvalchange_OnDblClick(row, col, value, cellx, celly) {

	if(row < 1) return;
	
}

function grid_sub_dmncdvalchange_OnClick(row, col, value, cellx, celly) {
...
function grid_sub_dmncdvalchange_OnSearchEnd(code, message, stCode, stMsg) {
	if (stCode == 401) {
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<!-- 그리드 입력 입력 -->
	<div class="grid_01">
	     <script type="text/javascript">createIBSheet("grid_sub_dmncdvalchange", "100%", "150px");</script>
	</div>
	<!-- 그리드 입력 입력 End -->
