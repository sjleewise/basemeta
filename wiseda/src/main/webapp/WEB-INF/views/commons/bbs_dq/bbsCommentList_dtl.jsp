...
function updt_commentList() {
	...
	form.action = "<c:url value='/commons/bbs_dq/updateArticleComment.do'/>";
	form.submit();
}
...
function select_commentForupdt(commentNo) {
	...
	form.action = "<c:url value='/commons/bbs_dq/selectBoardArticle.do'/>";
	form.submit();
}
...
function delete_commentList(commentNo) {
	...
	form.modified.value = "true";
	form.commentNo.value = commentNo;
	form.action = "<c:url value='/commons/bbs_dq/deleteArticleComment.do'/>";
	form.submit();
}
...
<%--			<c:if test="${fn:length(resultList) == 0}"> --%>
	...
	<div>
	<form:form name="cmtFrm" commandName="articleCommentVO" action="${pageContext.request.contextPath}/commons/bbs_dq/insertArticleComment.do" method="post" onSubmit="insert_commentList(); return false;" style="float:left; clear:both;">
