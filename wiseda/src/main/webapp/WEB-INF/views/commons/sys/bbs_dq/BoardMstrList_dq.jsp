...
// 		}
    ...
    $("#btnExcelLoad").click( function(){ 
		...
		var url = '<c:url value="/commons/sys/bbs_dq/popup/bbs_xls.do"/>';
...
// 		alert($("#searchWrd").val());
		grid_sheet.DoSearch('<c:url value="/commons/sys/bbs_dq/selectAllBoardMstrList.do" />', param);
		break;
	...
	case 'Delete' : //삭제
    	...
    	//url 호출
		var urls = '<c:url value="/commons/sys/bbs_dq/deleteBBSMasterInf.do"/>';
...
//     	$.postJSON(url, DelJson, ibscallback);
	...
	case 'SaveRow': //단건 저장
		//saction (I-입력, U-수정)
		var urls = '<c:url value="/commons/sys/bbs_dq/ajaxgrid/insertBBSMastetInf.do"/>';
...
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/commons/sys/bbs_dq/ajaxgrid/selectBBSMasterInf.do"/>', param, function(){
		//$('#tabs').show();
