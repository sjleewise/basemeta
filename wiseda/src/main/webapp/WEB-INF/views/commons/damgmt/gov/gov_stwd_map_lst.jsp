...
function initGrid()
{
    ...
    with(grid_sheet){
        ...
        var headers = [
                    {Text:"No.|상태|선택|공통표준단어ID|제정차수|공통표준단어명|공통표준단어영문약어명|공통표준단어 영문명|공통표준단어 설명|형식단어여부|공통표준도메인분류명|이음동의어 목록|금칙어 목록|기관표준단어ID|기관표준단어논리명|기관표준단어물리명|등록유형코드|등록일시|등록사용자ID|등록사용자명", Align:"Center"}
                ];
...
//                     {Type:"Combo",   Width:130,  SaveName:"stndAsrt",    Align:"Left", Edit:1, KeyField:1,UpdateEdit:0},
                    ...
                    {Type:"Text",   Width:130,  SaveName:"engMean",    Align:"Center", Edit:1},
                    {Type:"Text",   Width:400,  SaveName:"objDescn",    Align:"Left", Edit:1, KeyField:1},          
                    {Type:"Combo",   Width:80,  SaveName:"dmnYn",    Align:"Center", Edit:1},
                    {Type:"Text",   Width:200,  SaveName:"govDmngLnm",     Align:"Center", Edit:1},
                    {Type:"Text",   Width:200,  SaveName:"synonyms",     Align:"Center", Edit:1},
                    {Type:"Text",   Width:200,  SaveName:"prohibits",      Align:"Center",   Edit:1},
                    
                    {Type:"Text",   Width:200,  SaveName:"stwdId",    Align:"Left", Edit:0, Hidden:1},
                    {Type:"Text",   Width:200,  SaveName:"stwdLnm",    Align:"Center", Edit:1, KeyField:0},
...
function initDtlGrid() {
	initsubgrid_govstwdmapchange();
}

//상세정보호출
function loadDetail(param) {
	$('div#detailInfo').load('<c:url value="/commons/damgmt/gov/ajaxgrid/gov_stwd_map_dtl.do"/>', param, function(){
		if(!isBlankStr(param)) {
			initgrid_sub_govstwdmapchange();
			grid_sub_govstwdmapchange.DoSearch('<c:url value="/commons/damgmt/gov/ajaxgrid/govstwdmapchange_dtl.do" />', param);
		}
	});
...
function doAction(sAction)
{
    ...
    switch(sAction)
    {
        case "Search":
			...
			loadDetail();
			$('#gov_stwd_map_sel_title').html('');
			grid_sheet.DoSearch("<c:url value="/commons/damgmt/gov/getGovStwdMapList.do" />", param);
			//grid_sheet.DoSearchScript("testJsonlist");
        	break;
       
        case 'SaveRow': //단건 저장
    		//saction (I-입력, U-수정)
    		var urls = '<c:url value="/commons/damgmt/gov/saveGovStwdMapRow.do"/>';
        ...
        case 'Save' : //엑셀 업로드용 저장
        	...
        	var url = '<c:url value="/commons/damgmt/gov/saveGovStwdMapList.do"/>';
        ...
        case "Down2Excel":  //엑셀내려받기
            ...
            grid_sheet.Down2Excel({HiddenColumn:1, Merge:1, FileName:'공통표준단어매핑조회'});
		...
		case "Delete" :
        	...
        	var url = '<c:url value="/commons/damgmt/gov/deleteGovStwdMapList.do"/>';
        	var param = "";
        	IBSpostJson2(url, DelJson, param, ibscallback);
        	break;
            
        case 'Add' : //신규 추가
    		$('#gov_stwd_map_sel_title').html('단어를 클릭하시면 상세정보를 조회합니다.'); /* 유사어를 클릭하시면 상세정보를 조회합니다. */
    		loadDetail();
    		break;
    }       
}
...
function grid_sheet_OnClick(row, col, value, cellx, celly) {
	...
	var govStwdId = "&govStwdId="+grid_sheet.GetCellValue(row, "govStwdId");

	//선택한 그리드의 row 내용을 보여준다.....
	var tmphtml = '공통표준 단어 논리명 :'+ param.govStwdLnm; /* 유사어논리명 : */
	$('#gov_stwd_map_sel_title').html(tmphtml);
	
	//메뉴ID를 토대로 조회한다....
	loadDetail(govStwdId);
...
function grid_sheet_OnSearchEnd(code, message, stCode, stMsg) {
	...
	if (grid_sheet.SearchRows() == 1){
		...
		//선택한 그리드의 row 내용을 보여준다.....
		var tmphtml = '공통표준단어 : '+ param.govStwdLnm; /* 유사어논리명 : */
		$('#gov_stwd_map_sel_title').html(tmphtml);
		
		var govstwdId = "";
		govstwdId = grid_sheet.GetCellValue(1, "govStwdId");
		param = "govStwdId="+govstwdId;
		loadDetail(param);
	}
}
...
<div class="menu_subject">
	<div class="tab">
	    <div class="menu_title">공통표준 단어 매핑 관리</div> <!-- 유사어/금지어 관리 -->
	</div>
</div>
<!-- 메뉴 메인 제목 -->
...
<div id="search_div">
        ...
        <form id="frmSearch" name="frmSearch" method="post">
            ...
            <div class="tb_basic2">
                <table width="100%" border="0" cellspacing="0" cellpadding="0" summary="공통표준 단어 조회"> <!-- 표준단어조회 -->
...
<%-- 						         </select> --%>
<!-- 						         </td>                          -->
                                <th scope="row"><label for="govStwdLnm">공통표준 단어명</label></th> <!-- 유사어명 -->
                                <td colspan = "3"><input type="text" id="govStwdLnm" class="wd300" name="govStwdLnm" /></td>
                            </tr>
                            <tr>
...
<%-- 					                    </c:forEach> --%>
<%--                                     </select></td> --%>
                                <th scope="row"><label for="stwdLnm">기관표준 단어명</label></th> <!-- 대체어명 -->
                                <td colspan = "3"><input type="text" id="stwdLnm" class="wd300" name="stwdLnm" /></td>
                            </tr>
                            <tr>
...
<div style="clear:both; height:5px;"><span></span></div>
	...
	<div class="selected_title_area">
		    <div class="selected_title" id="gov_stwd_map_sel_title"> <span></span></div>
	</div>
	...
	<div id="tabs">
	  <ul>
	    <li><a href="#tabs-1">공통표준 단어 등록</a></li>
	  ...
	  <div id="tabs-1">
		...
		<div id="tabs-2">
			<%@include file="govstwdmapchange_dtl.jsp" %>
	  </div>
	 </div>
	<!-- 선택 레코드의 내용을 탭처리 END -->
