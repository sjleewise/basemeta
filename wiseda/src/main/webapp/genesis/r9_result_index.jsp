...
<%
	...
	if("1".equals(work)){
		...
		try{
			...
			list	= ERwin.getComboIndexList	(param	);
...
<table border=1 style="font-size:12">
	<tr style="font-size:13; color:blue">
		...
		<td>엔티티</td>
		<td>table</td>
<td>entity<br>Owner</td>
		<td>인덱스명</td>
		<td>Index</td>
		<td>Sort</td>
		<td>index<br>no</td>
		<td>index<br>Owner</td>
		<td>속성</td>
		<td>column</td>
...
%>
		<tr>
			<td><%= i+1 %></td><!--  index_id : <%= map.get("index_id")%> -->
			<td><input type=text style="font-size:11" size="20" readOnly size=20 value="<%= map.get("subjectArea_name") %>"></td>
			...
			<td><input type=text value="<%= map.get("index_name"			)%>" style="font-size:12" size="25" readOnly size=20></td>
			<td><input type=text value="<%= map.get("index_name_physical"	)%>" style="font-size:11" size="25" readOnly size=20></td>

			<td align="center"><%= map.get("key_group_sort_order")%></td>
			<td align="center"><%= map.get("keyGrop_order")%></td>
			<td align="center"><%= map.get("index_owner_name")%></td>
