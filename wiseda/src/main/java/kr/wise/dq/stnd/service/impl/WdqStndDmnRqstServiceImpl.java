...
import kr.wise.dq.stnd.service.WdqqCdValMapper;
 ...
 * </PRE>
 */
@Service("wdqStndDmnRqstService")
public class WdqStndDmnRqstServiceImpl implements WdqStndDmnRqstService {

	private final Logger logger = LoggerFactory.getLogger(getClass());

	@Inject
	private WdqqDmnMapper waqmapper;
	...
	@Inject
	private WdqqSditmMapper waqSditmMapper;
    ...
    @Inject
    private WdqqCdValMapper waqcdmapper;

	@Inject
	private WdqStndItemRqstService stndItemRqstService;
	...
	public int register(WaqMstr mstVo, List<?> reglist) throws Exception {
		...
		if(reglist != null) {
			for (WdqqDmn saveVo : (ArrayList<WdqqDmn>)reglist) {
	...
	public int saveRqstStndDmn(WdqqDmn saveVo) {
		int result  = 0;
...
//			saveVo.setStwdId(objid);
	...
	/** 단어구성정보 셋팅(삭제 후 추가) */
	public void setStwdCnfg(WdqqDmn saveVo) {
		//단어구성정보를 먼저 삭제한다.
...
//		waqmapper.checkDmnCriDsCnfg(checkmap);
		...
		checkmap2.put("tblnm", "WDQQ_CD_VAL");
...
//		boolean waq2wam = requestApproveService.setApproveProcess(mstVo, "WAQ_DMN");
	...
	public int regWaq2WamCdval(WaqMstr mstVo) throws Exception {
		...
		List<WdqqCdVal> list = waqcdmapper.selectWaqC(rqstno);
		for (WdqqCdVal savevo : list) {
	...
	public int regWaq2Wam(WaqMstr mstVo) throws Exception {
		...
		//신규 대상인 경우 ID 채번한다.
		List<WdqqDmn> waqclist =  waqmapper.selectWaqC(rqstno);

		for (WdqqDmn savevo : waqclist) {
			String id =  objectIdGnrService.getNextStringId();
			savevo.setDmnId(id);
		...
		WdqqStwdCnfgMapper.deleteWAM(rqstno);
		WdqqStwdCnfgMapper.insertWAM(rqstno);
		//
	...
	/** insomnia */
	public List<WdqqDmn> getInfotpList() {
		// TODO Auto-generated method stub
		return null;
	}

	/** 표준도메인 요청 상세정보 조회 insomnia */
	public WdqqDmn getStndDmnRqstDetail(WdqqDmn searchVo) {

		return waqmapper.selectDomainRqstDetail(searchVo);
	}

	/** insomnia */
	public Map<String, String> getDmnStwdInfo(WdqqDmn record) {
		...
		for(int i=0;i < arrSepDmnLnm.length;i++) {
			List<WdqqStwd> list = WdqqStwdMapper.selectListByStwdLnm(record.getRqstNo(), arrSepDmnLnm[i],stndAsrt);
			if(list.size() > 1) {
...
//				pnmCriDs += ";"+list.get(0).getStwdPnm();
	...
	/** insomnia */
	public List<WdqpDvCanAsm> getDmnDivisionlist(WdqpDvCanDic record) throws Exception {
		int execCnt = 0;
	...
	/** 도메인 자동분할 리스트스  */
	public List<WdqpDvCanAsm> getDmnDvRqstList(WdqpDvCanDic record) {
		//분할정보 조회
		List<WdqpDvCanAsm> list = WdqpDvCanAsmMapper.selectDmnDvRqstList(record);
		return list;
	}
	
	
	/** insomnia */
	public Map<String, String>  regDmnAutoDiv(List<WdqpDvCanAsm> list)  throws Exception {
...
//		WdqpDvCanAsmMapper.deleteDvCanAsmByDvRqstNo(record.getDvRqstNo());
		...
		//도메인 기존재
		WdqpDvCanDicMapper.checkDmnDup(dvrqstno);
		//분할 결과 검증
		//도메인그룹ID , INFOTYPE ID UPDATE
		WdqpDvCanDicMapper.updtDmngId(dvrqstno);
		WdqpDvCanDicMapper.updtInfotpId(dvrqstno);
		//도메인그룹 미존재
		WdqpDvCanDicMapper.checkDmngId(dvrqstno);
		//인포타입 미존재
		WdqpDvCanDicMapper.checkInfotpId(dvrqstno);
		//도메인그룹 인포타입 매핑 상이
		WdqpDvCanDicMapper.checkDmngInfotyp(dvrqstno);
		//구성정보 오류
	 ...
	 * @throws Exception */
	public Map<String, String> delDmnAutoDiv(List<WdqpDvCanAsm> list) throws Exception {
		Map<String, String> resultMap =  new HashMap<String, String>();
		int rtnCnt = 0;
		String dvrqstno = "";
		for (WdqpDvCanAsm savevo : list) {
			dvrqstno = savevo.getDvRqstNo();
			rtnCnt += WdqpDvCanAsmMapper.delDmnAutoDiv(savevo);
		}
	...
	public int regStndDmnByDiv(WaqMstr mstVo, ArrayList<WdqpDvCanAsm> list){
		...
		if(list != null) {
			for (WdqpDvCanAsm WdqpDvCanAsmVo : (ArrayList<WdqpDvCanAsm>)list) {
				...
				if(WdqpDvCanAsmVo.getIbsCheck().equals("1") ){
					...
					saveVo.setDmnLnm(WdqpDvCanAsmVo.getDvOrgLnm());
					saveVo.setDmnPnm(WdqpDvCanAsmVo.getDicAsmPnm());
//					saveVo.setLnmCriDs( WdqpDvCanAsmVo.getDicAsmDsLnm().replaceAll(";","_"));
					...
					saveVo.setDmngId(WdqpDvCanAsmVo.getDmngId());
					...
					saveVo.setSditmAutoCrtYn(WdqpDvCanAsmVo.getSditmAutoCrtYn());
					saveVo.setCdValTypCd(WdqpDvCanAsmVo.getCdValTypCd());
					saveVo.setCdValIvwCd(WdqpDvCanAsmVo.getCdValIvwCd()); 
					saveVo.setFullPath(WdqpDvCanAsmVo.getFullPath()); 
...
//		delResult += WdqpDvCanAsmMapper.deleteDvCanAsmByDvRqstNo(dvRqstNo);
	...
	/** insomnia */
	public List<WdqqDmn> getDmnRqstList(WaqMstr search) {
		return waqmapper.selectDmnRqstListbyMst(search);
	}

	/** insomnia */
	public int delList(WaqMstr reqmst, ArrayList<WdqqDmn> list) {
	...
	/** insomnia
	 * @throws Exception */
	public int regWam2Waq(WaqMstr reqmst, ArrayList<WdqqDmn> list) throws Exception {
		int result = 0;

		//WAM에서 WAQ에 적재할 내용을 가져온다.
		ArrayList<WdqqDmn> wamlist = waqmapper.selectwamlist(reqmst, list);

		//WAM_DMN 2 WAQ_DMN 적재(한번에 적재)
//		result = waqmapper.insertwam2waq(reqmst, list);

		//단어구성정보 적재(wam에 있는 내용을 적재하려고 했으나, 현재 wam에 단어구성정보가 없음.)
//		WdqqStwdCnfgMapper.insertwam2waq(reqmst, list);
...
//		result += waqcdmapper.insertwam2waq(reqmst, wamlist);
	...
	/** insomnia
	 * @throws Exception */
	public int delStndDmnRqst(WaqMstr reqmst, ArrayList<WdqqDmn> list) throws Exception {
		int result = 0;

		//TODO 성능이 문제가 될시에 한방 SQL로 처리한다.
//		result = waqmapper.deletedmnrqst(reqmst, list);

		for (WdqqDmn savevo : list) {
	...
	public List<WdqqCdVal> getCdvalRqstList(WaqMstr search) {

		return waqcdmapper.selectCdvalRqstVal(search) ;
	}

	/** insomnia */
	public int regCdvalList(WaqMstr reqmst, List<WdqqCdVal> list) {
		int result = 0;
		...
		for (WdqqCdVal savevo : list) {
			...
			result += waqmapper.insertWdqqDmnByCdVal(savevo); 
	...
	/** @param savevo
	/** @return insomnia */
	private int saveCdvalRqst(WdqqCdVal savevo) {
		int result = 0;
		...
		if("I".equals(tmpStatus)) {
			...
			List<WdqqDmn> dmnlist = waqmapper.selectListByCdval(savevo);
	...
	/** insomnia */
	public int delCdvalList(WaqMstr reqmst, List<WdqqCdVal> list) {
		int result = 0;

		//여러 도메인에 속한 리스트를 지우려면 성능이 떨어져도 건별 처리하자....
		for (WdqqCdVal delvo : list) {
	...
	/** insomnia */
	public WdqqDmn getDmnInfoByDmnLnm(WdqqDmn data) {
		return waqmapper.selectByDmnLnm(data);
	}

	/** insomnia */
	public int regapprove(WaqMstr mstVo, List<WdqqDmn> reglist) {
		int result = 0;
	...
	//도메인 신청내역이 있는지 체크
    public int checkExistsWdqqDmn(WaqMstr reqmst){
    
    	List<WdqqDmn> list = waqmapper.selectExistsDmnCheck(reqmst);
	...
	@Override
	public List<WdqqDmn> getDomainCdVal(WdqqDmn data) {
