...
public class MysqlPC05Sql {
	...
	public String getErrorValidPatternCondSql()  {
		...
		strQuery.append("\n               ,REPLACE(TRAN_DATA1,TRAN_DATA2, LPAD('C',TRAN_DATA2_LENGTH, 'C')) AS PATTERN ");
		strQuery.append("\n           FROM (SELECT "+dbcColNm );
		strQuery.append("\n                        ,TRAN_DATA1 ");
		strQuery.append("\n                        ,NVL(REPLACE(REPLACE(TRAN_DATA1,'"+G_TRANS_STRING+"', LPAD(' ', "+G_TRANS_STRING.length()+",' ')), ' ', ''),'C') AS TRAN_DATA2 ");
		strQuery.append("\n                        ,LENGTH(NVL(REPLACE(REPLACE(TRAN_DATA1,'"+G_TRANS_STRING+"', LPAD(' ', "+G_TRANS_STRING.length()+",' ')), ' ', ''),'C')) AS TRAN_DATA2_LENGTH ");
		strQuery.append("\n                   FROM (SELECT "+dbcColNm);
		strQuery.append("\n                               ,REPLACE("+dbcColNm+",'"+G_FROM_STRING+"' ");
...
//		}
		//사용자 정의 존재할 경우 
				if( 0 < prfDtlVO.size() ){
					...
					for(WamPrfPtrAnaVO patVO : prfDtlVO ){
						if(isFirst && patVO.getUserDfPtr().indexOf("^") != 0){
							strQuery.append("\n  WHERE RTRIM(LTRIM(PATTERN)) NOT IN (");
							strQuery.append("'"+patVO.getUserDfPtr()+"'");
							isFirst = false;
						}else if(patVO.getUserDfPtr().indexOf("^") != 0){
							strQuery.append(", '"+patVO.getUserDfPtr()+"'");
						}
						
						i++;
//						if(i == prfDtlVO.size()) strQuery.append(" )");
					}
				...
				if( 0 < prfDtlVO.size() ){
					for(WamPrfPtrAnaVO patVO : prfDtlVO ){
						if(isFirst && patVO.getUserDfPtr().indexOf("^") == 0){
							strQuery.append("\n  WHERE " + dbcColNm + " NOT REGEXP '"+patVO.getUserDfPtr()+"'");
//							strQuery.append(dbcColNm + ", '"+patVO.getUserDfPtr()+"'");
//							strQuery.append(" )");
							isFirst = false;
						} else if(!isFirst && patVO.getUserDfPtr().indexOf("^") == 0) {
							strQuery.append("\n  AND " + dbcColNm + " NOT REGEXP '"+patVO.getUserDfPtr()+"'");
//							strQuery.append("\n  AND NOT REGEXP(");
//							strQuery.append(dbcColNm + ", '"+patVO.getUserDfPtr()+"'");
//							strQuery.append(" )");
						}
