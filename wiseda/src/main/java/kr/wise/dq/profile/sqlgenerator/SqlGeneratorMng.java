...
public class SqlGeneratorMng {
	...
	public SqlGeneratorVO getSql(Map<String, Object> sqlGenMap)  {
		...
		//CUBRID
			...
			CubridSqlGenerator cubridSqlGen = new CubridSqlGenerator(); 
			sqlVO = cubridSqlGen.getSql(sqlGenMap); 
		}
		else if(dbmsTypCd.equals("DB2")){
			Db2SqlGenerator db2SqlGen = new Db2SqlGenerator(); 
			sqlVO = db2SqlGen.getSql(sqlGenMap); 
		}
		//sybaseIQ	
