...
public class PostPC05Sql {
	...
	public PostPC05Sql(Map<String, Object> sqlGenMap){
	...
	public String getErrorValidPatternCondSql()  {
		...
		strQuery.append("\n           FROM (SELECT "+dbcColNm );
		strQuery.append("\n                        ,TRAN_DATA1 ");
		strQuery.append("\n                        ,coalesce(REPLACE(TRANSLATE(TRAN_DATA1,'"+G_TRANS_STRING+"', LPAD(' ', "+G_TRANS_STRING.length()+",' ')), ' ', ''),'C') AS TRAN_DATA2 ");
		strQuery.append("\n                        ,LENGTH(coalesce(REPLACE(TRANSLATE(TRAN_DATA1,'"+G_TRANS_STRING+"', LPAD(' ', "+G_TRANS_STRING.length()+",' ')), ' ', ''),'C')) AS TRAN_DATA2_LENGTH ");
		strQuery.append("\n                   FROM (SELECT "+dbcColNm);
		...
		if( 0 < prfDtlVO.size() ){
			...
			for(WamPrfPtrAnaVO patVO : prfDtlVO ){
				...
				i++;
				if(i == prfDtlVO.size() && patVO.getUserDfPtr().indexOf("^") != 0) strQuery.append(" )");
			}
