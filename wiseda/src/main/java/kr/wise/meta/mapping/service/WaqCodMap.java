package kr.wise.meta.mapping.service;


import kr.wise.commons.cmm.CommonVo;

public class WaqCodMap extends CommonVo {

    private String cdMapId;

    private String mapDfType;

    private String cdCnvsType;

    private String cdMapType;

    private String tgtDmnId;

    private String tgtDmnPnm;

    private String tgtDmnLnm;

    private String tgtCdVal;

    private String tgtCdValNm;

    private String srcDbId;

    private String srcDbPnm;

    private String srcTblId;

    private String srcTblLnm;

    private String srcTblPnm;

    private String srcColLnm;

    private String srcColPnm;

    private String srcCdVal;

    private String srcCdValNm;
    
    private String tgtDmngLnm;
    
    private String tgtCdValTypCd;
    
    private String tgtCdValIvwCd;
    
    private String tgtDataType;
    
    private String tgtDataLen;
    
    private String srcBizId;
    private String srcBizNm;
    private String srcSysId;
    private String srcSysNm;
    private String srcUppCdVal;
    private String srcUppCdValNm;
    private String appCrgpId;
    private String appCrgpNm;
    private String cnvsCrgpId;
    private String cnvsCrgpNm;
    
    private String srcCdDmnCd;


    public String getCdMapId() {
		return cdMapId;
	}

	public void setCdMapId(String cdMapId) {
		this.cdMapId = cdMapId;
	}

	public String getMapDfType() {
        return mapDfType;
    }

    public void setMapDfType(String mapDfType) {
        this.mapDfType = mapDfType;
    }

    public String getCdCnvsType() {
        return cdCnvsType;
    }

    public void setCdCnvsType(String cdCnvsType) {
        this.cdCnvsType = cdCnvsType;
    }

    public String getCdMapType() {
        return cdMapType;
    }

    public void setCdMapType(String cdMapType) {
        this.cdMapType = cdMapType;
    }

    public String getTgtDmnId() {
        return tgtDmnId;
    }

    public void setTgtDmnId(String tgtDmnId) {
        this.tgtDmnId = tgtDmnId;
    }

    public String getTgtDmnPnm() {
        return tgtDmnPnm;
    }

    public void setTgtDmnPnm(String tgtDmnPnm) {
        this.tgtDmnPnm = tgtDmnPnm;
    }

    public String getTgtDmnLnm() {
        return tgtDmnLnm;
    }

    public void setTgtDmnLnm(String tgtDmnLnm) {
        this.tgtDmnLnm = tgtDmnLnm;
    }

    public String getTgtCdVal() {
        return tgtCdVal;
    }

    public void setTgtCdVal(String tgtCdVal) {
        this.tgtCdVal = tgtCdVal;
    }

    public String getTgtCdValNm() {
        return tgtCdValNm;
    }

    public void setTgtCdValNm(String tgtCdValNm) {
        this.tgtCdValNm = tgtCdValNm;
    }

    public String getSrcDbId() {
        return srcDbId;
    }

    public void setSrcDbId(String srcDbId) {
        this.srcDbId = srcDbId;
    }

    public String getSrcDbPnm() {
        return srcDbPnm;
    }

    public void setSrcDbPnm(String srcDbPnm) {
        this.srcDbPnm = srcDbPnm;
    }

    public String getSrcTblId() {
        return srcTblId;
    }

    public void setSrcTblId(String srcTblId) {
        this.srcTblId = srcTblId;
    }

    public String getSrcTblLnm() {
        return srcTblLnm;
    }

    public void setSrcTblLnm(String srcTblLnm) {
        this.srcTblLnm = srcTblLnm;
    }

    public String getSrcTblPnm() {
        return srcTblPnm;
    }

    public void setSrcTblPnm(String srcTblPnm) {
        this.srcTblPnm = srcTblPnm;
    }

    public String getSrcColLnm() {
        return srcColLnm;
    }

    public void setSrcColLnm(String srcColLnm) {
        this.srcColLnm = srcColLnm;
    }

    public String getSrcColPnm() {
        return srcColPnm;
    }

    public void setSrcColPnm(String srcColPnm) {
        this.srcColPnm = srcColPnm;
    }

    public String getSrcCdVal() {
        return srcCdVal;
    }

    public void setSrcCdVal(String srcCdVal) {
        this.srcCdVal = srcCdVal;
    }

    public String getSrcCdValNm() {
        return srcCdValNm;
    }

    public void setSrcCdValNm(String srcCdValNm) {
        this.srcCdValNm = srcCdValNm;
    }
    
    

	public String getTgtDmngLnm() {
		return tgtDmngLnm;
	}

	public void setTgtDmngLnm(String tgtDmngLnm) {
		this.tgtDmngLnm = tgtDmngLnm;
	}

	public String getTgtCdValTypCd() {
		return tgtCdValTypCd;
	}

	public void setTgtCdValTypCd(String tgtCdValTypCd) {
		this.tgtCdValTypCd = tgtCdValTypCd;
	}

	public String getTgtCdValIvwCd() {
		return tgtCdValIvwCd;
	}

	public void setTgtCdValIvwCd(String tgtCdValIvwCd) {
		this.tgtCdValIvwCd = tgtCdValIvwCd;
	}

	public String getTgtDataType() {
		return tgtDataType;
	}

	public void setTgtDataType(String tgtDataType) {
		this.tgtDataType = tgtDataType;
	}

	public String getTgtDataLen() {
		return tgtDataLen;
	}

	public void setTgtDataLen(String tgtDataLen) {
		this.tgtDataLen = tgtDataLen;
	}

	public String getSrcBizId() {
		return srcBizId;
	}

	public void setSrcBizId(String srcBizId) {
		this.srcBizId = srcBizId;
	}

	public String getSrcBizNm() {
		return srcBizNm;
	}

	public void setSrcBizNm(String srcBizNm) {
		this.srcBizNm = srcBizNm;
	}

	public String getSrcSysId() {
		return srcSysId;
	}

	public void setSrcSysId(String srcSysId) {
		this.srcSysId = srcSysId;
	}

	public String getSrcSysNm() {
		return srcSysNm;
	}

	public void setSrcSysNm(String srcSysNm) {
		this.srcSysNm = srcSysNm;
	}

	public String getSrcUppCdVal() {
		return srcUppCdVal;
	}

	public void setSrcUppCdVal(String srcUppCdVal) {
		this.srcUppCdVal = srcUppCdVal;
	}

	public String getSrcUppCdValNm() {
		return srcUppCdValNm;
	}

	public void setSrcUppCdValNm(String srcUppCdValNm) {
		this.srcUppCdValNm = srcUppCdValNm;
	}

	public String getAppCrgpId() {
		return appCrgpId;
	}

	public void setAppCrgpId(String appCrgpId) {
		this.appCrgpId = appCrgpId;
	}

	public String getAppCrgpNm() {
		return appCrgpNm;
	}

	public void setAppCrgpNm(String appCrgpNm) {
		this.appCrgpNm = appCrgpNm;
	}

	public String getCnvsCrgpId() {
		return cnvsCrgpId;
	}

	public void setCnvsCrgpId(String cnvsCrgpId) {
		this.cnvsCrgpId = cnvsCrgpId;
	}

	public String getCnvsCrgpNm() {
		return cnvsCrgpNm;
	}

	public void setCnvsCrgpNm(String cnvsCrgpNm) {
		this.cnvsCrgpNm = cnvsCrgpNm;
	}

	public String getSrcCdDmnCd() {
		return srcCdDmnCd;
	}

	public void setSrcCdDmnCd(String srcCdDmnCd) {
		this.srcCdDmnCd = srcCdDmnCd;
	}

	@Override
	public String toString() {
		return "WaqCodMap [cdMapId=" + cdMapId + ", mapDfType=" + mapDfType
				+ ", cdCnvsType=" + cdCnvsType + ", cdMapType=" + cdMapType
				+ ", tgtDmnId=" + tgtDmnId + ", tgtDmnPnm=" + tgtDmnPnm
				+ ", tgtDmnLnm=" + tgtDmnLnm + ", tgtCdVal=" + tgtCdVal
				+ ", tgtCdValNm=" + tgtCdValNm + ", srcDbId=" + srcDbId
				+ ", srcDbPnm=" + srcDbPnm + ", srcTblId=" + srcTblId
				+ ", srcTblLnm=" + srcTblLnm + ", srcTblPnm=" + srcTblPnm
				+ ", srcColLnm=" + srcColLnm + ", srcColPnm=" + srcColPnm
				+ ", srcCdVal=" + srcCdVal + ", srcCdValNm=" + srcCdValNm
				+ ", tgtDmngLnm=" + tgtDmngLnm + ", tgtCdValTypCd="
				+ tgtCdValTypCd + ", tgtCdValIvwCd=" + tgtCdValIvwCd
				+ ", tgtDataType=" + tgtDataType + ", tgtDataLen=" + tgtDataLen
				+ ", srcBizId=" + srcBizId + ", srcBizNm=" + srcBizNm
				+ ", srcSysId=" + srcSysId + ", srcSysNm=" + srcSysNm
				+ ", srcUppCdVal=" + srcUppCdVal + ", srcUppCdValNm="
				+ srcUppCdValNm + ", appCrgpId=" + appCrgpId + ", appCrgpNm="
				+ appCrgpNm + ", cnvsCrgpId=" + cnvsCrgpId + ", cnvsCrgpNm="
				+ cnvsCrgpNm + ", srcCdDmnCd=" + srcCdDmnCd + "]";
	}
}