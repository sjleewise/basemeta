...
import kr.wise.meta.mapping.service.WaqTblMapMapper;
 ...
 * </PRE>
 */
@Service("tblMapRqstService")
public class TblMapRqstServiceImpl implements TblMapRqstService {

	private final Logger logger = LoggerFactory.getLogger(getClass());

	@Inject
	private WaqTblMapMapper waqmapper;
	
	@Inject
	private WamTblMapMapper wammapper;
	...
	public int register(WaqMstr mstVo, List<?> reglist) throws Exception {
		...
		if(reglist != null) {
			for (WaqTblMap saveVo : (ArrayList<WaqTblMap>)reglist) {
				//요청번호 셋팅
...
//				saveVo.setAppCrgpNm(username); //응용담당자이름
	...
	/** 테이블매핑정의서 단건 저장... @return 유성열 */
	private int saveTblMapRqst(WaqTblMap saveVo) {
		int result  = 0;
...
//		waqRqstVrfDtls.setBizDtlCd("TBLMAP");
		...
		//매핑정의서ID 업데이트(타겟테이블물리명+'_'+매핑일련번호)
		waqmapper.updateMapDfId(rqstNo);
		...
		***************************/
		...
		//테이블매핑유형이 NEW, INITIAL인경우 소스(ASIS)정보 공백으로 업데이트
		...
		//요청서내 중복자료 검증(TM001)
		checkmap.put("vrfDtlCd", "TM001");
		waqmapper.checkDupTblMap(checkmap);

		//삭제일때 미존재 테이블매핑정의서 체크(TM002)
		checkmap.put("vrfDtlCd", "TM002");
		waqmapper.checkNotExistTblMap(checkmap);

		//요청중인 테이블매핑정의서 (TM003)
		checkmap.put("vrfDtlCd", "TM003");
		waqmapper.checkRequestTblMap(checkmap);
		
		//미존재 타겟테이블(TM004)
		checkmap.put("vrfDtlCd", "TM004");
		waqmapper.checkNotExistTgtTbl(checkmap);
		
		//응용담당자외 등록,수정,삭제불가(TM005)
		checkmap.put("vrfDtlCd", "TM005");
//		waqmapper.checkAppCrgp(checkmap);
		
		//미존재 소스테이블(TM006)
		checkmap.put("vrfDtlCd", "TM006");
		//waqmapper.checkNotExistSrcTbl(checkmap);

		//삭제일때 컬럼매핑정의서 체크(TM007)
		checkmap.put("vrfDtlCd", "TM007");
		waqmapper.checkNotExistColMap(checkmap);
		
		//REQ_TYP_CD이 변경일 때 변경된 데이터가 없을 경우(TM000)
		checkmap.put("vrfDtlCd", "TM000");
...
//
//	/** 테이블매핑정의서 요청서 등록요청 유성열 */
	public int submit(WaqMstr mstVo) {
//		// TODO Auto-generated method stub
		return 0;
	}
//
//	/** 테이블매핑정의서 요청서 승인 처리 유성열 */
...
//		boolean waq2wam = requestApproveService.setApproveProcess(mstVo, "WAQ_DMN");
	...
	private int regWaq2Wam(WaqMstr mstVo) throws Exception {
		...
		List<WaqTblMap> waqclist = waqmapper.selectWaqC(rqstno);

		for (WaqTblMap savevo : waqclist) {
	...
	/** 테이블매핑정의서 요청서 상세정보 조회 유성열 */
	public WaqTblMap getTblMapRqstDetail(WaqTblMap searchVo) {
		return waqmapper.selectTblMapDetail(searchVo);
	}

	/** 테이블매핑정의서 요청서 정보 조회 유성열 */
	public List<WaqTblMap> getTblMapRqstList(WaqMstr search) {
		return waqmapper.selectTblMapListbyMst(search);
	}
	
	/** 테이블매핑정의서 타겟테이블 정보 조회 유성열 */
	public List<WaqTblMap> getTgtTblList(WaqTblMap search) {
		return waqmapper.selectTgtTblListbyNm(search);
	}
	
	/** 테이블매핑정의서 변경대상리스트 정보 조회 유성열 */
	public List<WamTblMap> getTblMapList(WamTblMap search) {
		return wammapper.selectTblMapList(search);
	}

	/** 테이블매핑정의서 요청서 삭제 유성열
	 * @throws Exception */
	public int delTblMapRqst(WaqMstr reqmst, ArrayList<WaqTblMap> list) throws Exception {
		int result = 0;

		//TODO 성능이 문제가 될시에 한방 SQL로 처리한다.
//		result = waqmapper.deletedmnrqst(reqmst, list);

		for (WaqTblMap savevo : list) {
	...
	/** 유성열
	 * @throws Exception */
	public int regWam2Waq(WaqMstr reqmst, ArrayList<WaqTblMap> list) throws Exception {
		int result = 0;

		List<WaqTblMap> wamlist = waqmapper.selectwamlist(reqmst, list);
