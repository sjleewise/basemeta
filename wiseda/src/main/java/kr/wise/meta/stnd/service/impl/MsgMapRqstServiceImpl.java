...
import kr.wise.meta.stnd.service.WaqErrMsgMapMapper;
 ...
 * </PRE>
 */
@Service("msgMapRqstService")
public class MsgMapRqstServiceImpl implements MsgMapRqstService {

	private final Logger logger = LoggerFactory.getLogger(getClass());

	@Inject
	private WaqErrMsgMapMapper waqmapper;
	...
	public int register(WaqMstr mstVo, List<?> reglist) throws Exception {
		...
		if(reglist != null) {
			for (WaqErrMsgMap saveVo : (ArrayList<WaqErrMsgMap>)reglist) {
				//요청번호 셋팅
...
//					saveVo.setSysDivCd("M");
	...
	/** @return  */
	public int saveRqstMsg(WaqErrMsgMap saveVo) {
		int result = 0;
		try{
	...
	/** 메시지 요청 상세내용 조회  */
	public WaqErrMsgMap getMsgRqstDetail(WaqErrMsgMap searchVo) {
	...
	/** 메시지 요청 리스트 조회  */
	public List<WaqErrMsgMap> getMsgRqstList(WaqMstr search) {
		return waqmapper.selectMsgRqstListbyMst(search);
	}
	
		/** 메시지 삭제(waq) 
	 * @throws Exception */
	public int delMsgRqstList(WaqMstr reqmst, ArrayList<WaqErrMsgMap> list) throws Exception {
		int result = 0;
		//TODO 성능 문제 발생시 한방 SQL로 처리한다.
		//result = waqmapper.deleteitemrqst(reqmst, list);
		for (WaqErrMsgMap savevo : list) {
	...
	public int check(WaqMstr mstVo) {
		...
		waqRqstVrfDtls.setBizDtlCd("OEM");
...
//		boolean waq2wam = requestApproveService.setApproveProcess(mstVo, "WAQ_DMN");
	...
	public int regWaq2Wam(WaqMstr mstVo) throws Exception {
		...
		//신규 대상인 경우 ID를 채번한다.
		List<WaqErrMsgMap> waqclist = waqmapper.selectWaqC(rqstno);
		for (WaqErrMsgMap savevo : waqclist) {
			String id =  objectIdGnrService.getNextStringId();
			savevo.setMsgMapId(id);
...
//		result += waqmapper.updateUppDmnId(rqstno);
	...
	public int regWam2Waq(WaqMstr reqmst, List<WaqErrMsgMap> list) throws Exception {
		int result = 0;

		//WAM에서 WAQ에 적재할 내용을 가져온다...
		ArrayList<WaqErrMsgMap> wamlist = waqmapper.selectwamlist(reqmst, list);
