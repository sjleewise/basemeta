 ...
 *
 * 1. FileName : MsgStdRqstServiceImpl.java
...
import kr.wise.meta.stnd.service.WaqStdErrMsgMapper;
 ...
 * <PRE>
 * 1. ClassName :
 * 2. FileName  : MsgStdRqstServiceImpl.java
 ...
 * </PRE>
 */
@Service("msgStdRqstService")
public class MsgStdRqstServiceImpl implements MsgStdRqstService {

	private final Logger logger = LoggerFactory.getLogger(getClass());

	@Inject
	private WaqStdErrMsgMapper waqmapper;
	...
	public int register(WaqMstr mstVo, List<?> reglist) throws Exception {
		...
		if(reglist != null) {
			for (WaqStdErrMsg saveVo : (ArrayList<WaqStdErrMsg>)reglist) {
				//요청번호 셋팅
...
//					saveVo.setSysDivCd("M");
	...
	/** @return  */
	public int saveRqstMsg(WaqStdErrMsg saveVo) {
		int result = 0;
		try{
	...
	/** 메시지 요청 상세내용 조회  */
	public WaqStdErrMsg getMsgRqstDetail(WaqStdErrMsg searchVo) {
	...
	/** 메시지 요청 리스트 조회  */
	public List<WaqStdErrMsg> getMsgRqstList(WaqMstr search) {
		return waqmapper.selectMsgRqstListbyMst(search);
	}
	
		/** 메시지 삭제(waq) 
	 * @throws Exception */
	public int delMsgRqstList(WaqMstr reqmst, ArrayList<WaqStdErrMsg> list) throws Exception {
		int result = 0;
		//TODO 성능 문제 발생시 한방 SQL로 처리한다.
		//result = waqmapper.deleteitemrqst(reqmst, list);
		for (WaqStdErrMsg savevo : list) {
	...
	public int check(WaqMstr mstVo) {
		...
		waqRqstVrfDtls.setBizDtlCd("SEM");
...
//		waqmapper.checkMsgCdName(checkmap);
		...
		checkmap.put("vrfDtlCd", "MS010");
		waqmapper.checkMsgCdNameMache(checkmap);
		
		checkmap.put("vrfDtlCd", "MS011");
		waqmapper.checkMsgCdNamecheck(checkmap);
...
//		boolean waq2wam = requestApproveService.setApproveProcess(mstVo, "WAQ_DMN");
	...
	public int regWaq2Wam(WaqMstr mstVo) throws Exception {
		...
		//신규 대상인 경우 ID를 채번한다.
		List<WaqStdErrMsg> waqclist = waqmapper.selectWaqC(rqstno);
		for (WaqStdErrMsg savevo : waqclist) {
			String id =  objectIdGnrService.getNextStringId();
			savevo.setStdErrMsgId(id);
...
//		result += waqmapper.updateUppDmnId(rqstno);
	...
	public int regWam2Waq(WaqMstr reqmst, List<WaqStdErrMsg> list) throws Exception {
		int result = 0;
		logger.debug("list : " + list);
		//WAM에서 WAQ에 적재할 내용을 가져온다...
		ArrayList<WaqStdErrMsg> wamlist = waqmapper.selectwamlist(reqmst, list);
