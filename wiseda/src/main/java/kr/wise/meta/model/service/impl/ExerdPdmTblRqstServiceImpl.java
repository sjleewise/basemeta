...
import kr.wise.sysinf.exerd.service.ExerdMartMapper;
 ...
 * </PRE>
 */
@Service("exerdPdmTblRqstService")
public class ExerdPdmTblRqstServiceImpl implements ExerdPdmTblRqstService {
	...
	@Inject
	private	ExerdMartMapper exerdMapper;
...
//			waqcolmapper.insertByRqstMartCol(saveVo);
			List<WaqPdmCol> collist = exerdMapper.selectExerdColListbyTblId(saveVo);
			
			regPdmColList(collist);
...
//		waqcolmapper.updateNonObjEnm(rqstNo);
		...
		//컬럼에 오류가 있는 경우 체크(PT999)-단 변경사항 없음은 제외한다.
...
//		String rqstno = reqmst.getRqstNo();
		...
		for (WaqPdmCol savevo : list) {
			if(savevo.getPkYn().equals("Y")){
				savevo.setPkOrd(pkOrd);
				pkOrd++;
			}
//			savevo.setRqstNo(rqstno);
...
//			Integer rqstSno = tblvo.getRqstSno();
	...
	/** insomnia
	 * @throws Exception */
	public int regExerd2Waq(WaqMstr reqmst, ArrayList<WaqPdmTbl> list) throws Exception {
		int result = 0;

//		List<WaqPdmTbl> martlist = exerdMapper.selectexerdlist(reqmst, list);
	...
	@Override
	public List<WaqPrfRelColVO> getExerdRelLst(WaqPrfRelColVO search) {
		List<WaqPrfRelColVO> list = exerdMapper.selectExerdRelLst(search);
		return list;
	}
	
	@Override
	public List<WaqPdmTbl> getExerdTblList(WaqPdmTbl search) {
		String[] arrFullPath = UtilString.null2Blank(search.getSubjLnm()).split("\\>");
		search.setMdlLnm(arrFullPath[0]);
		search.setSubjLnm(arrFullPath[1]);
		
		List<WaqPdmTbl> list = exerdMapper.selectExerdList(search);
		for(int i=0; i<list.size(); i++){
			list.get(i).setIbsStatus("I");
		}
		return list;
	}
