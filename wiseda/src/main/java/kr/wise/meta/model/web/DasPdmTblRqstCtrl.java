 ...
 * </PRE>
 */
@Controller("DasPdmTblRqstCtrl")
public class DasPdmTblRqstCtrl {
	...
	@Inject
	private DasPdmTblRqstService daspdmTblRqstService;
...
//		codeMap.put("usergTypCd", cmcdCodeService.getCodeList("USERG_TYP_CD")); //사용자그룹유형코드
    ...
    @RequestMapping("/meta/model/das_pdmtbl_rqst.do")
...
//    	logger.debug("\n dasDist:" + dasDist); 
		...
		returnstr = "/meta/model/das_pdmtbl_rqst"; 
     ...
     * @throws Exception */
    @RequestMapping("/meta/model/popup/das_pdmtblSearchPop.do")
    public String goDasMartTblPop(@ModelAttribute("search") WamPdmTbl search, Model model, Locale locale) throws Exception {
    	logger.debug("{}", search);
    	...
    	return "/meta/model/popup/das_pdmTblSearch_pop";
    }

    /** R9 모델마트 테이블 조회 - IBSHEET JSON @return insomnia */
    @RequestMapping("/meta/model/getMartDasTblList.do")
    @ResponseBody
    public IBSheetListVO<WaqPdmTbl> getMartDasTblList(WaqPdmTbl search, Locale loclae) throws Exception {
    	logger.debug("{}", search);
    	
    	List<WaqPdmTbl> list = daspdmTblRqstService.getMartDasTblList(search);
//    	logger.debug("{}", list);
...
//    /** 물리모델 테이블 변경대상 요청 리스트 추가... @throws Exception insomnia */
    @RequestMapping("/meta/model/regMartDasWaqPdmTbl.do")
    @ResponseBody
	public IBSResultVO<WaqMstr> regMartDasWaqlist(@RequestBody WaqPdmTbls data, WaqMstr reqmst, Locale locale) throws Exception {

		logger.debug("reqmst:{} \ndata:{}", reqmst, data);

		ArrayList<WaqPdmTbl> list = data.get("data");

		int result = daspdmTblRqstService.regMartDasWaq(reqmst, list);
