...
public interface WamPdmTblMapper {
	...
	List<GovVo> selectGovList(GovVo search);

	GovVo selectOrgNm();

	int insertServerInfo(WaaGovServerInfo record);

	WaaGovServerInfo getGovServerInfo();

	int updateServerInfo(WaaGovServerInfo record);

	List<GovVo> selectGovListForSendFile(GovVo search);

	int connTestGovServer(@Param("resMsg") String resMsg, @Param("resErrMsg") String resErrMsg);

	List<PdmTblColDownVo> selectPdmTblForDownBaiExl(PdmTblColDownVo search);

	List<PdmTblColDownVo> selectPdmColForDownBaiExl(PdmTblColDownVo search);
