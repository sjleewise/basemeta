...
import kr.wise.meta.model.service.ExerdPdmTblRqstService;
 ...
 * </PRE>
 */
@Controller("ExerdPdmTblRqstCtrl")
public class ExerdPdmTblRqstCtrl {
	...
	@Inject
	private ExerdPdmTblRqstService exerdpdmTblRqstService;
...
//		codeMap.put("usergTypCd", cmcdCodeService.getCodeList("USERG_TYP_CD")); //사용자그룹유형코드
    ...
    @RequestMapping("/meta/model/exerd_pdmtbl_rqst.do")
    public String goModelPdmTblRqst(WaqMstr reqmst, ModelMap model) throws Exception {
    	...
    	String returnstr = "/meta/model/exerd_pdmtbl_rqst";

    	return returnstr;
    }


    /** EXERD 모델마트 테이블 검색 팝업... @return insomnia */
//    @RequestMapping("/meta/model/popup/exerd_pdmtblSearchPop.do")
//    public String goExerdMartTblPop(@ModelAttribute("search") WamPdmTbl search, Model model, Locale locale) {
//    	logger.debug("{}", search);
//
//    	return "/meta/model/popup/exerd_pdmTblSearch_pop";
//    }
    
    /** EXERD 모델마트 테이블 검색 팝업... @return insomnia 
     * @throws Exception */
    @RequestMapping("/meta/model/popup/exerd_pdmtblSearchPop.do")
    public String goExerdMartTblPop(@ModelAttribute("search") WamPdmTbl search, Model model, Locale locale) throws Exception {
    	logger.debug("{}", search);
    ...
    @RequestMapping("/meta/model/ajaxgrid/exerd_pdmtbl_rqst_dtl.do")
    public String getpdmtblDetail(WaqPdmTbl searchVo, ModelMap model) {
    	...
    	if(searchVo.getRqstSno() == null) {
    		...
    		WaqPdmTbl resultvo =  exerdpdmTblRqstService.getPdmTblRqstDetail(searchVo);
    		model.addAttribute("result", resultvo);
    		model.addAttribute("saction", "U");
    	}

    	return "/meta/model/exerd_pdmtbl_rqst_dtl";
...
//	}
//
    /** EXERD 모델마트 테이블 조회 - IBSHEET JSON @return insomnia */
    @RequestMapping("/meta/model/getexerdtbllist.do")
    @ResponseBody
    public IBSheetListVO<WaqPdmTbl> getMartTblList(WaqPdmTbl search, Locale loclae) {
    	logger.debug("{}", search);
    	List<WaqPdmTbl> list = exerdpdmTblRqstService.getExerdTblList(search);
    	
    	
    	
    	for(int i=0; i<list.size(); i++){
    		list.get(i).setSubjId(search.getSubjId());
    	}
...
//    	List<WamPdmTbl> listm = modelPdmService.getPdmTblList(searchm);
    ...
    @RequestMapping("/meta/model/regExerd2WaqPdmTbl.do")
    @ResponseBody
	public IBSResultVO<WaqMstr> regExerd2Waqlist(@RequestBody WaqPdmTbls data, WaqMstr reqmst, Locale locale) throws Exception {

		logger.debug("reqmst:{} \ndata:{}", reqmst, data);

		ArrayList<WaqPdmTbl> list = data.get("data");

		int result = exerdpdmTblRqstService.regExerd2Waq(reqmst, list);
