 ...
 * <PRE>
 * 1. ClassName :
 * 2. FileName  : DdlTsfIdxRqstCtrl.java
 * 3. Package  : kr.wise.meta.ddltsf.web
 * 4. Comment  : DDL 인덱스 이관 요청 컨트롤러...
 * 5. 작성자   : AJH
 * 6. 작성일   : 2020. 09. 01. 
 * </PRE>
 */
@Controller("DdlTsfIdxRqstCtrl")
public class DdlTsfIdxRqstCtrl {
	...
	static class WaqDdlTsfIdxs extends HashMap<String, ArrayList<WaqDdlTsfIdx>> {}

	static class WaqDdlTsfIdxCols extends HashMap<String, ArrayList<WaqDdlTsfIdxCol>> {}
...
//		codeMap.put("sysareaibs", sysareaibs);
    ...
    @RequestMapping("/meta/ddltsf/ddltsfidx_rqst.do")
    public String goModelPdmTblRqst(WaqMstr reqmst, ModelMap model) throws Exception {
    	...
    	return "/meta/ddltsf/ddltsfidx_rqst";

    }

    /** DDL 이관 요청 리스트 조회 @return yeonho */
    @RequestMapping("/meta/ddltsf/getDdlTsfIdxRqstList.do")
    @ResponseBody
	public IBSheetListVO<WaqDdlIdx> getDdlTsfIdxRqstList(WaqMstr search) {

		logger.debug("search:{}", search);

		List<WaqDdlIdx> list = ddlIdxRqstService.getDdlTsfIdxRqstList(search); 
    ...
    @RequestMapping("/meta/ddltsf/ddltsfidx_rqst_dtl.do")
    public String getDdlidxDetail(WaqDdlIdx searchVo, ModelMap model) {
    	...
    	if(searchVo.getRqstSno() == null) {
    		...
    		WaqDdlIdx resultvo =  ddlTsfRqstService.getDdlTsfIdxRqstDetail(searchVo);
    		model.addAttribute("result", resultvo);
    		model.addAttribute("saction", "U");
    	}

    	return "/meta/ddltsf/ddltsfidx_rqst_dtl";
...
//    public String goDdlIdxPop(@ModelAttribute("search") WaaDbMapVo search, Model model, Locale locale) {
//		logger.debug("search:{}", search);
//
