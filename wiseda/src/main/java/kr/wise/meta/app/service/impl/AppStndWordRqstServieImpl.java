 ...
 *
 * 1. FileName : AppStndWordRqstServieImpl.java
 * 2. Package : kr.wise.meta.app.web.service.impl
 * 3. Comment :
 * 4. 작성자  : mse
 ...
 * <PRE>
 * 1. ClassName :
 * 2. FileName  : AppStndWordRqstServieImpl.java
 * 3. Package  : kr.wise.meta.app.web.service.impl
 * 4. Comment  :
 * 5. 작성자   : mse
 * 6. 작성일   : 2016. 3. 16. 오전 10:55:20
 * </PRE>
 */
@Service("appStndWordRqstServie")
public class AppStndWordRqstServieImpl implements AppStndWordRqstServie {
	...
	@Inject
	private WaqAppStwdMapper waqmapper;
	...
	public int check(WaqMstr mstVo) throws Exception {
		...
		waqRqstVrfDtls.setBizDtlCd("APD");
		...
		checkmap.put("tblnm"	, "WAQ_APP_STWD");
		checkmap.put("rqstNo"	, rqstNo);
		checkmap.put("bizDtlCd"	, "APD");

		//등록요청중인 표준단어 검증(AD011)
		checkmap.put("vrfDtlCd", "AD011");
		waqmapper.checkRequestStwd(checkmap);

		//요청서내 중복자료 검증(AD001)
		checkmap.put("vrfDtlCd", "AD001");
		waqmapper.checkDupStwd(checkmap);
		//미존재 표준단어(삭제시)(AD002)
		checkmap.put("vrfDtlCd", "AD002");
		waqmapper.checkNotExistStwd(checkmap);
		//표준단어 한글명 중복(AD003)
//		checkmap.put("vrfDtlCd", "AD003");
		//표준단어 영문명 중복(AD004)
//		checkmap.put("vrfDtlCd", "AD004");
		//사용중 표준단어(삭제시)(AD005)
//		checkmap.put("vrfDtlCd", "AD005");
//		waqmapper.checkUseStwd(checkmap);
		//표준단어 한글명 공백 존재(AD007)
		checkmap.put("vrfDtlCd", "AD007");
		waqmapper.checkLnmExistSpac(checkmap);
		//표준단어 영문명 공백 존재(AD008)
		checkmap.put("vrfDtlCd", "AD008");
		waqmapper.checkPnmExistSpac(checkmap);
		//표준단어 한글명 길이 초과(AD009)
		checkmap.put("vrfDtlCd", "AD009");
		waqmapper.checkLnmMaxLen(checkmap);
		//표준단어 영문명 길이 초과(AD010)
		checkmap.put("vrfDtlCd", "AD010");
		waqmapper.checkPnmMaxLen(checkmap);
		//표준단어 변경된 데이터 없음(AD000)
		checkmap.put("vrfDtlCd", "AD000");
		waqmapper.checkNoChg(checkmap);

		//등록가능여부(검증코드) 업데이트
		result += waqRqstVrfDtlsMapper.updateVrfCd("WAQ_APP_STWD", rqstNo, "APD");
	...
	public List<WaqAppStwd> getStndWordRqstist(WaqMstr search) {

		return waqmapper.selectrqstStndWordListbyMst(search);
	}

	/** insomnia */
	public int regStndWordRqstlist(ArrayList<WaqAppStwd> list) {
		int result = 0;

		LoginVO user = (LoginVO) UserDetailHelper.getAuthenticatedUser();
		String userid = user.getUniqId();

		for (WaqAppStwd saveVo : list) {
...
//			result += saveStndWordRqst(saveVo);
	...
	public WaqAppStwd getStndWordRqstDetail(WaqAppStwd searchVO) {
	...
	public int register(WaqMstr mstVo, List<?> reglist) throws Exception {
		...
		if(reglist != null) {
			for (WaqAppStwd saveVo : (ArrayList<WaqAppStwd>)reglist) {
				//요청번호 셋팅...
...
//		requestMstService.updateWaqMstrqstNm(mstVo.getRqstNo(), "WAQ_STWD", "STWD_LNM",  bizdtlnm);
	...
	/** @return insomnia
	 * @throws Exception */
	private int saveRqstStndWord(WaqAppStwd saveVo) throws Exception {
...
//			saveVo.setStwdId(objid);
	...
	public int approve(WaqMstr mstVo, List<?> reglist) throws Exception {
		...
		boolean waq2wam = requestApproveService.setApproveProcess(mstVo, "WAQ_APP_STWD");
	...
	public int regWaq2Wam(WaqMstr mstVo) throws Exception {
		...
		//신규 대상인 경우 ID 채번한다.
		List<WaqAppStwd> waqclist =  waqmapper.selectWaqC(rqstno);

		for (WaqAppStwd savevo : waqclist) {
			String id = objectIdGnrService.getNextStringId();
			savevo.setAppStwdId(id);
//			savevo.setIbsStatus("U");
	 ...
	 * @throws Exception */
	public int regWam2Waq(WaqMstr reqmst, ArrayList<WaqAppStwd> list) throws Exception {
	...
	/** insomnia
	 * @throws Exception */
	public int delStndWordRqstList(WaqMstr reqmst, ArrayList<WaqAppStwd> list) throws Exception {
		int result = 0;
	...
	/** insomnia */
	public int regapprove(WaqMstr mstVo, ArrayList<WaqAppStwd> reglist) {
		int result = 0;
